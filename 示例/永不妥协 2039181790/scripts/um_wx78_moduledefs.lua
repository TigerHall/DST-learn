...
local function maxsanity1_activate(inst, wx, isloading)
    if wx.components.sanity ~= nil then
        ...
        if wx._cherriftchips and wx._cherriftchips > 0 then
            wx.components.sanity.dapperness = wx.components.sanity.dapperness +
            TUNING.DAPPERNESS_TINY * (1 + 0.75 * wx._cherriftchips)
            wx.components.sanity:SetMax(wx.components.sanity.max +
            TUNING.WX78_MAXSANITY1_BOOST * (1 + 0.5 * wx._cherriftchips))
        else
...
local function maxsanity_activate(inst, wx, isloading)
    if wx.components.sanity ~= nil then
        ...
        if wx._cherriftchips and wx._cherriftchips > 0 then
            wx.components.sanity.dapperness = wx.components.sanity.dapperness +
            TUNING.WX78_MAXSANITY_DAPPERNESS * (1 + 0.75 * wx._cherriftchips)
            wx.components.sanity:SetMax(wx.components.sanity.max +
            TUNING.WX78_MAXSANITY_BOOST * (1 + 0.5 * wx._cherriftchips))
        else
...
local function thawtask(obj, wx)
    ...
    if percent > 0 then
        PushColour(obj, FREEZE_COLOUR[1] * percent, FREEZE_COLOUR[2] * percent, FREEZE_COLOUR[3] * percent,
...
local function onobjectfreeze(wx, data, inst)
    ...
    if obj then
        ...
        if not obj:HasTag("frozen") then
            if obj.components.workable.workmultiplierfn == nil then obj.components.workable:SetWorkMultiplierFn(
                workmultiplierfn) end
    ...
    if (data and data.attacker and not data.redirected) and not inst._cdtask then
        ...
        if data.attacker.components.combat and not (data.attacker.components.health and data.attacker.components.health:IsDead())
            ...
            data.attacker.components.combat:GetAttacked(wx,
                damage_mult *
                (TUNING.WX78_TASERDAMAGE + (wx._cherriftchips and wx._cherriftchips > 0 and 10 * wx._cherriftchips or 0)),
                nil, "electric")
...
local function maxhunger_activate(inst, wx, isloading)
    if wx.components.hunger ~= nil then
        ...
        if wx._cherriftchips and wx._cherriftchips > 0 then
            wx.components.hunger:SetMax(wx.components.hunger.max +
            TUNING.WX78_MAXHUNGER_BOOST * (1 + 0.5 * wx._cherriftchips))
        else
        ...
        if wx._cherriftchips and wx._cherriftchips > 0 then
            wx.components.hunger.burnratemodifiers:SetModifier(inst,
                TUNING.WX78_MAXHUNGER_SLOWPERCENT * (0.9 ^ wx._cherriftchips))
        else
...
local function maxhunger1_activate(inst, wx, isloading)
    if wx.components.hunger ~= nil then
        ...
        if wx._cherriftchips and wx._cherriftchips > 0 then
            wx.components.hunger:SetMax(wx.components.hunger.max +
            TUNING.WX78_MAXHUNGER1_BOOST * (1 + 0.5 * wx._cherriftchips))
        else
...
local function music_activate(inst, wx)
    ...
    if wx._cherriftchips and wx._cherriftchips > 0 then
        wx.components.sanity.dapperness = wx.components.sanity.dapperness +
        TUNING.WX78_MUSIC_DAPPERNESS * (1.25 ^ wx._cherriftchips)
    else
    ...
    if wx.components.birdattractor ~= nil then
        ...
        wx.components.birdattractor.spawnmodifier:SetModifier(inst, TUNING.BIRD_SPAWN_DELAYDELTA_FEATHERHAT.MIN,
            "mindelay")
        wx.components.birdattractor.spawnmodifier:SetModifier(inst, TUNING.BIRD_SPAWN_DELAYDELTA_FEATHERHAT.MAX,
            "maxdelay")
    end
