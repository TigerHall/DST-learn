...
local assets =
{
	Asset("ANIM", "anim/terror_lemon_bomb.zip"),
}

local prefabs =
{
	"terror_lemon_bomb_explode_fx",
	"reticule",
	"reticuleaoe",
	"reticuleaoeping",
}
...
local function OnHit(inst, attacker, target)
	...
	inst.components.explosive.explosiverange = TUNING.TERROR_LEMON_BOMB_EXPLOSION_RANGE
	inst.components.explosive.explosivedamage = TUNING.TERROR_LEMON_BOMB_EXPLOSION_DAMAGE
    inst.components.explosive.buildingdamage = 0
	inst.components.explosive.lightonexplode = true
	if inst.ispvp then
	...
	SpawnPrefab("terror_lemon_bomb_explode_fx").Transform:SetPosition(x, y, z)
end
...
local function onequip(inst, owner)
	...
	if skin_build ~= nil then
		...
		owner.AnimState:OverrideItemSkinSymbol("swap_object", skin_build, "swap_terror_lemon_bomb", inst.GUID, "terror_lemon_bomb")
	else
		owner.AnimState:OverrideSymbol("swap_object", "terror_lemon_bomb", "swap_terror_lemon_bomb")
	end
...
local function CreateSpinCore()
	...
	inst.AnimState:SetBank("terror_lemon_bomb")
	inst.AnimState:SetBuild("terror_lemon_bomb")
...
local function fn()
	...
	inst.spinticks = net_smallbyte(inst.GUID, "terror_lemon_bomb.spinticks", "spinticksdirty")
    ...
    inst.components.inventoryitem.atlasname = "images/inventoryimages/terror_lemon_bomb.xml"
...
return  Prefab("terror_lemon_bomb", fn, assets, prefabs),
	    Prefab("terror_lemon_bomb_explode_fx", fxfn, assets)
