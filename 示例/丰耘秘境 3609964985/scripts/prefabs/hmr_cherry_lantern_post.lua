...
local function MakeLanternPost(name)
	local assets =
	{
		Asset("ANIM", "anim/"..name.."_lantern_post.zip"),
		Asset("ANIM", "anim/ui_chest_1x1.zip"),
		Asset("ANIM", "anim/reticuledash.zip"),
		Asset("ATLAS", "images/inventoryimages/"..name.."_lantern_post_item.xml"),
		Asset("IMAGE", "images/inventoryimages/"..name.."_lantern_post_item.tex"),
	}

	local prefabs =
	{
		name.."_lantern_light",
		name.."_lantern_light_chain",
	}
	...
	local LIGHT_COLOUR = Vector3(255 / 255, 255 / 255, 128 / 255)
	...
	local function is_battery_type(item)
		return item:HasTag("lightbattery")
			...
			or item:HasTag("hmr_cherry_rock_item")
	end
	...
	local function FindPartners(inst)
		...
		local function spawnlight(inst, partner)
			...
			local newlight = SpawnPrefab(name.."_lantern_light_chain")
	...
	local function lightson(inst)
		...
		if not inst.light_obj then
			local newlight = SpawnPrefab(name.."_lantern_light")
	...
	local function onbuilt(inst)
		...
		local light = SpawnPrefab("hmr_cherry_rock_item")
	...
	local function SpawnButterflies(inst)
		local x, y, z = inst.Transform:GetWorldPosition()
		inst._butterfly_task = inst:DoPeriodicTask(4, function()
			local butterfly = SpawnPrefab("hmr_cherry_lantern_post_fx")
			if butterfly then
				local anim_num = math.random(1, 3)
				butterfly.AnimState:PlayAnimation("idle"..anim_num)
				for i = 1, 5 do
					butterfly.AnimState:PushAnimation("idle"..anim_num)
				end
				local r = math.random() * 0.5 + 0.5
				local theta = math.random()*2*PI
				butterfly.Transform:SetPosition(x+r*math.cos(theta), anim_num == 2 and 0 or math.random() * 6, z-r*math.sin(theta))
			end
		end)
	end

	local function StopSpawningButterflies(inst)
		if inst and inst._butterfly_task then
			inst._butterfly_task:Cancel()
			inst._butterfly_task = nil
		end
	end
	...
	local function fn()
		...
		inst.AnimState:SetBank(name.."_lantern_post")
		inst.AnimState:SetBuild(name.."_lantern_post")
		inst.AnimState:PlayAnimation("idle", true)

		inst:AddTag(name.."_post")

		inst.AnimState:Hide("light_on")

		-- inst.MiniMapEntity:SetIcon(name.."_lantern_post.png")
		...
		inst.components.container:WidgetSetup(name.."_lantern_post")
		...
		if name == "hmr_cherry" then
			inst:DoTaskInTime(0, SpawnButterflies)
			inst:ListenForEvent("onremove", StopSpawningButterflies)
		end
	...
	local function OnUpdateFn(inst, dt)
		if inst == nil or ThePlayer == nil then
			return
		end
	...
	local function lightchainfn()
		...
		inst.partner1 = net_entity(inst.GUID, name.."_lantern_light.partner1", "partner1dirty")
		inst.partner2 = net_entity(inst.GUID, name.."_lantern_light.partner2", "partner2dirty")
		inst.enablelight = net_tinybyte(inst.GUID, name.."_lantern_light.enablelight", "lightdirty")
	...
	local deployable_data =
	{
		...
		master_postinit = function(inst)
			inst.components.inventoryitem.atlasname = "images/inventoryimages/"..name.."_lantern_post_item.xml"
		end
	}

	table.insert(prefs, Prefab(name.."_lantern_post", fn, assets, prefabs))
	table.insert(prefs, Prefab(name.."_lantern_light", lightfn, assets, prefabs))
	table.insert(prefs, Prefab(name.."_lantern_light_chain", lightchainfn, assets, prefabs))
	table.insert(prefs, MakeDeployableKitItem(name.."_lantern_post_item", name.."_lantern_post", name.."_lantern_post", name.."_lantern_post", "kit", {Asset("ANIM", "anim/"..name.."_lantern_post.zip")}, {size = "med", scale = 0.77}, nil, {fuelvalue = TUNING.LARGE_FUEL}, deployable_data))
	table.insert(prefs, MakePlacer(name.."_lantern_post_item_placer", name.."_lantern_post", name.."_lantern_post", "placer", nil, nil, nil, nil, nil, nil, placer_postinit_fn))
end

local fx_assets = {
    Asset("ANIM", "anim/hmr_cherry_lantern_post_fx.zip")
}

-- 在指定的时间（appear_time）内逐渐显现
local function AppearGradually(inst, appear_time)
    local time_to_appear = appear_time or 1
    ...
    inst:StartThread(function()
        local ticks = 0
        while ticks * tick_time < time_to_appear do
            local appear_amount = ticks * tick_time / time_to_appear
...
local function fx_fn()
    ...
    inst.AnimState:SetBank("hmr_cherry_lantern_post_fx")
    inst.AnimState:SetBuild("hmr_cherry_lantern_post_fx")
    ...
    AppearGradually(inst, 1)
    inst:DoTaskInTime(math.random() * 2 + 1, ErodeAway, nil, 1 + math.random() * 0.5)

    inst:ListenForEvent("animqueueover", inst.Remove)

    return inst
end
table.insert(prefs, Prefab("hmr_cherry_lantern_post_fx", fx_fn, fx_assets))

MakeLanternPost("hmr_cherry")
