...
---
    -----------------------------------------------------------------------------------
    ----- 容器界面名 --- 要独特一点，避免冲突
        local container_widget_name = "tbat_container_cherry_blossom_rabbit_mini"
    -----------------------------------------------------------------------------------
    ----- 检查和注册新的容器界面
        local all_container_widgets = require("containers")
        all_container_widgets.MAXITEMSLOTS = math.max(all_container_widgets.MAXITEMSLOTS, 5*5)
        ...
        if params[container_widget_name] == nil then
            params[container_widget_name] = {
                ...
                -- type = "chest",
                type = "tbat_container_cherry_blossom_rabbit_mini",
            ...
            --- 左上角是格子1
                local x_num = 5
                local y_num = 5
                local start_x, start_y = -170, -170
                local horizontal_slot_offset = 91  -- 水平间距
                local vertical_slot_offset = 90    -- 垂直间距
                ...
                for y = 1, y_num do
                    for x = 1, x_num do
                        local x_pos = horizontal_slot_offset * (x - 1) + start_x  -12
                        local y_pos = vertical_slot_offset * (y_num - y) + start_y  -15
            ...
            ---- item test
                widget_data.itemtestfn =  function(container_com, item, slot)
                    ...
                    --     return false
                    -- end
                    if item.prefab == container_com.inst.prefab then
                        return false
                    end
                    return true
                end
