...
---
    local function widget_open(inst,origin_root)
        ...
        --------------------------------------------------------------------
        --- hook 进官方UI
            local fn = require("prefabs/06_tbat_containers/06_03_02_snail_ui_origin")
            fn(inst,front_root,origin_root)
        --------------------------------------------------------------------
        ---
            front_root.inst:ListenForEvent("close_widget",function()
                front_root:Hide()
                TBAT.FNS:RPC_PushEvent(ThePlayer,"close_widget",nil,inst)
            end)
...
---
    return function(inst)
        ...
        inst:ListenForEvent("close_widget",function()
