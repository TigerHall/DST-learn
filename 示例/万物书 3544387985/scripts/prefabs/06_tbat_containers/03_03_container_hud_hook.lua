...
---
    ...
    local AUTO_SEARCH_FULL_MAP = TBAT.CONFIG.CHERRY_BLOSSOM_RABBIT_SEARCH_RADIUS == 999
    local AUTO_SEARCH_RADIUS = TBAT.CONFIG.CHERRY_BLOSSOM_RABBIT_SEARCH_RADIUS or 300
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
------ 界面控件
...
---
    ...
    local function hud_hook_fn(inst,front_root)
        ...
        --------------------------------------------------------                        
        --- 覆盖背景贴图
            front_root.bganim:GetAnimState():OverrideSymbol("frame","tbat_container_cherry_blossom_rabbit","window_big")
        --------------------------------------------------------                        
        --- close button
            local button_close = front_root:AddChild(AnimButton("tbat_container_cherry_blossom_rabbit",{
                idle = "button_close",
                over = "button_close",
                disabled = "button_close",
            }))
            button_close:SetPosition(-450,-360,0)
        ...
        --------------------------------------------------------                        
        --- button in
            local button_in = front_root:AddChild(AnimButton("tbat_container_cherry_blossom_rabbit",{
                idle = "button_in",
                over = "button_in",
                disabled = "button_close",
            }))
            button_in:SetPosition(450,-360,0)
            button_in:SetOnClick(function()
                ...
                TBAT.FNS:RPC_PushEvent(ThePlayer,"button_in",nil,inst)
            end)
            table.insert(inst.hud_data,button_in)
        --------------------------------------------------------                        
        --- button refresh
            local button_refresh = front_root:AddChild(AnimButton("tbat_container_cherry_blossom_rabbit",{
                idle = "button_flower",
                over = "button_flower",
                disabled = "button_flower",
            }))
            button_refresh:SetPosition(-380,380,0)
            button_refresh:SetOnClick(function()
                ...
                TBAT.FNS:RPC_PushEvent(ThePlayer,"button_refresh",nil,inst)
            end)
            table.insert(inst.hud_data,button_refresh)
        --------------------------------------------------------                        
        --- cook
            -- print(front_root,"6644")
            front_root.bganim.inst:ListenForEvent("animover",function()
                front_root.bganim.inst:RemoveAllEventCallbacks()                
                local cook_bg = front_root:AddChild(UIAnim())
                cook_bg:GetAnimState():SetBank("tbat_container_cherry_blossom_rabbit")
                cook_bg:GetAnimState():SetBuild("tbat_container_cherry_blossom_rabbit")
                cook_bg:GetAnimState():PlayAnimation("window_cook")
                cook_bg:SetPosition(1050,0,0)
                cook_bg:MoveToBack()
                table.insert(inst.hud_data,cook_bg)
                local button_cook = cook_bg:AddChild(AnimButton("tbat_container_cherry_blossom_rabbit",{
                    idle = "button_cook",
                    over = "button_cook",
                    disabled = "button_cook",
                }))
                button_cook:SetPosition(10,-260,0)
                button_cook:SetOnClick(function()
                    if button_cook.cd then
                        return
                    end
                    button_cook.cd = button_cook.inst:DoTaskInTime(0.5,function()
                        button_cook.cd = nil
                    end)
                    TBAT.FNS:RPC_PushEvent(ThePlayer,"button_cook",nil,inst)
                end)
                -- front_root.move_btm:MoveToBack()
            end)
        --------------------------------------------------------                        
        --- 点击拖动
            -- local move_btm = front_root:AddChild(AnimButton("tbat_container_cherry_blossom_rabbit",{idle = "window_big",over = "window_big",disabled = "window_big",}))
            -- -- move_btm.anim:GetAnimState():OverrideSymbol("window_big","tbat_container_cherry_blossom_rabbit","frame")
            -- front_root.move_btm = move_btm
    ...
    local function search_and_pickup_items(inst)
        ...
        ---
            ...
            local function put_into(tempInst)
                if tempInst and tempInst:IsValid() and tempInst.prefab 
                    and prefab_list[tempInst.prefab]
...
return function(inst)
    ...
    inst:ListenForEvent("button_refresh", container_item_refresh)
    inst:ListenForEvent("button_in", container_item_in)
