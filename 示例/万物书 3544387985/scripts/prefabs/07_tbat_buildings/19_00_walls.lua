...
function MakeWallType(data)
    ...
    local function ondeploywall(inst, pt, deployer)
        ...
        if wall ~= nil then
            inst.components.tbat_com_skin_data:OnDeployItem(wall, deployer)
    ...
    local function itemfn()
        ...
        TBAT.SKIN:SetDefaultBankBuild(inst,bank,"wall_"..data.name)
        ...
        if data.wall_item_master_fn then
            data.wall_item_master_fn(inst)
        end
        return inst
    end
    ...
    local function fn()
        ...
        inst:AddComponent("inspectable")
        inst.components.inspectable:SetDescription(TBAT:GetString2("wall_"..data.name,"inspect_str"))
        ...
        if data.wall_master_fn then
            data.wall_master_fn(inst)
        end
        ------------------------------------------------
            inst:AddComponent("tbat_com_skin_data")
        ------------------------------------------------
...
local walldata =
{
    { 
        ...
        material = "wood",
        tags = { "wood" },
        loot = "tbat_material_white_plum_blossom",
        maxloots = 2,
        maxhealth = TUNING.WOODWALL_HEALTH,
        flammable = true,
        buildsound = "dontstarve/common/place_structure_wood",
        wall_master_fn = function(inst)
            inst.components.repairable.repairmaterial = "wood"
            inst.components.propagator.flashpoint = 30 + math.random() * 10
        end,
        wall_item_master_fn = function(inst)
            inst.components.inventoryitem:TBATInit("wall_tbat_wood","images/map_icons/wall_tbat_wood.xml")
    ...
    { 
        ...
        material = "straw",
        tags = { "grass" },
        loot = "tbat_item_holo_maple_leaf",
        maxloots = 2,
        maxhealth = TUNING.WOODWALL_HEALTH,
        flammable = true,
        buildsound = "dontstarve/common/place_structure_wood",
        ...
        wall_item_master_fn = function(inst)
            inst.components.inventoryitem:TBATInit("wall_tbat_maple","images/map_icons/wall_tbat_maple.xml")
            inst.components.repairer.repairmaterial = MATERIALS.HAY
        end,
    },
    { 
        name = "tbat_osmanthus_stone",
        assets = require("prefabs/07_tbat_buildings/19_01_wall_skins_for_osmanthus_stone"),
        material = "stone",
        tags = { "stone" },
        loot = "tbat_material_osmanthus_ball",
        maxloots = 2,
        maxhealth = TUNING.WOODWALL_HEALTH,
        flammable = false,
        ...
        wall_item_master_fn = function(inst)
            inst.components.inventoryitem:TBATInit("wall_tbat_osmanthus_stone","images/map_icons/wall_tbat_osmanthus_stone.xml")
