...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 前置准备
    local this_prefab = "tbat_building_cherry_blossom_rabbit_swing"
...
--- Assets素材资源
    local assets =
    {
        ...
        Asset("ANIM", "anim/tbat_building_cherry_blossom_rabbit_swing_2.zip"),
    }
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 皮肤API套件
    local skins_data = {
        ["tbat_building_cherry_blossom_rabbit_swing_2"] = {                    --- 
            bank = "tbat_building_cherry_blossom_rabbit_swing_2",
            build = "tbat_building_cherry_blossom_rabbit_swing_2",
            atlas = "images/map_icons/tbat_building_cherry_blossom_rabbit_swing.xml",
            image = "tbat_building_cherry_blossom_rabbit_swing",  -- 不需要 .tex
            name = "ANIM-2",        --- 切名字用的
    ...
    TBAT.SKIN:AddForDefaultUnlock("tbat_building_cherry_blossom_rabbit_swing_2")
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- sitting event
    local function player_on_set_event(inst,doer)
        ...
        --- 饥饿控制
            if doer.components.hunger then
                doer.components.hunger.burnratemodifiers:SetModifier(inst,0)
            end
        --------------------------------------------------------------------
        --- 血量控制
    ...
    local function player_stop_sitting_event(inst,doer)
        ...
        --- 饥饿控制
            if doer.components.hunger then
                doer.components.hunger.burnratemodifiers:RemoveModifier(inst)
            end
        --------------------------------------------------------------------
        --- 血量控制
            if inst.__health_task then
...
--- 创建物品
    local function fn()
        ...
        TBAT.SKIN:SetDefaultBankBuild(inst,"tbat_building_cherry_blossom_rabbit_swing","tbat_building_cherry_blossom_rabbit_swing")
