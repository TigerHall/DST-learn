...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 前置准备
    local this_prefab = "tbat_building_stump_table"
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- Assets素材资源
    local assets =
    {
        Asset("ANIM", "anim/tbat_building_stump_table.zip"),
        Asset("ANIM", "anim/tbat_building_stump_table_sunbloom_side_table.zip"),
    }
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 皮肤API套件
    local skins_data = {
        ["tbat_sunbloom_side_table"] = {                    --- 
            bank = "tbat_building_stump_table_sunbloom_side_table",
            build = "tbat_building_stump_table_sunbloom_side_table",
            atlas = "images/map_icons/tbat_building_stump_table_sunbloom_side_table.xml",
            image = "tbat_building_stump_table_sunbloom_side_table",  -- 不需要 .tex
            name = TBAT:GetString2(this_prefab,"skin.sunbloom_side_table"),        --- 切名字用的
            name_color = "pink",
            server_fn = function(inst) -- 切换到这个skin调用 。服务端
                inst.MiniMapEntity:SetIcon("tbat_building_stump_table_sunbloom_side_table.tex")
                inst.AnimState:PlayAnimation("idle",true)
                inst.AnimState:SetTime(2.5*math.random())
                inst.___sunbloom_side_table_event_fn = inst.___sunbloom_side_table_event_fn  or function(inst)
                    local item = inst.components.container:GetItemInSlot(1)
                    if item and item:IsValid() then
                        inst.AnimState:Hide("ITEM")
                    else
                        inst.AnimState:Show("ITEM")
                    end
                end
                inst.___sunbloom_side_table_event_fn(inst)
                inst:RemoveEventCallback("display_inner_item",inst.___sunbloom_side_table_event_fn)
                inst:ListenForEvent("display_inner_item",inst.___sunbloom_side_table_event_fn)
            end,
            server_switch_out_fn = function(inst) -- 切换离开这个皮肤用
                inst.MiniMapEntity:SetIcon(this_prefab..".tex")
                if inst.___sunbloom_side_table_event_fn then
                    inst:RemoveEventCallback("display_inner_item",inst.___sunbloom_side_table_event_fn)
                    inst.___sunbloom_side_table_event_fn = nil
                end
                inst.AnimState:Show("ITEM")
            end,
            unlock_announce_data = { -- 解锁提示
                ...
                fn = function(icon_anim,slot)
                    icon_anim:GetAnimState():OverrideSymbol("slot","tbat_building_stump_table_sunbloom_side_table","empty")
                end
            },
            placer_fn = function(inst)
                inst.AnimState:SetBank("tbat_building_stump_table_sunbloom_side_table")
                inst.AnimState:SetBuild("tbat_building_stump_table_sunbloom_side_table")
                inst.AnimState:PlayAnimation("idle",true)
                inst.AnimState:OverrideSymbol("slot","tbat_building_stump_table_sunbloom_side_table","empty")
            end
        },
    }
    TBAT.SKIN:DATA_INIT(skins_data,this_prefab)
    TBAT.SKIN.SKIN_PACK:Pack("pack_warm_and_cozy_home","tbat_sunbloom_side_table")
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 只能放烹饪出来的东西
...
--- 物品显示
    local function display_item(inst)
        ...
        if item then
            ...
            item_fx.Follower:FollowSymbol(inst.GUID, "slot",0,-10,0,true)
            inst.item_fx = item_fx
            inst:PushEvent("display_inner_item",item)
        else
            inst:PushEvent("display_inner_item",nil)
        end
    end
...
--- 创建物品
    local function fn()
        ...
        inst.MiniMapEntity:SetIcon("tbat_building_stump_table.tex")
        TBAT.SKIN:SetDefaultBankBuild(inst,"tbat_building_stump_table","tbat_building_stump_table")
        -- inst.AnimState:SetBank("tbat_building_stump_table")
        -- inst.AnimState:SetBuild("tbat_building_stump_table")
        inst.AnimState:PlayAnimation("idle",true)
        inst.AnimState:OverrideSymbol("slot","tbat_building_stump_table","empty")
