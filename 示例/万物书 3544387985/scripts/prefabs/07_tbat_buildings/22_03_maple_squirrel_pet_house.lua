...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 前置准备
    local this_prefab = "tbat_building_maple_squirrel_pet_house"
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- Assets素材资源
    local assets =
    {
        Asset("ANIM", "anim/tbat_building_maple_squirrel_pet_house.zip"),
    }
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 灯光控制
...
--- 通用
    local function common_fn()
        ...
        inst.MiniMapEntity:SetIcon("tbat_building_maple_squirrel_pet_house.tex")
        MakeObstaclePhysics(inst, .5)
        inst.AnimState:SetBank("tbat_building_maple_squirrel_pet_house")
        inst.AnimState:SetBuild("tbat_building_maple_squirrel_pet_house")
...
---- 野生的房子
    ------------------------------------------------------------------------
    --- 物品接受升级
        local acceptable_com_install = require("prefabs/07_tbat_buildings/22_03_wild_house_shop")
    ------------------------------------------------------------------------
    --- 生成动物
        local cat_prefab = "tbat_animal_maple_squirrel"
...
---- 建造出来的
    local function build_house()
        local inst = common_fn()
        TBAT.PET_MODULES:PetHouseComInstall(inst,"tbat_pet_eyebone_maple_squirrel")
        if not TheWorld.ismastersim then
            return inst
        end
        return inst
