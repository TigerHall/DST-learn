...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 前置准备
    local this_prefab = "tbat_food_fantasy_apple"
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- Assets素材资源
    local assets =
    {
        Asset("ANIM", "anim/tbat_food_fantasy_apple.zip"),
    }
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 
    local foods_data = {
        ...
        [this_prefab.."_cooked"] ={
            ...
            image = "tbat_food_fantasy_apple_cooked",
        },
...
--- 创建物品
    ...
    for temp_prefab, data in pairs(foods_data) do
            ...
            local function fn()
                ...
                inst.AnimState:SetBank("tbat_food_fantasy_apple")
                inst.AnimState:SetBuild("tbat_food_fantasy_apple")
                inst.AnimState:PlayAnimation(data.anim.normal)
                inst:AddTag("cattoy")
                ...
                --- 腐烂
                    ...
                    inst.components.perishable:SetPerishTime(TBAT.PARAM.ONE_DAY*8)
                -----------------------------------------------------------
                --- 可叠堆
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- 添加到烹饪锅
    AddIngredientValues({this_prefab,this_prefab.."_cooked"}, {fruit=1})
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
