...
PLANT_DEFS["tbat_farm_plant_fantasy_apple"]	= {
	build = "tbat_farm_plant_fantasy_apple", 			--这是作物的动画文件，也就是你需要改的地方
	bank = "tbat_farm_plant_fantasy_apple",				--这是bank文件，直接用土豆的就行
	--生长时间
	...
	--农作物代码
	prefab = "tbat_farm_plant_fantasy_apple",			--记得改
	--果实
	product = "tbat_food_fantasy_apple",				--记得改
	--巨型果实
	product_oversized = "tbat_eq_fantasy_apple_oversized",		--记得改
	--种子
	seed = "tbat_food_fantasy_apple_seeds",				--记得改
	--作物标签
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
---
	AddPrefabPostInit("tbat_farm_plant_fantasy_apple",function(inst)
		if not TheWorld.ismastersim then
			return
		end
		local old_UpdateResearchStage = inst.UpdateResearchStage
		inst.UpdateResearchStage = function(self, stage)
			-- print("tbat_farm_plant_fantasy_apple.UpdateResearchStage",stage)
			...
			if stage == 5 and self.is_oversized then
				if math.random() <= .3 or self.force_mutated then
					SpawnPrefab("tbat_farm_plant_fantasy_apple_mutated"):PushEvent("normal_switched",self)
				end
			end
