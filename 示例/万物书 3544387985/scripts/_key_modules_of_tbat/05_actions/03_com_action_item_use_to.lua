...
local TBAT_COM_ITEM_USE_ACTION = Action({priority = 10})   --- 距离 和 目标物体的 碰撞体积有关，为 0 也没法靠近。
TBAT_COM_ITEM_USE_ACTION.id = "TBAT_COM_ITEM_USE_ACTION"
TBAT_COM_ITEM_USE_ACTION.strfn = function(act) --- 客户端检查是否通过,同时返回显示字段
    local item = act.invobject
    local target = act.target
    local doer = act.doer

    if item and doer and target and item.replica.tbat_com_item_use_to then
        local replica_com = item.replica.tbat_com_item_use_to or item.replica._.tbat_com_item_use_to
        if replica_com then
...
TBAT_COM_ITEM_USE_ACTION.fn = function(act)    --- 只在服务端执行~
    local item = act.invobject
    local target = act.target
    local doer = act.doer

    if item and target and doer and item.components.tbat_com_item_use_to then
        local replica_com = item.replica.tbat_com_item_use_to or item.replica._.tbat_com_item_use_to
        if replica_com and replica_com:Test(target,doer,true) then
            return item.components.tbat_com_item_use_to:Active(target,doer)
        end
    end
    return false
end
AddAction(TBAT_COM_ITEM_USE_ACTION)
...
AddComponentAction("USEITEM", "tbat_com_item_use_to", function(item, doer, target, actions, right_click) -- -- 一个物品对另外一个目标用的技能，
    if doer and item and target then
        ...
        if tbat_com_item_use_to_com and tbat_com_item_use_to_com:Test(target,doer,right_click) then
            local this_action = ACTIONS.TBAT_COM_ITEM_USE_ACTION
            this_action.distance = tbat_com_item_use_to_com:GetDistance()
...
AddStategraphActionHandler("wilson",ActionHandler(TBAT_COM_ITEM_USE_ACTION,function(player)
    local creash_flag , ret = pcall(function()
        ...
        local replica_com = item and ( item.replica.tbat_com_item_use_to or item.replica._.tbat_com_item_use_to )
        if replica_com then
    ...
    if creash_flag == true then
        ...
        print("error in TBAT_COM_ITEM_USE_ACTION ActionHandler")
        print(ret)
    end
    return "dolongaction"
end))
AddStategraphActionHandler("wilson_client",ActionHandler(TBAT_COM_ITEM_USE_ACTION, function(player)    
    local creash_flag , ret = pcall(function()
        local target = player.bufferedaction.target
        local item = player.bufferedaction.invobject
        local ret_sg_action = "dolongaction"
