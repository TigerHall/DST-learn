...
local function onbuffinfo(self,buffinfo)
	if self.inst.replica.tbat_showbufftime then
		self.inst.replica.tbat_showbufftime._buffinfo:set(buffinfo)
	end
end
...
--获取Buff时长
function tbat_showbufftime:getBuffTime(inst)
...
--设置buff信息
function tbat_showbufftime:SetBuffInfo()
	local buff_info={}
	local buffstr=""
		...
		for _, v in pairs(self.inst.components.debuffable.debuffs) do
			if v.inst and v.inst.components.timer then
				...
				elseif v.inst.prefab == "buff_medal_confused" then
					...
					--凡骨
				elseif v.inst.prefab == "debuff_mcw_fg" then
					buffdata.bufflayer = self.inst.mcw_fgcount or 1
					--春之舞会
				elseif v.inst.prefab == "debuff_mcw_springdance" then
					buffdata.bufflayer = self.inst.mcw_floralcount or 1
				end
...
--自定义buff信息读取方案
function tbat_showbufftime:SetGetBuffInfoFn(fn)
	self.getbuffinfofn=fn
end
--自定义时间读取方案
function tbat_showbufftime:SetGetBuffTimeFn(fn)
