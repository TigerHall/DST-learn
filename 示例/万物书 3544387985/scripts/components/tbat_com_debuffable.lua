...
--[[
    ...
    复制 官方的同款 Debuffable 组件

    为的是 死亡的时候，挂载在这里的debuff 不会丢失。
...
function Debuffable:TransferComponent(newinst)
    ...
    if data then
        local newcomponent = newinst.components.tbat_com_debuffable
        if not newcomponent then
            newinst:AddComponent("tbat_com_debuffable")
            newcomponent = newinst.components.tbat_com_debuffable
        end
