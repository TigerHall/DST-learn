...
local TextButton = require("widgets/textbutton") --文本按钮模块
local TbatBuffPanel = Class(Widget, function(self,owner)
	Widget._ctor(self, "TbatBuffPanel")
	self.owner = owner
	...
	--展开按钮
	self.open_button = self.root:AddChild(TextButton("images/ui.xml", "blank.tex"))
	self.open_button:SetPosition(-2,-10,0)
	self.open_button:SetTextSize(24) --文本大小
	self.open_button:SetText("展开") --按钮文本
	self.open_button:SetOnClick(function()
		TUNING.TBAT_BUFF_SHOW_NUM=false--解除显示数量限制
		-- SaveMedalSettingData()
		self.close_button:Show()
		self.open_button:Hide()
	end)
	--收起按钮
	self.close_button = self.root:AddChild(TextButton("images/ui.xml", "blank.tex"))
	self.close_button:SetTextSize(24) --文本大小
	self.close_button:SetText("收起") --按钮文本
	...
	self.close_button:SetOnClick(function()
		TUNING.TBAT_BUFF_SHOW_NUM=2--单次最多显示2个
		-- SaveMedalSettingData()
		self.close_button:Hide()
		self.open_button:Show()
	end)
	if TUNING.TBAT_BUFF_SHOW_NUM then
		self.close_button:Hide()
	else
		self.open_button:Hide()
	end
	...
	self:InitBuffLoot()
	self:StartUpdating()
	self:SetPosition(266,266,0)
	self:SetScale(0.8)
	self.isopen=true
end)

--构造单个buff卡
function TbatBuffPanel:BuffCard()
...
function TbatBuffPanel:InitBuffLoot()
...
local other_mod_buff={
	buff_keep_lucky = TBAT.LANGUAGE == "ch" and "保运符" or "Keep Lucky Symbol", --风滚草是私生子,所以有英文名很合理吧
	--神话
	fly_pill_buff = "腾云丹",
...
function TbatBuffPanel:SetBuffInfo()
	local player=self.owner or ThePlayer
	local buff_data
	if player and player.replica.tbat_showbufftime then
		local buffstr=player.replica.tbat_showbufftime:GetBuffInfo()
	...
	if buff_data then
		...
		for k, v in ipairs(buff_data) do
			...
			if TUNING.TBAT_BUFF_SETTING > 1 and buff_str == nil then
				buff_str = v.buffname
			end
			...
			--超量了就不显示了
			if (TUNING.TBAT_BUFF_SHOW_NUM and count>=TUNING.TBAT_BUFF_SHOW_NUM) or count>=self.maxbuffnum then
				break
			end
		end
...
function TbatBuffPanel:OnUpdate(dt)
    if self.isopen and TUNING.TBAT_BUFF_SETTING>0 then
		self:SetBuffInfo()
    end

	if  TUNING.TBAT_BUFF_SETTING>0 and not self.isopen then
		self.isopen=true
		self:SetBuffInfo()
	elseif self.isopen and TUNING.TBAT_BUFF_SETTING==0 then
		self.isopen=nil
		self.root:Hide()
	end
