...
local actionhandlers = 
{
    ...
    ActionHandler(ACTIONS.TBAT_POG_BARK, "bark_at_friend"),  
    ActionHandler(ACTIONS.TBAT_POG_RANSACK, "ransack"),  
...
local function testrummage(inst,target) -- 判断当前正在翻找的箱子是否需要继续翻找。
     if target and target.components.container
        and target.components.container.canbeopened
        and not target:HasTag("nosteal")
        then
            local notags = {"FX", "NOCLICK", "DECOR","INLIMBO", "aquatic","nosteal"}
            local food = FindEntity(inst, 20, function(item) return inst.components.eater:CanEat(item) and item:IsOnValidGround() and item:GetTimeAlive() > 0.5 end, nil, notags)    

            local items = target.components.container:FindItems(function(item) return can_steal_item(inst,item) end)
...
local function tossitems(inst,target)
    if target and target.components.container then
        local items = target.components.container:FindItems(function(item)
            if not target.components.container.canbeopened then
                return false
            end
...
local function bark_at_friends(inst)
    ...
    local ents = TheSim:FindEntities(pt.x,pt.y,pt.z, 4, {"tbat_animal_snow_plum_chieftain"})
    ...
    State{
        ...
        timeline=
        {
            ...
            TimeEvent(16*FRAMES, function(inst)
                -- inst.SoundEmitter:PlaySound("dontstarve/wilson/make_whoosh")
            end),
        },
...
return StateGraph("tbat_animal_snow_plum_chieftain", states, events, "idle", actionhandlers)