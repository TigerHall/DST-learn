...
local assets =
{
    Asset("ANIM", "anim/jx_cookpot.zip"),
    Asset("ANIM", "anim/jx_cookpot_snow_build.zip"),
...
local function SetProductSymbol(inst, product, overridebuild)
    ...
    local realname = recipe ~= nil and (recipe.basename or recipe.name) or product
    local build = (recipe ~= nil and recipe.overridebuild) or overridebuild
    local overridesymbol = (recipe ~= nil and recipe.overridesymbolname) or product
    
    -- 参考棱镜模组的实现：如果没有指定overridebuild，检查是否是mod料理
    if build == nil then
        build = IsModCookingProduct("cookpot", realname) and realname or "cook_pot_food"
    end
...
local function OnSnowCovered(inst, issnowcovered)
  if issnowcovered then
    inst.AnimState:SetBuild("jx_cookpot_snow_build")
  else
    inst.AnimState:SetBuild("jx_cookpot")
  end
end
...
local function fn()
    ...
    inst.AnimState:SetBank("jx_cookpot")
    inst.AnimState:SetBuild("jx_cookpot")
    inst.AnimState:PlayAnimation("idle_empty")

    inst.MiniMapEntity:SetIcon("jx_cookpot.tex")
    ...
    inst.components.stewer.cooktimemult = 0.85

    inst:AddComponent("container")
    inst.components.container:WidgetSetup("jx_cookpot")
...
return Prefab("jx_cookpot", fn, assets),
    MakePlacer("jx_cookpot_placer", "jx_cookpot", "jx_cookpot", "idle_empty")
