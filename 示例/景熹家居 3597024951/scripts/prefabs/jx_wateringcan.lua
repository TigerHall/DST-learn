...
local function onpercentusedchanged(inst, data)
    if data.percent <= 0 and inst.fuel then
        MakeFuel(inst)
    else
        RemoveFuel(inst)
    end
...
local function displaynamefn(inst)
    return inst:HasTag("usesdepleted") and STRINGS.NAMES[string.upper(inst.prefab).."_EMPTY"] or nil
end
...
local function MakeWateringCan(name, uses, water_amount, fuel, burnable)
    local assets =
    {
...
--                      name,             uses,                        water_amount,                   fuel,  burnable
return MakeWateringCan("jx_wateringcan", TUNING.WATERINGCAN_USES * 5, TUNING.WATERINGCAN_WATER_AMOUNT, false, false)