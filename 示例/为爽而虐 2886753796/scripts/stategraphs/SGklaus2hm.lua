...
local function PickCommandDeer(inst, highprio, lowprio)
    ...
    -- for i, v in ipairs(inst.components.commander:GetAllSoldiers()) do
        -- if DeerCanCast(v) and v:FindCastTargets() ~= nil then
            -- if v.gem == highprio then
                -- return v
            -- elseif v.gem == lowprio then
                -- lowpriodeer = v
...
local function ChooseAttack(inst)
    ...
    if phase ~= 8 and inst.components.commander:GetNumSoldiers() > 0 and
...
local events =
{
    ...
    EventHandler("doforcecast", function(inst)
        if not (inst.sg:HasStateTag("busy") or inst.components.health:IsDead()) then
            ForceCast(inst)
        end
    end),
...
local states =
{
    ...
    State{
        ...
        onexit = function(inst)
			...
			elseif inst.trickAttack == true then
				inst.trickAttack = false
            elseif math.random() <= 0.5 then
				inst.trickAttack = true
				inst:DoTaskInTime(0.1, function() inst:TrickAttack() end)
            end
        end,
    ...
    State{
        ...
        events =
        {
            EventHandler("animover", function(inst)
                if inst.AnimState:AnimDone() then
					if inst.sg.statemem.deer[1] ~= nil then
                    local deer = PickCommandDeer(inst, inst.sg.statemem.deer[1].gem)
                    if deer ~= nil then
                        inst.sg.mem.last_command_deer = deer[1].gem
						
						local phase = inst:GetCurrentPhase()
						
						if phase == 4 or phase == 6 then
							local alldeer = inst.components.commander:GetAllSoldiers()
							for i, v in ipairs(alldeer) do
								v.SpellMode = "Direct"
								v:PushEvent("deercast")
							end
						else
							for i, v in ipairs(deer) do
								if i > 1 then
									v.SpellMode = "Predict"
								else
									v.SpellMode = "Normal"
								end
								inst:DoTaskInTime((i - 1) * 0.5, function() v:PushEvent("deercast") end)
							end
						end
					...
					local deer = PickCommandDeer(inst, nil, nil)
                    inst.sg:GoToState("command_loop", deer)
					end
                end
            end),
    ...
    State{
        ...
        ontimeout = function(inst)
			...
			if deer ~= nil then
				if deer[1] ~= nil then
				if deer[1]:IsValid() and inst.components.commander:IsSoldier(deer[1]) and deer[1].sg ~= nil then
					if deer[1].sg:HasStateTag("casting") then
						inst.sg.statemem.commanding = true
						inst.sg:GoToState("command_loop", deer)
						return
					elseif deer[1].sg.mem.wantstocast then
						if deer[1].sg:HasStateTag("attack") or deer[1].sg:HasStateTag("hit") then
							inst.sg.statemem.commanding = true
							inst.sg:GoToState("command_loop", deer)
							return
						end
						deer[1].sg.mem.wantstocast = nil
					end
				end
				end
			end
