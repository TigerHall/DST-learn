...
function Stackable:SetMaxSize(maxsize)
    local code = STACK_SIZE_CODES[maxsize]
    if code == nil then
        -- 如果传入的 maxsize 不在 STACK_SIZE_CODES 表中，使用默认值（这里选择第一个元素的索引）
        code = STACK_SIZE_CODES[STACK_SIZES[1]]
        print("Warning: maxsize", maxsize, "not found in STACK_SIZE_CODES. Using default value.")
    end
    self._maxsize:set(code - 1)
end
