...
--]] local function FindPlayersInRangeSq2hm(x, y, z, rangesq, isalive)
    local players = {}
    for i, v in ipairs(AllPlayers) do
        if (isalive == nil or isalive ~= IsEntityDeadOrGhost(v)) and v:GetDistanceSqToPoint(x, y, z) < rangesq then table.insert(players, v) end
    end
    return players
end

local function FindPlayersInRange2hm(x, y, z, range, isalive) return FindPlayersInRangeSq2hm(x, y, z, range * range, isalive) end

local function FindClosestPlayerInRangeSq2hm(x, y, z, rangesq, isalive)
    local closestPlayer = nil
    for i, v in ipairs(AllPlayers) do
        if (isalive == nil or isalive ~= IsEntityDeadOrGhost(v)) then
...
local function FindClosestPlayerInRange2hm(x, y, z, range, isalive) return FindClosestPlayerInRangeSq2hm(x, y, z, range * range, isalive) end

function IsAnyPlayerInRangeSq2hm(x, y, z, rangesq, isalive)
    for i, v in ipairs(AllPlayers) do
        if (isalive == nil or isalive ~= IsEntityDeadOrGhost(v)) and v:GetDistanceSqToPoint(x, y, z) < rangesq then return true end
    end
    return false
end

function IsAnyPlayerInRange2hm(x, y, z, range, isalive) return IsAnyPlayerInRangeSq2hm(x, y, z, range * range, isalive) end

local function AllPlayers(inst, self)
    if not inst:IsValid() then return end
    local x, y, z = inst.Transform:GetWorldPosition()
    local players = FindPlayersInRange2hm(x, y, z, self.near, self.alivemode)
    ...
    for i, player in ipairs(players) do
        ...
        if self.closeplayers[player] then
            ...
            inst:PushEvent("onnear2hm", player)
    ...
    for player in pairs(self.closeplayers) do
        if player:IsValid() then
            if (self.alivemode == nil or self.alivemode ~= IsEntityDeadOrGhost(player)) and player:GetDistanceSqToPoint(x, y, z) < farsq then
                closeplayers[player] = true
            else
                inst:PushEvent("onfar2hm", player)
                if self.onfar ~= nil then self.onfar(inst, player) end
            end
        end
    end
...
local function AnyPlayer(inst, self)
    ...
    if not self.isclose then
        local player = FindClosestPlayerInRange2hm(x, y, z, self.near, self.alivemode)
        if player ~= nil then
            self.isclose = true
            inst:PushEvent("onnear2hm")
            if self.onnear ~= nil then self.onnear(inst, player) end
        end
    elseif not IsAnyPlayerInRange2hm(x, y, z, self.far, self.alivemode) then
        self.isclose = false
        inst:PushEvent("onfar2hm")
...
local function SpecificPlayer(inst, self)
    ...
    if not self.isclose then
        if self.target:IsNear(inst, self.near) then
            self.isclose = true
            inst:PushEvent("onnear2hm")
            if self.onnear ~= nil then self.onnear(inst, self.target) end
        end
...
local function OnTargetLeft(self)
    self:Stop()
    self.target = nil
    if self.initialtargetmode == LockAndKeepPlayer or self.initialtargetmode == LockOnPlayer then self:SetTargetMode(self.initialtargetmode) end
    if self.losttargetfn ~= nil then self.losttargetfn() end
end
...
PlayerProx.AliveModes = {AliveOnly = true, DeadOnly = false, DeadOrAlive = nil}
...
function PlayerProx:GetDebugString() return self.isclose and "NEAR" or "FAR" end
...
function PlayerProx:SetLostTargetFn(func) self.losttargetfn = func end

function PlayerProx:SetPlayerAliveMode(alivemode) self.alivemode = alivemode end
...
function PlayerProx:ForceUpdate() if self.task ~= nil and self.targetmode ~= nil then self.targetmode(self.inst, self) end end
...
function PlayerProx:SetTargetMode(mode, target, override)
    if not override then self.originaltargetmode = mode end
...
function PlayerProx:SetTarget(target)
    ...
    if self.target ~= nil then self.inst:RemoveEventCallback("onremove", self._ontargetleft, self.target) end
    self.target = target
    if target ~= nil then self.inst:ListenForEvent("onremove", self._ontargetleft, target) end
end
