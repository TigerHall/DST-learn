...
function IsTrinket(name,is_hallowednights)
	return (trinkets_loot[name] or 0) > (is_hallowednights and 1 or 0)
end
...
function MedalSpawnCircleItem(player,spawnLoot,talkstr)
	...
	for _,v in ipairs(spawnLoot) do
		...
		if v.item or v.randomlist then
			local num = FunctionOrValue(v.num,player) or 1--生成数量
...
local num_to_veggies = table.invert(veggies_to_num)
-- local scions_to_num = {}--接穗转数字
-- for k, v in pairs(veggies_to_num) do
-- 	if k ~= "immortal_fruit" then
-- 		scions_to_num["medal_fruit_tree_"..k.."_scion"] = v
-- 	end
-- end
function GetPayTributeData(key)
	-- return veggies_to_num[key] or scions_to_num[key] or num_to_veggies[key]
...
----------------------------------------------有本源勋章-----------------------------------------------
function HasOriginMedal(inst,tag)
	if inst ~= nil and (tag == nil or inst:HasTag(tag) or inst[tag])then
...
function MedalAddUse(material,target,adduse)
	adduse = adduse or 0
	...
	local fuelnum = material.components.stackable ~= nil and material.components.stackable:StackSize() or 1--材料数量
	local consume = fuelnum > neednum and neednum or fuelnum--实际消耗量
	--修补前执行
	...
	--消耗材料
	if material.components.stackable then
		material.components.stackable:Get(consume):Remove()
	else
		material:Remove()
	end
...
GLOBAL.MedalAddUse=MedalAddUse--补充耐久(材料,目标,单个材料可增加的耐久)