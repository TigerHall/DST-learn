...
local NEWMEDAL_LOOT={
	...
	large_multivariate_certificate={
		target="origin_certificate",
		tag="under_origin_tree",
	},
}
...
local actions = {
			...
			-- elseif act.invobject.prefab ~= cur_item.prefab then
				...
				if old_item ~= nil then 
					...
					if item.prevcontainer ~= nil then
						item.prevcontainer:GiveItem(old_item, item.prevslot)
					else
		...
		fn = function(act)
			if act.doer ~= nil and act.doer:HasTag("addjustice") and act.invobject ~= nil and act.invobject.prefab=="medal_monster_essence" then
				...
				if medal and medal.medal_repair_common and medal.medal_repair_common.medal_monster_essence then
					return MedalAddUse(act.invobject,medal,medal.medal_repair_common.medal_monster_essence)
				end
			end
		end,
		state = "give",
	...
	{
		id = "MEDALAUTODISTRIBUTION", --高效耕地机一键分配(主要用来兼容手柄操作)
		str = STRINGS.MEDAL_UI.DISTRIBUTION,
		fn = function(act)
			if act.doer ~= nil and act.invobject ~= nil and act.invobject.prefab=="medal_farm_plow_item" then
				if act.invobject.components.container ~= nil and not act.invobject.components.container:IsEmpty() and act.invobject.autoDistribution then
					act.invobject:autoDistribution()
					return true
				end
			end
		end,
	...
	{
		id = "MEDAL_EAT_SPICES", --直接吃调料粉
		str = STRINGS.MEDAL_NEWACTION.MEDAL_EAT_SPICES,
		fn = function(act)
			if act.doer ~= nil and HasOriginMedal(act.doer,"seasoningchef") and act.invobject ~= nil and act.invobject:HasTag("ediblespice") then
				if act.invobject.MedalSpiceEatFn then
					act.doer.medal_eat_spice_flag = true--临时加个吸粉标记,方便对Buff进行处理
					act.invobject:MedalSpiceEatFn(act.doer)
					act.doer.medal_eat_spice_flag = nil--处理完就马上移除掉标记
					removeItem(act.invobject)
					return true
				end
				return false,"TOOCROWDED"
	...
	----------------------------USEITEM拿起某物品放到另一个物品上点击后执行----------------------------
	{
		...
		fn = function(act)
			if act.doer ~= nil and act.invobject ~= nil and act.invobject.prefab == "immortal_gem" and act.target:HasTag("immortalable") then
				if act.target.components.itemmimic then return false end--不能给拟态的目标赋予不朽之力
				--吞噬法杖升级成不朽法杖
				...
				if newitem then
					...
					if olditem ~= nil then
						if olditem.prevcontainer ~= nil then
							olditem.prevcontainer:GiveItem(newitem, olditem.prevslot)
		...
		fn = function(act)
			if act.doer ~= nil and act.doer:HasTag("seasoningchef") and act.invobject ~= nil and act.invobject:HasTag("spice") and act.target:HasTag("preparedfood") and not act.target:HasTag("spicedfood") then
				...
				if newfood then
					...
					if not (container and (container.IsOpen == nil or container:IsOpen()) and container:GiveItem(newfood,act.target.prevslot)) then
			...
			if act.doer ~= nil and act.doer:HasTag("player") and ((act.invobject:HasTag("copyfunctional") and act.target:HasTag("blank_certificate")) or (act.invobject:HasTag("blank_certificate") and act.target:HasTag("copyfunctional"))) then
				...
				if blankmedal.prefab == "blank_certificate" then
					...
					if blankmedal.prevcontainer ~= nil then
						blankmedal.prevcontainer:GiveItem(newmedal, blankmedal.prevslot)
				...
				elseif newmedal and newmedal.PowerPrint  then
					...
					if oldmedal ~= nil then
						if newmedal.prevcontainer ~= nil then
							newmedal.prevcontainer:GiveItem(newmedal, newmedal.prevslot)
						elseif isequipped and newmedal.components.equippable then
							act.doer.components.inventory:Equip(newmedal)
						else
		...
		fn = function(act)
			if act.doer ~= nil and act.invobject ~= nil and act.target~=nil then
				...
				if adduse ~= nil then
					...
					return MedalAddUse(act.invobject, act.target, adduse)
				end
			end
		end,
		state = "give",
	...
	},
	{
		...
		fn = function(act)
			if act.doer ~= nil and act.doer:HasTag("player") and act.target.medal_addexp_loot and act.invobject ~= nil and act.target.medal_addexp_loot[act.invobject.prefab]~=nil then
				return MedalAddUse(act.invobject,act.target,act.target.medal_addexp_loot[act.invobject.prefab])
			end
		end,
		state = "give",
		actiondata = {
	...
	},
	{
		...
		fn = function(act)
			if act.doer ~= nil and act.doer:HasTag("groggy") and TheWorld.state.isfullmoon and act.doer.replica.sanity:IsLunacyMode() and act.invobject ~= nil and act.invobject.prefab=="messagebottleempty" and act.target:HasTag("cansalvage") then
				local mark = act.doer:GetMedalDebuffLayer("buff_medal_confused")--迷糊标记
				if mark and mark >= TUNING_MEDAL.MEDAL_BUFF_CONFUSED_MAX then
					return false,"TOOCONFUSED"
				end
				local newitem=SpawnPrefab("bottled_moonlight")
				if newitem~=nil then
					act.doer.components.inventory:GiveItem(newitem)
					act.doer:AddMedalDebuff("buff_medal_confused")--添加迷糊标记
					removeItem(act.invobject)
	...
	{
		id = "MAKEMANDRAKEPLANT",--曼德拉草种植
		str = STRINGS.MEDAL_NEWACTION.MAKEMANDRAKEPLANT,
		fn = function(act)
			if act.doer ~= nil and act.doer:HasTag("has_transplant_medal") and (TheWorld.state.isfullmoon or act.doer:HasTag("inmoonlight") or HasOriginMedal(act.doer)) 
	...
	},
	{
		...
		str = STRINGS.MEDAL_NEWACTION.MAKEMANDRAKEPLANT,--共用文字
		fn = function(act)
					...
					if act.target.components.equippable and not act.target.components.equippable:IsEquipped() then
						...
						if newmedal then
							if act.target.prevcontainer ~= nil then
								act.target.prevcontainer:GiveItem(newmedal, act.target.prevslot)
							else
	...
	},
	{
		...
		fn = function(act)
			if act.doer ~= nil and act.invobject ~= nil and act.invobject:HasTag("medal_tradeable") and act.target:HasTag("medal_trade") then
				...
				local infosave = TheWorld.components.medal_infosave
				--py交易cd中
				local cd = infosave and infosave.InPyTradeCd and infosave:InPyTradeCd(act.target,act.doer)
				if cd then
					MedalSay(act.doer,STRINGS.EXCHANGEGIFT_SPEECH.INCD1 .. cd .. STRINGS.EXCHANGEGIFT_SPEECH.INCD2)
					return true
				end
				--新月才能Py
				...
				--py成功
				if infosave and infosave.DoPyTrade and infosave:DoPyTrade(act.target,act.doer) then
	...
	},
	{
		...
		fn = function(act)
			if act.doer ~= nil and act.invobject ~= nil and act.invobject.prefab=="flint" and act.target.prefab=="medal_farm_plow_item" then
				return MedalAddUse(act.invobject,act.target,TUNING_MEDAL.MEDAL_FARM_PLOW_ADDUSE)
			end
		end,
		state = "give",
		actiondata = {
	...
	},
	{
		...
		fn = function(act)
			if act.doer ~= nil and act.invobject ~= nil and act.invobject.prefab=="toil_money" and act.target.prefab=="medal_skin_staff" then
				return MedalAddUse(act.invobject,act.target,TUNING_MEDAL.MEDAL_SKIN_STAFF_ADDUSE)
			end
		end,
		state = "give",
		actiondata = {
			...
			if act.doer ~= nil and act.invobject ~= nil and act.invobject.prefab=="medal_skin_coupon" and act.target.prefab=="medal_skin_staff" then
				if act.target.buy_skin and act.invobject.skin_name and act.invobject.skin_id then
					...
					if act.target:buy_skin(act.invobject.skin_name,act.invobject.skin_id,0) then
						...
						return MedalAddUse(act.invobject,act.target,TUNING_MEDAL.MEDAL_SKIN_STAFF_ADDUSE_COUPON)
					end
				end
	...
	},
	{
		...
		str = STRINGS.MEDAL_NEWACTION.MEDALFEEDBIRD,
		fn = function(act)
	...
	{
		id = "MEDAL_ORIGIN_TREE_BECOME_MONSTER",--本源之树魔化
		str = STRINGS.MEDAL_NEWACTION.MEDAL_ORIGIN_TREE_BECOME_MONSTER,
		fn = function(act)
			if act.doer ~= nil and act.invobject ~= nil and act.invobject.prefab=="shadowheart_infused" and act.target.prefab=="medal_origin_tree" and not act.target:HasTag("monster_origin_tree") then
				--已经有一棵本源之树了
				if TheWorld and TheWorld.medal_origin_tree ~= nil then
					return false,"ALREADY"
				end
				if act.target.CanBecomeMonster and act.target:CanBecomeMonster() and act.target.BecomeMonster then
					act.target:BecomeMonster()
					removeItem(act.invobject)
					return true
				end
				return false,"TOOCROWDED"
	...
	{
		id = "MEDAL_QUICK_DRY",--新版晾肉架一键晾干
		str = STRINGS.MEDAL_NEWACTION.MEDAL_QUICK_DRY,
		fn = function(act)
			if act.doer ~= nil and act.doer:HasTag("medal_fastpicker") 
				and act.invobject ~= nil and act.invobject:HasTag("dryable")
				and act.target.components.dryingrack and act.target.components.container and not act.target.components.container:IsFull() then
					local count = act.target.components.container:GetEmptySlot(true) or 0
					count = math.min(count, act.invobject.components.stackable and act.invobject.components.stackable:StackSize() or 0)
					for i=1,count do
						local ingredient = act.doer.components.inventory:RemoveItem(act.invobject)
						act.target.components.container:GiveItem(ingredient, nil, act.target:GetPosition())
					end
					return true
			end
		end,
	...
	{
		id = "TRIBUTE_BOX_LEVEL_UP",--奉纳盒升级
		str = STRINGS.MEDAL_NEWACTION.TRIBUTE_BOX_LEVEL_UP,
		fn = function(act)
			if act.doer ~= nil and act.invobject ~= nil and act.invobject.prefab=="medal_tribute_symbol" and act.target.prefab=="medal_pay_tribute_box" then
				if act.target.gift_level and act.target.gift_level < 20 then
					local neednum = math.min(20 - act.target.gift_level, GetStackSize(act.invobject))
					act.target.gift_level = act.target.gift_level + neednum
					removeItem(act.invobject, neednum)
					SpawnPrefab("round_puff_fx_lg").Transform:SetPosition(act.target.Transform:GetWorldPosition())
					return true
				end
				return false,"ISMAX"
			end
	...
	},
	{
		...
		str = STRINGS.MEDAL_NEWACTION.HARVEST,
		fn = function(act)
			if act.doer ~= nil and HasOriginMedal(act.doer,"has_handy_medal") and act.target ~= nil and act.target:HasTags({"oversized_veggie","waxable"}) then
				if act.target.components.workable and act.target.components.workable.onfinish then
					act.target.components.workable.onfinish(act.target,act.doer)
					return true
				end
			end
	...
	},
	{
		...
		fn = function(act)
			if act.doer ~= nil and HasOriginMedal(act.doer,"has_transplant_medal") and act.target ~= nil and act.target.prefab=="moon_tree" and act.target:HasTag("medal_harvestable") and not act.target:HasOneOfTags("burnt","stump") then
	...
	{
		id = "MEDALEXCHANGEGIFT", --童心箱换取礼物
		str = STRINGS.MEDAL_UI.EXCHANGEGIFT,
		fn = function(act)
			if act.doer ~= nil and act.target ~= nil and act.target.prefab=="medal_childishness_chest" then
				if act.target.components.container ~= nil and act.target.components.container:IsFull() and act.target.exchangeGift then
					act.target:exchangeGift(act.doer)
					return true
				end
			end
		end,
	...
	{
		id = "MEDAL_FASTPICK_MEATRACK",--新版晾肉架快采
		str = STRINGS.MEDAL_NEWACTION.HARVEST,
		fn = function(act)
			if act.doer ~= nil and act.target ~= nil and act.doer:HasTag("medal_fastpicker") and act.doer.components.inventory and act.target.components.dryingrack then
				if act.target.components.container and not act.target.components.container:IsEmpty() then
					local items = act.target.components.container:RemoveAllItems()
					for slot, item in ipairs(items) do
						local result = act.doer.components.inventory:GiveItem(item)
					end
					return true
				end
			end
	...
	{
		id = "MEDAL_CHANGE_CHAOS_LEVEL", --切换混沌拳击袋实体抵抗等级
		str = STRINGS.MEDAL_NEWACTION.MEDAL_CHANGE_CHAOS_LEVEL,
		fn = function(act)
			if act.doer ~= nil and act.target ~= nil and act.target.prefab=="punchingbag_chaos" then
				if act.target.ChangeChaosTimes then
					act.target:ChangeChaosTimes()
					return true
				end
			end
		end,
...
local component_actions = {
	{
		...
		tests = {
			...
			{
				action = "MEDALAUTODISTRIBUTION",--高效耕地机一键分配(主要用来兼容手柄操作)
				testfn = function(inst,doer,actions,right)
					return doer:HasTag("player") and inst ~= nil and inst.prefab=="medal_farm_plow_item" and inst:HasTag("CanAutoDistribution") and
						(	inst.replica.container ~= nil and
							inst.replica.container:IsOpenedBy(doer) and
							not inst.replica.container:IsEmpty()
						)
				end,
			},
			-- {
			-- 	action = "RUMMAGE",--手柄可以打开勋章靠边容器
			-- 	testfn = function(inst,doer,actions,right)
			-- 		if inst.prefab == "medal_krampus_chest_item" then
			-- 			local container = inst.replica.container
			-- 			if container:CanBeOpened() then
			-- 				local inventory = doer.replica.inventory
			-- 				if inventory and inventory:GetActiveItem() ~= inst then
			-- 					return true
			-- 				end
			-- 			end
			-- 		end
			...
			-- },
			{
				action = "MEDAL_EAT_SPICES",--直接吃调料粉
				testfn = function(inst,doer,actions,right)
					return HasOriginMedal(doer,"seasoningchef") and inst:HasTag("ediblespice")
				end,
			},
		},
	},
	{
		...
		tests = {
			...
			{
				action = "MAKEMANDRAKEPLANT",--曼德拉草种植
				testfn = function(inst, doer, target, actions, right)
					return doer:HasTag("has_transplant_medal") and (TheWorld.state.isfullmoon or doer:HasTag("inmoonlight") or HasOriginMedal(doer)) 
			...
			{
				action = "MEDAL_ORIGIN_TREE_BECOME_MONSTER",--本源之树魔化
				testfn = function(inst, doer, target, actions, right)
					return inst.prefab=="shadowheart_infused" and target.prefab=="medal_origin_tree" and not target:HasTag("monster_origin_tree")
				end,
			},
			{
				action = "MEDAL_QUICK_DRY",--新版晾肉架一键晾干
				testfn = function(inst, doer, target, actions, right)
					return right and doer:HasTag("medal_fastpicker") and inst:HasTag("dryable")
						and target.prefab=="meatrack" and target.replica.container ~= nil and not target.replica.container:IsFull()
				end,
			},
			{
				action = "TRIBUTE_BOX_LEVEL_UP",--奉纳盒升级
				testfn = function(inst, doer, target, actions, right)
					return right and inst.prefab == "medal_tribute_symbol" and target.prefab == "medal_pay_tribute_box"
				end,
			},
		},
	},
	...
	},
	{
		...
		tests = {
			...
			},
			{
				...
				testfn = function(inst,doer,actions,right)
					return not right and HasOriginMedal(doer,"has_handy_medal") and inst ~= nil and inst:HasTags({"oversized_veggie","waxable"})--inst.prefab=="medal_gift_fruit_oversized"
				end,
			},
			{
				action = "MEDALMOONTREEHARVEST",--采摘月树花
				testfn = function(inst,doer,actions,right)
					return right and HasOriginMedal(doer,"has_transplant_medal") and inst ~= nil and inst.prefab=="moon_tree" and inst:HasTag("medal_harvestable") and not inst:HasOneOfTags("burnt","stump")
				end,
			},
			{
				action = "MEDALEXCHANGEGIFT",--童心箱换取礼物(主要用来兼容手柄操作)
				testfn = function(inst,doer,actions,right)
					return right and doer:HasTag("player") and inst ~= nil and inst.prefab=="medal_childishness_chest" and 
						(	inst.replica.container ~= nil and
			...
			{
				action = "MEDAL_FASTPICK_MEATRACK",--新版晾肉架快采
				testfn = function(inst,doer,actions,right)
					return right and doer:HasTag("medal_fastpicker") and inst ~= nil and inst.prefab == "meatrack" and 
						(	inst.replica.container ~= nil and
							not inst.replica.container:IsEmpty() and
							not inst.replica.container:IsOpenedBy(doer)
						)
				end,
			},
			{
				action = "MEDAL_CHANGE_CHAOS_LEVEL",--切换混沌拳击袋实体抵抗等级
				testfn = function(inst,doer,actions,right)
					return right and doer:HasTag("player") and inst ~= nil and inst.prefab=="punchingbag_chaos" and not inst:HasTag("burnt")
				end,
			},
		},
	},
...
local old_actions = {
	...
	--打开快捷轮盘
	{
		...
		actiondata = {
			strfn = function(act)
				...
				if act.invobject == nil then return old_usespellbook_strfn(act) end
				return (act.invobject.prefab == "medal_shadow_tool" and "SHADOW_TOOL")
					--擦个屁股，不同的道具对应不同的动作名，而不是用人物标签去处理
					or (act.invobject:HasTag("willow_ember") and "PYROKINESIS")
					or (act.invobject.prefab == "winona_remote" and "REMOTE")
					or ((act.invobject.prefab == "wobysmall" or act.invobject:HasTag("woby")) and "WOBY")
					or old_usespellbook_strfn(act)
			end,
		},
	},
	--关闭快捷轮盘
	{
		...
		actiondata = {
			strfn = function(act)
				-- return act.invobject and act.invobject.prefab == "medal_shadow_tool" and "SHADOW_TOOL" or old_usespellbook_strfn(act)
				if act.invobject == nil then return old_usespellbook_strfn(act) end
				return (act.invobject.prefab == "medal_shadow_tool" and "SHADOW_TOOL")
					--擦个屁股，不同的道具对应不同的动作名，而不是用人物标签去处理
					or (act.invobject:HasTag("willow_ember") and "PYROKINESIS")
	...
	--将料理单个放入红晶锅
	{
		...
		actiondata = {
			fn = function(act)
				...
				if target.prefab == "medal_cookpot" and not HasOriginMedal(act.doer,"seasoningchef") then
					if target.components.container ~= nil and act.invobject.components.inventoryitem ~= nil and act.doer.components.inventory ~= nil then
						...
						if not target.components.container:CanTakeItemInSlot(act.invobject) then
							return false, "NOTALLOWED"
						end

						local item = act.invobject.components.inventoryitem:RemoveFromOwner()
						if item ~= nil then
							local targetslot = target.components.container:GetEmptySlot()--寻找空位
