...
ammo_defs.medalslingshotammo_sanityrock={
	...
	onhit = function(inst, attacker, target)
		...
		target:AddDebuff("buff_medal_bleed", "buff_medal_bleed",{max_duration_mult=4,has_origin = HasOriginMedal(attacker,"senior_childishness")})
		inst:Remove()
	end,
...
ammo_defs.mandrakeberry={
	...
	onhit = function(inst, attacker, target)
		...
		local mandrake_chance = TUNING_MEDAL.MANDRAKEBERRY_SEED_CHANCE.MANDRAKE_LESS
		local weed_chance = TUNING_MEDAL.MANDRAKEBERRY_SEED_CHANCE.WEED_LESS + mandrake_chance
		local prefabname = nil
		local rand = math.random()
		--蔓草种子
		if rand<mandrake_chance then
