...
local chesspiece_list={
	land={
		...
		chesspiece_warg_mutated={weight=12,key="warg",fx="collapse_small"},--变异座狼-座狼
...
local unsolved_item_list={
	land={
		atrium_key={weight=15*un_mult,key="yuangu",testfn=function(inst)--远古钥匙--远古祭坛
			return not (TheWorld and TheWorld:HasTag("cave"))--不能在洞穴献祭
		end},
		minotaurhorn={weight=15*un_mult,key="yuangu",testfn=function(inst)--守护者之角--远古祭坛
		...
		oceanfish_small_7_inv={weight=8*un_mult,key="SpringComes",testfn=function(inst)--花朵金枪鱼--春天来了
			return inst and inst:HasTag("spacetime_medal") and TheWorld and TheWorld.state.season ~= "spring" 
		end},
		oceanfish_small_8_inv={weight=8*un_mult,key="SummerComes",testfn=function(inst)--炽热太阳鱼--夏天来了
			return inst and inst:HasTag("spacetime_medal") and TheWorld and TheWorld.state.season ~= "summer" 
		end},
		oceanfish_small_6_inv={weight=8*un_mult,key="AutumnComes",testfn=function(inst)--落叶比目鱼--秋天来了
			return inst and inst:HasTag("spacetime_medal") and TheWorld and TheWorld.state.season ~= "autumn" 
		end},
		oceanfish_medium_8_inv={weight=8*un_mult,key="WinterComes",testfn=function(inst)--冰鲷鱼--冬天来了
			return inst and inst:HasTag("spacetime_medal") and TheWorld and TheWorld.state.season ~= "winter" 
		end},
		messagebottleempty={chance=1, key="newmoon", testfn=function(inst)
			--必须在非启迪夜的月圆时献祭
			return TheWorld and TheWorld.state.moonphase == "full" and not (TheWorld.state.isalterawake or TheWorld:HasTag("cave"))
		end},--空瓶子--新月
	},
	water={
...
local unsolved_index={
	land={
		...
		{key="AutumnComes",weight=0},--秋天来了
		{key="WinterComes",weight=0},--冬天来了
		{key="SpringComes",weight=0},--春天来了
		{key="SummerComes",weight=0},--夏天来了
		{key="newmoon",weight=0},--新月
	},
	water={
...
local unsolved_loot={
	...
	newmoon={--新月
		{playerfn=function(player)
			...
			--获得一瓶瓶装月光
			if player.components.inventory then
				player.components.inventory:GiveItem(SpawnPrefab("bottled_moonlight"))
			end
		end},
	},
	yankuang={--盐矿
...
local book_defs={
	...
	},
	{
		...
		readfn=function(inst,reader)
			...
			--1催长本源之树、2复活周围的曼德拉草、3催长嫁接树、4催长巨大作物
			local state = 0--状态，确保单次只对同一类型的植物生效
			local x, y, z = reader.Transform:GetWorldPosition()
			local ents = TheSim:FindEntities(x, y, z, TUNING_MEDAL.BOOK_NORMAL_RADIUS, nil, 
				{"FX", "NOCLICK", "DECOR", "INLIMBO", "pickable"}, 
				{"cookable","small_origin_tree","medal_fruit_tree","farm_plant"})
			local count = 0
			for i, v in ipairs(ents) do
				--催熟本源之树
				if state==0 and v.prefab == "medal_origin_small_tree" and v.DoMedalMagicGrowth and v:DoMedalMagicGrowth() then
					state = 1
					return true
				--堆叠的不能复活
				elseif (state == 0 or state == 2) and v.prefab == "mandrake" and (v.components.stackable == nil or not v.components.stackable:IsStack()) then
					state = 2
					...
					v:Remove()
					count = count + 1
					if count >= 10 then return true end--单次最多复活10棵
				--催熟嫁接树
				elseif (state == 0 or state == 3) and v:HasTag("medal_fruit_tree") and v.components.pickable and not v:HasTag("pickable") then
					state = 3
					count = count + (v.components.pickable.nomagic and 2 or 1)--原本不能催熟的硬催要按2棵算(不朽果实)
					v.components.pickable.medal_magic = true--植物图鉴强行催不能催的
					v.components.pickable:FinishGrowing()
					if count >= 30 then return true end--单次最多催熟30棵
				--催长农作物为巨大作物
				elseif (state == 0 or state == 4) and v:HasTag("farm_plant") and not v:HasOneOfTags({ "pickable", "weed" }) and v.components.growable and v.components.growable.domagicgrowthfn ~= nil then
					state = 4
					count = count + (v.prefab == "farm_plant_immortal_fruit" and 2 or 1)
					v.force_oversized = true--强制巨大
					v.components.growable:DoMagicGrowth()--催熟
					if count >= 16 then return true end--单次最多巨大化16棵
				end
			end
			--产生过特殊用途了就不能生成奇遇了
			if state == 0 then
