...
-- same_extended--布尔值,为true则表示onextendedfn和onattachedfn一样，省得再写一遍了
...
-----------------------------------------公共方法---------------------------------------------
--添加带层数的buff
local function AddBuffLayerCommon(inst, target, data)
	if target and target:IsValid() and not IsEntityDeadOrGhost(target) then
		inst.buff_layer = math.min((inst.buff_layer or 0) + (data and data.buff_layer or 1), inst.maxlayers or 1)--叠加层数
		--满层回调
		if inst.maxlayerfn and inst.maxlayers and inst.buff_layer >= inst.maxlayers then
			inst:maxlayerfn(target, data)
		end
	end
end
--移除带层数的buff
local function RemoveBuffLayerCommon(inst, target)
	inst.buff_layer = nil
end
...
local function medal_aoecombat_onattack(inst, data)
	local buff = inst:GetDebuff("buff_medal_aoecombat")
	...
	if data.projectile==nil then
		...
		--根据命中距离消耗层数
		if buff and buff.buff_layer then
			buff.buff_layer = buff.buff_layer - consume
		end
	end
	if buff and (buff.buff_layer==nil or buff.buff_layer<=0) then
...
buff_defs.buff_medal_aoecombat={
	...
	priority=2,
	onattachedfn=function(inst, target)
		if inst.buff_layer == nil or inst.buff_layer<=0 then
			target:ListenForEvent("onattackother", medal_aoecombat_onattack)
		end
		--设置AOE伤害(只对原本没AOE的目标生效)
		if target and target:IsValid() and target.components.combat and not IsEntityDeadOrGhost(target) and target.components.combat.areahitrange==nil then
			...
			inst.buff_layer = TUNING_MEDAL.MEDAL_BUFF_AOECOMBAT_VALUE--aoe伤害层数
		end
	end,
	onextendedfn=function(inst, target)
		if inst.buff_layer ~= nil then
			inst.buff_layer = TUNING_MEDAL.MEDAL_BUFF_AOECOMBAT_VALUE--重置aoe伤害层数
		end
	end,
	ondetachedfn=function(inst, target)
		if target and target.components.combat then
			...
			if inst.buff_layer then
				inst.buff_layer = nil
				target.components.combat:SetAreaDamage(nil)--取消aoe伤害
			end
		end
...
local function OnTickInjured(inst,target)
	if target.components.health ~= nil and
		...
		target.components.health:DoDelta((inst.buff_layer or 1)*TUNING_MEDAL.MEDAL_BUFF_INJURED_DAMAGE, nil, "buff_medal_injured",nil,nil,true)--无视防御
		--中毒效果界面
...
buff_defs.buff_medal_injured={
	...
	maxlayers = TUNING_MEDAL.MEDAL_BUFF_INJURED_MAX,--最大层数
	priority=2,
	onattachedfn=function(inst, target, data)
		AddBuffLayerCommon(inst, target, data)
	...
	ondetachedfn=function(inst, target)
		...
		RemoveBuffLayerCommon(inst, target)
	end,
}
...
buff_defs.buff_medal_bloodhoneymark={
	...
	maxlayers = TUNING_MEDAL.MEDAL_BUFF_BLOODHONEYMARK_MAX,--最大层数
	onattachedfn = AddBuffLayerCommon,
	same_extended = true,
	ondetachedfn = RemoveBuffLayerCommon,
}
...
buff_defs.buff_medal_poisonmark={
	...
	maxlayers = TUNING_MEDAL.MEDAL_BUFF_POISONMARK_MAX,--最大层数
	onattachedfn = AddBuffLayerCommon,
	same_extended = true,
	ondetachedfn = RemoveBuffLayerCommon,
}
...
buff_defs.buff_medal_mermcurse={
	...
	maxlayers = TUNING_MEDAL.MEDAL_BUFF_MERMCURSE_MAX,--最大层数
	onattachedfn=function(inst, target, data)
		AddBuffLayerCommon(inst, target, data)
	end,
	same_extended = true,
	ondetachedfn = RemoveBuffLayerCommon,
	maxlayerfn = function(inst, target, data)--满层回调
...
buff_defs.buff_medal_confused={
	...
	maxlayers = TUNING_MEDAL.MEDAL_BUFF_CONFUSED_MAX,--最大层数
	onattachedfn = AddBuffLayerCommon,
	same_extended = true,
	ondetachedfn = RemoveBuffLayerCommon,
}
...
buff_defs.buff_medal_bluemark={
	...
	onattachedfn=function(inst, target, data)--添加蓝晶标记
...
-------------------------------------混乱buff(反向操作)-------------------------------------------------
local function OnTickConfusionFx(inst,target)
	if target ~= nil then
		SpawnPrefab("medal_confusion_fx").entity:SetParent(target.entity)
	end
end
...
buff_defs.buff_medal_confusion={
	...
	onattachedfn=function(inst, target)
		...
		if target.confusion_fx_task == nil then
			target.confusion_fx_task=inst:DoPeriodicTask(1,OnTickConfusionFx, nil, target)
		end
	end,
	ondetachedfn=function(inst, target)
		target:RemoveTag("medal_confusion")
		if target.confusion_fx_task ~= nil then
			target.confusion_fx_task:Cancel()
			target.confusion_fx_task = nil
		end
	end,
}

-------------------------------------------混沌侵蚀----------------------------------------------
buff_defs.buff_medal_chaos_erosion={
	name = "buff_medal_chaos_erosion",
	duration = TUNING_MEDAL.MEDAL_BUFF_CHAOS_EROSION_DURATION,
	maxlayers = TUNING_MEDAL.MEDAL_BUFF_CHAOS_EROSION_MAX,--最大层数
	onattachedfn = AddBuffLayerCommon,
	same_extended = true,
	ondetachedfn = RemoveBuffLayerCommon,
}
...
-------------------------------------------本源能量----------------------------------------------
buff_defs.buff_medal_origin_energy={
	name = "buff_medal_origin_energy",
	duration = TUNING_MEDAL.MEDAL_BUFF_ORIGIN_ENERGY_DURATION,
	maxlayers = TUNING_MEDAL.MEDAL_BUFF_ORIGIN_ENERGY_MAX,--最大层数
	onattachedfn = AddBuffLayerCommon,
	same_extended = true,
	ondetachedfn = RemoveBuffLayerCommon,
}
