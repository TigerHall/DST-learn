...
medal_defs.mediumminer_certificate={
	...
	onequipfn=function(inst,owner)
		...
		owner:PushEvent("changeminermedal")
		if owner.components.workmultiplier then 
			owner.components.workmultiplier:AddMultiplier(ACTIONS.MINE,TUNING_MEDAL.MINER_MEDAL.MEDIUM_MULT,inst)--提升挖矿效率
		end
		--复制的勋章不需要监听
		if inst.finishedwork ~= nil then
			owner:ListenForEvent("finishedwork", inst.finishedwork)
...
medal_defs.largeminer_certificate={
	...
	onequipfn=function(inst,owner)
		...
		owner:PushEvent("changeminermedal")
		if owner.components.workmultiplier then
			local mult = TUNING_MEDAL.MINER_MEDAL[HasOriginMedal(owner) and "ORIGIN_MULT" or "LARGE_MULT"]
			owner.components.workmultiplier:AddMultiplier(ACTIONS.MINE,mult,inst)--提升挖矿效率
		end
...
medal_defs.ommateum_certificate={
	...
	deletefn=function(inst)--耐久用完执行的函数
		if inst.isequipped and inst.ismedallosehealth then--处于装备状态则添加掉血特效
			inst.ismedallosehealth:set(true)
		end
	end,
	...
	onequipfn=function(inst,owner)
		...
		inst.medal_losehealth_task = inst:DoPeriodicTask(1,function()
			if inst.components.fueled then
				...
				--在任何风暴中加速消耗
				if owner and owner.components.stormwatcher ~= nil and owner.components.stormwatcher:GetCurrentStorm() > 0 and owner.components.stormwatcher:GetStormLevel() > TUNING.SANDSTORM_FULL_LEVEL then
					inst.components.fueled.rate_modifiers:SetModifier("storm", 2)
					inst.components.fueled:StartConsuming()
				--在夜晚或洞穴中正常消耗(非月圆)
				elseif TheWorld.state.isnight and not TheWorld.state.isfullmoon then
					inst.components.fueled.rate_modifiers:RemoveModifier("storm")
					local isinlight,oldisinlight = owner:IsInLight()
					if has_origin and isinlight and oldisinlight==nil then--原本就在光中就不需要消耗耐久了
			...
			if inst.components.fueled == nil or inst.components.fueled:IsEmpty() then
				if owner.components.health and not owner.components.health:IsDead() then
					--扣血闪烁
					if owner.player_classified and owner.player_classified.medal_injured then
						owner.player_classified.medal_injured:set_local(false)
						owner.player_classified.medal_injured:set(true)
					end
		...
		--装备时无耐久则添加掉血特效
		if inst.ismedallosehealth and (inst.components.fueled == nil or inst.components.fueled:IsEmpty()) then
			inst.ismedallosehealth:set(true)
		end
	end,
	onunequipfn=function(inst,owner)
		...
		--移除掉血特效
		if inst.ismedallosehealth then
			inst.ismedallosehealth:set(false)
		end
	end,
	extrafn=function(inst)
		--补充耐久前
		inst.medal_before_repairfn=function(inst,item,num)
			if inst.components.fueled ~= nil then
				--增加耐久上限
				local addfueldata = item ~= nil and TUNING_MEDAL.OMMATEUM_MEDAL.ADD_MAXTIME_LOOT[item.prefab]
				if addfueldata ~= nil and inst.components.fueled.maxfuel < addfueldata.maxtime then
					inst.components.fueled.maxfuel = math.min(inst.components.fueled.maxfuel + addfueldata.addtime * (num or 1) , addfueldata.maxtime)
				end
			end
		end
		--补充耐久
	...
	client_extrafn=function(inst)--额外扩展函数
		--物品栏掉血特效
		inst.ismedallosehealth = net_bool(inst.GUID, "ismedallosehealth", "ismedallosehealthdirty")
		inst:ListenForEvent("ismedallosehealthdirty", function(inst)
			inst:PushEvent("medallosehealthchange")
		end)
	end,
	onsavefn=function(inst,data)--扩展存储函数
		if inst.components.fueled ~= nil then
...
medal_defs.treadwater_certificate={
	...
	onequipfn=function(inst,owner)
		...
		--洞穴中仅限本源勋章可踏虚空
		if HasOriginMedal(owner) or not (TheWorld and TheWorld:HasTag("cave")) then
			SetMedalTreadWaterCollides(owner,true)--踏水
		end
...
-------------------------------------------------沉默勋章-------------------------------------------------
--挨打时记录伤害
local function onSilenceAttacked(inst,data)
	if HasOriginMedal(inst) and data and data.original_damage then
		inst.medal_silence_damage = math.min((inst.medal_silence_damage or 0) + data.original_damage*.6, 500)
	end
end
...
medal_defs.silence_certificate={
	...
	onequipfn=function(inst,owner)
		...
		if HasOriginMedal(owner) then--本源
			...
			owner:ListenForEvent("attacked", onSilenceAttacked)--挨打
		end
	end,
	onunequipfn=function(inst,owner)
		...
		owner:PushEvent("refreshcrafting")--更新制作栏
		if HasOriginMedal(owner) then--本源
			owner:RemoveEventCallback("attacked", onSilenceAttacked)--挨打
	...
	extrafn=function(inst)--额外扩展函数
		...
		inst.changeState=function(inst,isaoe)
			...
			if isaoe then
				...
				local buff_layer = owner and owner:GetMedalDebuffLayer("buff_medal_aoecombat")--aoe buff层数
				if buff_layer then
					if buff_layer > 0 and inst.components.finiteuses then
						inst.components.finiteuses:SetUses(math.min(inst.components.finiteuses:GetUses()+math.ceil(buff_layer * .5),inst.components.finiteuses.total))
					end
		...
		inst.medal_CalcChaosDamage = function(owner, target)
			...
			if target and not inst:HasTag("medal_aoe") then--仅限毒伤模式
				target:AddMedalDebuff("buff_medal_poisonmark",HasOriginMedal(owner) and 2 or 1)
				local poison_count = math.min(target:GetMedalDebuffLayer("buff_medal_poisonmark") or 1,TUNING_MEDAL.BEE_KING_MEDAL.POISON_MAX_NOCONSUME)--蜂毒层数
		...
		inst.medal_extra_health_delta = function(target, amount, overtime, cause, ignore_invincible, afflicter, ignore_absorb)
			local poison_count = target and target.GetMedalDebuffLayer and target:GetMedalDebuffLayer("buff_medal_poisonmark") or 0--蜂毒层数
