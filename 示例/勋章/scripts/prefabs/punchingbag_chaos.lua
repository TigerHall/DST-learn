...
--切换混沌实体级别
local function ChangeChaosTimes(inst)
    if inst.components.planarentity then
        inst.components.planarentity.chaos_times = ((inst.components.planarentity.chaos_times or 0) + 1) % 4
        SpawnPrefab("waterplant_destroy").Transform:SetPosition(inst.Transform:GetWorldPosition())
    end
end
--显示当前混沌实体级别
local function getMedalInfo(inst)
    local level = (inst.components.planarentity and inst.components.planarentity.chaos_times or 0) + 1
    return STRINGS.MEDAL_INFO.CHAOS_LEVEL .. level .. STRINGS.MEDAL_INFO.CHAOS_LEVEL_TIPS
end
...
local function fn(build, tags)
    ...
    inst.MiniMapEntity:SetIcon("punchingbag_chaos.tex")

    inst.AnimState:SetBank("punchingbag")
    inst.AnimState:SetBuild("punchingbag_chaos")
    ...
    inst.components.planarentity.chaos_times = 0--混沌抵抗级别
    ...
    inst.OnSave = on_save
    inst.OnLoad = on_load
    inst.ChangeChaosTimes = ChangeChaosTimes
...
----------------------------------------------
return Prefab("punchingbag_chaos", fn, chaos_assets, prefabs),
