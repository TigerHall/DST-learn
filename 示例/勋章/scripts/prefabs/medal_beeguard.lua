...
local function CalcChaosDamage(inst, target)
    ...
    local injured_damage = target and target:GetMedalDebuffLayer("buff_medal_injured")--蜂毒层数
    if injured_damage then
		local blood_mark = target and target.GetMedalDebuffLayer and target:GetMedalDebuffLayer("buff_medal_bloodhoneymark") or 0--血蜜标记层数
...
local function OnHitOther(inst,data)
	if data and data.target then
        local blood_mark = data.target.GetMedalDebuffLayer and data.target:GetMedalDebuffLayer("buff_medal_bloodhoneymark") or 0--血蜜标记层数
        --炸毛状态记录总吸血量
        if inst.puffy_drink_blood then
            ...
            if data.damage then
                ...
                if blood_mark > 0 then
                    data.target:ReduceMedalDebuffLayer("buff_medal_bloodhoneymark")--减少一层血蜜标记
                end
            end
        else--非炸毛状态
            data.target:AddMedalDebuff("buff_medal_bloodhoneymark")--添加1层血蜜标记
            --血蜜标记层数超标了,概率叠毒
            if blood_mark>=TUNING_MEDAL.MEDAL_BUFF_BLOODHONEYMARK_MAX and math.random()<.25 then
                data.target:AddMedalDebuff("buff_medal_injured")--叠1层蜂毒
            end
        end
    end
end
...
local function OnDeath(inst)
	...
	if #ents>0 then
		for i,v in ipairs(ents) do
            local blood_mark = v.GetMedalDebuffLayer and v:GetMedalDebuffLayer("buff_medal_bloodhoneymark") or 0--血蜜标记层数
            if blood_mark < TUNING_MEDAL.MEDAL_BEEGUARD_MARK_CONSUME then
                v:AddMedalDebuff("buff_medal_bloodhoneymark")--添加一层血蜜标记
            elseif v.components.pinnable then
                ...
                v:ReduceMedalDebuffLayer("buff_medal_bloodhoneymark",TUNING_MEDAL.MEDAL_BEEGUARD_MARK_CONSUM)--消耗血蜜标记
            end
			--造成自身吸血量的aoe伤害
