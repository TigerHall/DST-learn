...
local function dropSoul(inst)
    ...
    inst.drop_soul_times = inst.drop_soul_times + 1
    local remaintime = TUNING_MEDAL.RAGE_KRAMPUS_SOUL_LIFETIME - TUNING_MEDAL.RAGE_KRAMPUS_SOUL_DROP_TIME * inst.drop_soul_times
    --在倒数第二下的时候大量爆魂，提醒玩家即将结束
    if remaintime >= 3 and remaintime <= 5 then
        soulnum = 8
    end
...
local function bigfn()
    ...
    inst.drop_soul_times = 0--爆魂计数
	
	inst:AddComponent("sanityaura")
	inst.components.sanityaura.aura = TUNING.SANITYAURA_SMALL--精神光环
	
	inst:DoPeriodicTask(TUNING_MEDAL.RAGE_KRAMPUS_SOUL_DROP_TIME, dropSoul)--每隔3秒掉几个灵魂
