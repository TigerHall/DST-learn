...
local function Retarget(inst)
    ...
    local plants = TheWorld and TheWorld.medal_origin_tree ~= nil and TheWorld.medal_origin_tree.origin_plants or nil--本源之树的植物列表
    local target = FindEntity(
        ...
        function(guy)
            ...
            local total = 0
            --遍历植物列表,如果目标被过多的守卫盯上了就算了
            for i, plant in pairs(plants)do
                if plant ~= inst and plant:HasTag("origin_guard") then
                    if plant.components.combat and plant.components.combat.target and plant.components.combat.target == guy then
                        total = total + 1
                    end
                end
            end
...
local function fn()
	...
	inst.origin_chop_absorb = TUNING_MEDAL.MEDAL_ORIGIN_TREE_GUARD_ABSORB--本源之树减伤
