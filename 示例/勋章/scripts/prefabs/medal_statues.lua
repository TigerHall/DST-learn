...
local function MakeStatue(def)
	local function fn()
		...
		inst:AddTag("rotatableobject")--可用栅栏击剑旋转
		--特定标签
		if def.taglist then
		...
		inst.fencerotator_delta = 180--栅栏击剑单次旋转角度
		...
		inst:AddComponent("savedrotation")--存储旋转角度
...
for i, v in ipairs(require("medal_defs/medal_statue_defs")) do
	...
	table.insert(statues,MakePlacer(v.name.."_placer", v.bank, v.symbol or v.build, v.anim, nil, nil, nil, nil, 180, "two"))
end
