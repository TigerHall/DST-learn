...
local function pickitem(inst,item,player,pos)
	...
	-----------------------丰收勋章采集、收获资源-------------------------
	if player.quickpickmedal then-- and inst.space_target == nil then
		--采集
...
local function pickup_func(inst,target,pos,doer,single)
	...
	elseif target then
		...
		if canpick and caster ~= nil and caster.components.inventory~=nil then
			local origin_energy = caster:GetMedalDebuffLayer("buff_medal_origin_energy") or 0--本源能量
			--单目标拾取
			if single then
				hungerLoss=hungerLoss+pickitem(inst,target,caster)
			--大范围采摘本源之花
			elseif target.prefab == "medal_origin_flower" and target.origin_plant_idx ~= nil
				and caster:HasTag("under_origin_tree") and caster.quickpickmedal and origin_energy >= 5--在本源树下且佩戴了丰收勋章,且有充足的本源能量
				and TheWorld and TheWorld.medal_origin_tree ~= nil and TheWorld.medal_origin_tree.origin_plants ~= nil then--且目前有魔化的本源之树
					-- SpawnMedalFX(scalefx,TheWorld.medal_origin_tree,nil,TUNING_MEDAL.MEDAL_ORIGIN_TREE_SHADE_CANOPY_RANGE)--生成范围圈
					caster:ReduceMedalDebuffLayer("buff_medal_origin_energy",5)--消耗本源能量
					--遍历本源植物列表,采摘所有本源之花
					for i,v in pairs(TheWorld.medal_origin_tree.origin_plants) do
						if v.prefab == "medal_origin_flower" then
							hungerLoss=hungerLoss+pickitem(inst,v,caster)
						end
					end
			else--拾取同种物品
