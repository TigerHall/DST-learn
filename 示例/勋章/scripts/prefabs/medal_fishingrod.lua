...
--时令鱼(春、夏、秋、冬)
local SeasonFishes = {"oceanfish_small_7_inv", "oceanfish_small_8_inv", "oceanfish_small_6_inv", "oceanfish_medium_8_inv"}
local seasonidx = {spring=1,summer=2,autumn=3,winter=4}
		...
		if lure.components.oceanfishingtackle and lure.components.oceanfishingtackle.lure_setup ~= nil and lure.components.oceanfishingtackle.lure_setup.single_use then
			if pond.prefab=="medal_seapond" or pond.prefab=="medal_spacetime_pond" then
				...
				if math.random() < chance then
					--虚空钓鱼池随机时令鱼
					if pond.prefab=="medal_spacetime_pond" then return GetRandomItem(SeasonFishes) end
					--否则只能钓出应季时令鱼
					return SeasonFishes[seasonidx[TheWorld.state.season] or 1]
				end
			end
		else
