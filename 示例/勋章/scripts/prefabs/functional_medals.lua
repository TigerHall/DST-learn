...
local function onequipfn(inst,owner)
	--只有玩家装备了会生效
	if owner and owner:HasTag("player") and inst.onequipfn then
		inst:onequipfn(owner)
	end
end
--卸下
local function onunequipfn(inst,owner)
	if owner and owner:HasTag("player") and inst.onunequipfn then
		inst:onunequipfn(owner)
	end
end
...
local function MakeCertificate(def)
	...
	local function fn()
		...
		inst.onequipfn = def.onequipfn
		inst.onunequipfn = def.onunequipfn
