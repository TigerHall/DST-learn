...
--掉落礼物(inst,是否为万圣节掉落,玩具种类数量,空白勋章数量,本源加成)
local function DropGifts(inst,ishalloween,trinket_kind_num,blank_medal_num,has_origin)
	local bundleitems = {}
	...
	if ishalloween then
		...
		if devoursoulammo and devoursoulammo.components.stackable then
			...
			devoursoulammo.components.stackable:SetStackSize(math.floor(ammo_num * ammo_mult))
		end
		...
		--特制鱼食
		if trinket_kind_num and trinket_kind_num >= 8 then
			local chum=SpawnPrefab("medal_chum")
			if chum and chum.components.stackable then
				chum.components.stackable:SetStackSize(math.floor(trinket_kind_num * .5 - 3))
			end
...
local function exchangeGift(inst,player)
	...
	local trinket_kind_num = 0--玩具种类数量
	...
	for k,v in ipairs(itemlist) do
		...
		--玩具
		elseif IsTrinket(v.prefab) then
			--统计玩具种类数量
			if not trinket_list[v.prefab] then
				trinket_list[v.prefab] = true
				trinket_kind_num = trinket_kind_num + 1
			end
			--统计万圣节玩具数量
			if IsTrinket(v.prefab,true) then
				halloween_toy_num = halloween_toy_num+1
			end
		else--不能有玩具、空白勋章之外的东西
			MedalSay(player,STRINGS.EXCHANGEGIFT_SPEECH.FAIL2)
			return false
		end
	end
	--种类太少
	if trinket_kind_num<5 then
	...
	DropGifts(inst,halloween_toy_num>1,trinket_kind_num,blank_medal_num,HasOriginMedal(player,"senior_childishness"))--掉落礼物
	local fx = SpawnPrefab("collapse_small")
	fx.Transform:SetPosition(inst.Transform:GetWorldPosition())
	fx:SetMaterial("wood")
	inst:Remove()
