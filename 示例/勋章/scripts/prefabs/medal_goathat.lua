...
local function onlightingstrike(inst)
	if inst.components.health ~= nil and not (inst.components.health:IsDead() or inst.components.health:IsInvincible()) then
		...
		if inst.components.inventory:IsInsulated() then
			...
			inst:AddDebuff("buff_medal_electricattack", "buff_medal_electricattack",{max_duration_mult = 2},nil,function()
...
local function transfromMermFn(inst,owner)
	--鱼人诅咒层数达到上限
	local buff_layer = owner and owner:GetMedalDebuffLayer("buff_medal_mermcurse")--诅咒层数
	if buff_layer and buff_layer >= TUNING_MEDAL.MEDAL_BUFF_MERMCURSE_MAX then
		--骑牛你就歇歇吧
