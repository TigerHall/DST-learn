...
--催生飞虫
local SPAWNINSECT_CANT_TAGS = { "INLIMBO", "FX" }
local SPAWNINSECT_ONEOF_TAGS = { "origin_flower" }
local function DoAreaSpawnInsect(inst)
    local tree = TheWorld and TheWorld.medal_origin_tree
    --需要有魔化本源之树且昆虫数量没达到上限
    if tree == nil or tree.origin_insect_num == nil or tree.origin_insect_num >= TUNING_MEDAL.MEDAL_ORIGIN_TREE_INSECT_NUM_MAX then
        return
    end
    
    local x, y, z = inst.Transform:GetWorldPosition()
    local ents = TheSim:FindEntities(x, y, z, inst.components.aura.radius, nil, SPAWNINSECT_CANT_TAGS, SPAWNINSECT_ONEOF_TAGS)
    for i, v in ipairs(ents) do
        ...
        if insect ~= nil then
            insect.Transform:SetPosition(v.Transform:GetWorldPosition())
            if tree.RegisterOriginInsect then
                tree:RegisterOriginInsect(insect)--登记
            end
        end
    end
end
...
local function fn()
    ...
    inst.components.aura.pretickfn = DoAreaSpawnInsect--催生飞虫
