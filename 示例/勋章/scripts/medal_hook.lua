--[[致其他moder：
以下是我预留的一些勋章中常用功能的兼容方法,如有兼容需求可直接参考
...
※※※书籍兼容智慧勋章※※※
--如果你的mod里有书籍，原本无阅读能力的角色在佩戴智慧勋章时阅读该书籍时，若san值不够则会默认消耗10点耐久；耐久不足则本次阅读不生效
...
※※※buff栏兼容※※※
如果你希望你的Buff在勋章Buff栏里能正常显示，则可按如下方式兼容：
兼容方法很简单,给你的buff添加一个STRINGS.NAMES就可以了
例如风滚草的保运符buff的prefab是buff_keep_lucky,只需要参考下面这条格式添加buff名即可
STRINGS.NAMES.BUFF_KEEP_LUCKY="保运符"--可以是你对这个Buff的命名,也可以是这个buff的获取方式,随你喜欢~
如果你的Buff也有“层数”的概念,那你可以给你的buff预制物添加一个buff_layer参数
inst.buff_layer = 3--表示当前这个buff叠了3层,在层数发生变更的时候同步变更一下这个参数即可
如果你的Buff定时器命名很特殊,或者有什么特别的显示方法,可以查看medal_showbufftime.lua这个文件,已经预留好了对应的接口~


※※※调料可直接食用※※※
玩家佩戴本源主厨勋章时,可直接吃调料粉添加buff(添加的buff时长*0.85)
如需要可按如下方法兼容(你愿意的话直接写到对应的预制物代码里也OK)：

--相关buff信息
TUNING.MEDAL_EDIBLE_SPICE_BUFFS = TUNING.MEDAL_EDIBLE_SPICE_BUFFS or {}
TUNING.MEDAL_EDIBLE_SPICE_BUFFS.buff代码 = buff时长
TUNING.MEDAL_EDIBLE_SPICE_BUFFS.buff_playerabsorption = TUNING.BUFF_PLAYERABSORPTION_DURATION--蒜粉
TUNING.MEDAL_EDIBLE_SPICE_BUFFS.buff_workeffectiveness = TUNING.BUFF_WORKEFFECTIVENESS_DURATION--蜂蜜水晶

--调料及对应buff
local dst_spices = {
	spice_garlic = "buff_playerabsorption",--蒜粉
	spice_sugar = "buff_workeffectiveness",--蜂蜜水晶
	spice_chili = "buff_attack",--辣椒面
}
local function setEdibleSpice(inst)
	inst:AddTag("ediblespice")--可食用调味料
	if TheWorld.ismastersim then
		inst.MedalSpiceEatFn = function(inst, eater)
			local buff = dst_spices[inst.prefab]
			if buff then
				eater:AddMedalDebuff(buff)
			end
		end
	end
end
for k, v in pairs(dst_spices) do
	AddPrefabPostInit(k,setEdibleSpice)
end


※※※调味料理叠Buff时长※※※
玩家佩戴本源主厨勋章时,重复吃调味料理可以叠加Buff时长
如果你希望你模组中的调料支持该功能，可按如下方式添加：
TUNING.MEDAL_SPECIAL_SPICES = TUNING.MEDAL_SPECIAL_SPICES or {}
TUNING.MEDAL_SPECIAL_SPICES.SPICE_JELLY = {--果冻粉(调料代码,需大写)
	buff = "buff_moistureimmunity",--添加的Buff名
	duration = TUNING.BUFF_MOISTUREIMMUNITY_DURATION,--buff默认时长
}
TUNING.MEDAL_SPECIAL_SPICES.SPICE_VOLTJELLY = {--带电果冻粉
	buff = "buff_electricattack",
...
local function CanBlinkTo(inst,pt)
    if inst.CanBlinkTo then
		--这边还是要pcall走一波,很多老Mod这边都只传1个参数pt
		local success,result = pcall(inst.CanBlinkTo,inst,pt)
		if not success then
			success,result = pcall(inst.CanBlinkTo,pt)
		end
		return result
		-- return inst:CanBlinkTo(pt)
	end
	return TheWorld.Map:IsPassableAtPoint(pt:Get())
		and not TheWorld.Map:IsGroundTargetBlocked(pt) 
		and (TheWorld.Map:IsPointInWagPunkArenaAndBarrierIsUp(pt:Get()) == TheWorld.Map:IsPointInWagPunkArenaAndBarrierIsUp(inst.Transform:GetWorldPosition()))
...
local function MedalBlinkPointSpecialActions(inst, pos, useitem, right, ...)
	--拥有临时穿梭者、自由穿梭者、勋章穿梭者标签
	if right and useitem == nil and (inst.replica.rider == nil or not inst.replica.rider:IsRiding()) and CanBlinkTo(inst,pos) then
	...
	--原本就有特殊动作的话可以在灵魂跳跃条件不通过的时候调用,但是也需要注意防止嵌套
	if not inst.medal_blink_flag and inst.medal_old_pointspecialactionsfn and inst.medal_old_pointspecialactionsfn ~= MedalBlinkPointSpecialActions then
		inst.medal_blink_flag = true--设置标记，防止多次嵌套调用
		local result = inst.medal_old_pointspecialactionsfn(inst, pos, useitem, right, ...) or {}
		inst.medal_blink_flag = nil
		return result
	end
	return {}
...
local function setBlinkFn(player)
	if player.components.playeractionpicker ~= nil then
		player.medal_old_pointspecialactionsfn = player.components.playeractionpicker.pointspecialactionsfn
		-- print(player.medal_old_pointspecialactionsfn,MedalBlinkPointSpecialActions)
		--有老函数的就记一下防止被覆盖了,只需要确保不重复定义就可以了
		if player.medal_old_pointspecialactionsfn ~= MedalBlinkPointSpecialActions then
			player.components.playeractionpicker.pointspecialactionsfn = MedalBlinkPointSpecialActions
		end
	end
end
--设置玩家减速触发器(蜘蛛网)
...
AddPlayerPostInit(function(inst)
	...
	if oldIsInLight ~= nil then
		inst.IsInLight = function(inst, ...)
			local oldresult = oldIsInLight(inst, ...)
			if oldresult then return oldresult end
			if inst.medalnightvision and inst.medalnightvision:value() then
				return true, oldresult--多return一个原结果给复眼做特殊判断
			end
			return oldresult
		end
	end
	if TheWorld.ismastersim then
		...
		inst:ListenForEvent("ms_playerreroll", function(inst)
			...
			--消耗所有寄生值
			if inst.components.health and inst.components.health.ConsumeMedalParasitic then
				inst.components.health:ConsumeMedalParasitic()
			end
		end)
		--变身后(换人后)
		...
		inst:ListenForEvent("oneat", function(inst,data)
			if data and data.food and data.feeder then
				...
				elseif data.food.prefab == "shroomcake" or basename == "shroomcake" then
					if data.feeder:HasDebuff("buff_medal_confused") then
		...
		if inst.components.eater then
			...
			inst.components.eater.custom_stats_mod_fn = function(inst, health_delta, hunger_delta, sanity_delta, food, feeder, ...)
				local absorption = 1
				local buff_layer = feeder and feeder.GetMedalDebuffLayer and feeder:GetMedalDebuffLayer("buff_medal_mermcurse")--诅咒层数
				if buff_layer then
					if food and food.components.edible and food.components.edible.foodtype == FOODTYPE.MEAT then
						absorption = 1 - buff_layer * .1
					end
				end
		...
		inst.OnLoad = function(inst,data)
			...
			if data~=nil then
				inst.medal_rewardtoiler_mark = data.medal_rewardtoiler_mark--天道酬勤
			end
		end
		--移除玩家身上的暗影仆从
...
---------------------------------------------------------------------------------------------------------
--原版调料Buff
local dst_spice_buffs = {
	buff_playerabsorption = TUNING.BUFF_PLAYERABSORPTION_DURATION,--蒜粉
	buff_workeffectiveness = TUNING.BUFF_WORKEFFECTIVENESS_DURATION,--蜂蜜水晶
	buff_attack = TUNING.BUFF_ATTACK_DURATION,--辣椒面
	buff_moistureimmunity = TUNING.BUFF_ATTACK_DURATION,--果冻粉
	buff_electricattack = TUNING.BUFF_ATTACK_DURATION,--带电果冻粉
}
--方便兼容其他mod
TUNING.MEDAL_EDIBLE_SPICE_BUFFS = TUNING.MEDAL_EDIBLE_SPICE_BUFFS or {}
for k, v in pairs(dst_spice_buffs) do
	TUNING.MEDAL_EDIBLE_SPICE_BUFFS[k]=v
end
--添加buff
EntityScript.AddMedalDebuff = function(self,name,buff_layer,data,...)
	if self.AddDebuff then
		--如果是直接吃粉加的原版Buff的话,需要特殊处理逻辑
		local duration = TUNING.MEDAL_EDIBLE_SPICE_BUFFS[name]
		if self.medal_eat_spice_flag and duration then
			local buff = self:GetDebuff(name)
			local bufftime = 0
			duration = duration * .85
			--获取原本剩余的时长,防止把原本可能更长的时间覆盖掉
			if buff and buff.components.timer then
				bufftime = buff.components.timer:GetTimeLeft("buffover")
			end
			self:AddDebuff(name, name, data, ...)--不管三七二十一都要执行一遍，因为可能有调onextendedfn的需求
			if buff == nil then
				buff = self:GetDebuff(name)--原本没拿到buff那就再试着拿一次
			end
			--覆盖Buff时间
			if buff and buff.components.timer then
				buff.components.timer:StopTimer("buffover")
				buff.components.timer:StartTimer("buffover", math.max(duration,bufftime))
			end
			return true
		end
		...
		if buff_layer then
			data = data or {}
			data.buff_layer = buff_layer
		end
		if self.medal_eat_spice_flag then
			data = data or {}
			data.duration_mult = 0.85
		elseif self.medal_eat_spicefood_flag then
			data = data or {}
			data.max_duration_mult = 6
		end
		return self:AddDebuff(name, name, data, ...)
	end
end

--获取buff层数
EntityScript.GetMedalDebuffLayer = function(self,name,...)
	local buff = self.GetDebuff and self:GetDebuff(name,...)
	return buff and buff.buff_layer
end

--减少buff层数(self,buff名,移除层数,尝试移除)
EntityScript.ReduceMedalDebuffLayer = function(self,name,buff_layer,try_reduce,...)
	local buff = self.GetDebuff and self:GetDebuff(name,...)
	if buff then
		buff_layer = buff_layer or 1
		--本来就没层级或者小于减少的层数
		if buff.buff_layer == nil or buff.buff_layer < buff_layer then
			if try_reduce then--如果是尝试移除,那么判定为失败
				return false
			else--否则直接移除
				self:RemoveDebuff(name)
			end
		--等于时直接移除Buff
		elseif buff.buff_layer == buff_layer then
			self:RemoveDebuff(name)
		else--大于时移除层数
			buff.buff_layer = buff.buff_layer - buff_layer
		end
		return true
	end
end
...
AddPrefabPostInit("mandrake_active",function(inst)
	if TheWorld.ismastersim then
        ...
        inst.components.periodicspawner:SetRandomTimes(TUNING_MEDAL.MANDRAKE_BERRY_SPAWNER_TIME, TUNING_MEDAL.MANDRAKE_BERRY_SPAWNER_TIME_VARIANCE, false)--掉落周期
...
---------------------------------------------触手不主动攻击拥有触手勋章的玩家-------------------------------------------------
local tentacle_hook_loot = {}--hook列表,把已经执行过的记一下,省得一直重复执行了
local function tentacleRetarget(inst)
	if TheWorld.ismastersim then
		if tentacle_hook_loot[inst.prefab] == nil then
			--在寻敌屏蔽列表中加入需要屏蔽的标签
			local targetfn = inst.components.combat and inst.components.combat.targetfn
			if targetfn then
				local RETARGET_CANT_TAGS = upvaluehelper.Get(targetfn,"RETARGET_CANT_TAGS")
				if RETARGET_CANT_TAGS then
					table.insert(RETARGET_CANT_TAGS,"senior_tentaclemedal")--拥有高级触手勋章的玩家
					table.insert(RETARGET_CANT_TAGS,"chaos_creature")--混沌生物
					tentacle_hook_loot[inst.prefab] = true
				end
			end
		end
	end
	...
	if TheWorld.ismastersim then
		if inst.components.sanityaura then
			...
			inst.components.sanityaura.aurafn = function(inst,observer,...)
				if observer:HasTag("tentaclemedal") then
					return 0
				end
				return old_aurafn and old_aurafn(inst,observer,...) or inst.components.sanityaura.aura
			end
		end
	end
end)
--被召唤出来的大暗影触手不会让佩戴了本源触手勋章的玩家掉san
AddPrefabPostInit("bigshadowtentacle",function(inst)
	if TheWorld.ismastersim then
		if inst.components.sanityaura then
			local old_aurafn = inst.components.sanityaura.aurafn
			inst.components.sanityaura.aurafn = function(inst,observer,...)
				if inst.from_tentaclemedal and HasOriginMedal(observer,"tentaclemedal") then
					return 0
				end
				return old_aurafn and old_aurafn(inst,observer,...) or inst.components.sanityaura.aura
			end
		end
		--召唤的大暗影触手也不能切换目标，防止误伤友军
		local old_targetfn = inst.components.combat and inst.components.combat.targetfn
		inst.components.combat.targetfn = function(inst,...)
			if inst.from_tentaclemedal then return end 
			return old_targetfn and old_targetfn(inst,...)
		end
	end
end)
	...
	if TheWorld.ismastersim then
		if inst.components.trader and inst.components.trader.onaccept then
			...
			inst.components.trader:SetAcceptTest(function(inst, item, giver, ...)
				if giver:HasTag("merm") and HasOriginMedal(giver,"has_merm_medal") then
			...
			inst.components.workable.onfinish=function(inst, worker, ...)
				...
				for i, v in ipairs(players) do
					if not worker:HasTag("merm") then--小鱼人或佩戴鱼人勋章的玩家不会受到诅咒
						v:AddMedalDebuff("buff_medal_mermcurse",inst.awake and 2 or 1)--被破坏了给边上的玩家添加一层鱼人诅咒(月圆额外增加一层)
					end
				end
				if oldonfinish then
...
local function DigestAllFood(inst, food)
	...
	if prefab ~= nil then
		...
		if prefab == "guano" then--鸟粪不是百分百出的，需要按概率计算数量
			local p = 0.33--单次出鸟粪概率
			--使用正太分步来模拟随机n次的结果
			rewardnum = math.max(0, math.min(rewardnum, math.floor(rewardnum * p + math.sqrt(rewardnum * p * (1 - p)) * math.sqrt(-2 * math.log(math.random())) * math.cos(2 * math.pi * math.random()) + 0.5)))
		end
...
AddComponentPostInit("workable", function(self,inst)
	...
	self.WorkedBy_Internal = function(self, worker, numworks, ...)
		...
		--本源高级伐木对砍伐本源之树有加成,可超出单次工作次数上限
		if self.inst.prefab == "medal_origin_tree" and HasOriginMedal(worker,"large_chop") then
			numworks = numworks + 5
		end
...
AddComponentPostInit("container", function(self)
	...
	--获取空格子(self,是否统计空格子数量)
	self.GetEmptySlot=function(self,countnum)
		local count = 0
		for i = 1, self.numslots do
			if self.slots[i] == nil then
				if countnum then
					count = count + 1--统计空格子数量
				else
					return i--不统计空格子数的话返回第一个空格子就可以了
				end
			end
		end
		return countnum and count or nil
	end
	--获取制作材料道具
	...
	function self:MoveItemFromOneOfSlot(slot, container, opener)
		...
		if item ~= nil and container ~= nil then
			...
			if container ~= nil and container:IsOpenedBy(opener) then-- and item.components.stackable ~= nil and item.components.stackable:IsStack() then
				...
				local targetslot = container:GetEmptySlot()--寻找空位
				-- print(targetslot)
	...
	--没戴本源主厨往红晶锅里丢食材的时候只放单个(鼠标放置)
	local oldPutAllOfActiveItemInSlot = self.PutAllOfActiveItemInSlot
	self.PutAllOfActiveItemInSlot = function(self, slot, opener)
		-- print(self.inst.prefab,slot,opener)
		if self.inst ~= nil and self.inst.prefab == "medal_cookpot" and self.PutOneOfActiveItemInSlot and not HasOriginMedal(opener,"seasoningchef") then
			self:PutOneOfActiveItemInSlot(slot, opener)
		elseif oldPutAllOfActiveItemInSlot then
			oldPutAllOfActiveItemInSlot(self, slot, opener)
		end
	end
...
AddClassPostConstruct("components/container_replica", function(self)
    local oldPutAllOfActiveItemInSlot = self.PutAllOfActiveItemInSlot
	self.PutAllOfActiveItemInSlot = function(self, slot)
		-- print(self.inst.prefab,slot,opener)
		if self.inst ~= nil and self.inst.prefab == "medal_cookpot" and self.PutOneOfActiveItemInSlot and not HasOriginMedal(ThePlayer,"seasoningchef") then
			self:PutOneOfActiveItemInSlot(slot)
		elseif oldPutAllOfActiveItemInSlot then
			oldPutAllOfActiveItemInSlot(self, slot)
		end
	end
end)
...
AddComponentPostInit("temperature", function(self)
	...
	-- 	if self.inst:HasTag("under_origin_tree") then
	-- 		return TUNING_MEDAL.DOWN_FILLED_COAT_INSULATION, TUNING_MEDAL.HAT_BLUE_CRYSTAL_INSULATION
	-- 	elseif oldGetInsulation then
	-- 		return oldGetInsulation(self,...)
	-- 	end
	-- end
end)
...
AddComponentPostInit("health", function(self)
	...
	--消耗寄生值
	self.ConsumeMedalParasitic = function(self,amount)
		if self.medal_parasitic_value > 0 then
			local tree = TheWorld and TheWorld.medal_origin_tree--本源之树
			local phase = tree and tree.phase or 1--本源之树当前阶段
			local parasitic = self.medal_parasitic_value--待消耗的寄生值
			if amount then
				parasitic = math.min(parasitic , math.ceil(amount * TUNING_MEDAL.PARASITIC_CONVERSION_RATIO[phase]))--寄生值消耗
				amount = amount - parasitic
			end
			self:DoDeltaMedalParasitic(-parasitic)
			--给本源之树回血
			if tree ~= nil and tree.DoRecovery then
				tree:DoRecovery(parasitic*TUNING_MEDAL.PARASITIC_CONVERSION_MULT[phase])--回血
			end
		end
		return amount
	end
	...
	self.DoDelta = function(self, amount, overtime, cause, ignore_invincible, afflicter, ignore_absorb, ...)
		...
		if amount > 0 and self.medal_parasitic_value > 0 then
			amount = self:ConsumeMedalParasitic(amount)
		end
	...
	self.SetVal = function(self, val, cause, afflicter, ...)
		...
		if val <= min_health and self.currenthealth > min_health then
			--轮回
			if self.inst and HasOriginMedal(self.inst,"spacetime_medal") then
...
AddComponentPostInit("pickable", function(self)
	...
	self.FinishGrowing = function(self, ...)
		--植物图鉴的力量,临时无视季节强制催熟一次
		if self.medal_magic then
			self.medal_magic = nil
			if not (self.canbepicked or self.inst:HasTag("withered")) then
				if self.task ~= nil then
					self.task:Cancel()
					self.task = nil
				end
				if self.stopregentimer then
					self.stopregentimer(self.inst)
				end
				self:Regen()
				return true
			end
		elseif self.nomagic then--禁止催熟
			return false
		end
...
AddComponentPostInit("healer", function(self)
	...
	self.Heal = function(self,target, ...)
		if oldHeal then
			if self.inst.prefab=="bandage" and target:HasDebuff("buff_medal_injured") and oldHeal then
...
--攻击可添加寄生值的预制物
local parasitic_value_list={
	medal_sporecloud = TUNING_MEDAL.MEDAL_SPORECLOUD_PARASITIC,--本源孢子云
	medal_origin_mushgnome_spore = TUNING_MEDAL.MEDAL_ORIGIN_MUSHGNOME_SPORE_PARASITIC,--本源地精孢子
	medal_origin_fruitfly = TUNING_MEDAL.MEDAL_ORIGIN_FRUITFLY_PARASITIC,--本源果蝇
}
--修改攻击组件
AddComponentPostInit("combat", function(self)
	...
	self.GetAttacked = function(self, attacker, damage, weapon, stimuli, spdamage, ...)
		if oldGetAttacked then
			--混沌侵蚀(混沌伤害倍率加成)
			-- if self.inst.medal_chaos_erosion_mark and spdamage and spdamage.medal_chaos then
			if spdamage and spdamage.medal_chaos then
				local buff_layer = self.inst:GetMedalDebuffLayer("buff_medal_chaos_erosion")
				if buff_layer then
					spdamage.medal_chaos = spdamage.medal_chaos * (1 + buff_layer * TUNING_MEDAL.MEDAL_BUFF_CHAOS_EROSION_MULT)
				end
				-- spdamage.medal_chaos = spdamage.medal_chaos * (1 + self.inst.medal_chaos_erosion_mark * TUNING_MEDAL.MEDAL_BUFF_CHAOS_EROSION_MULT)
			end
			--本源沉默的爆发伤害
			...
			--添加寄生值
			if attacker and parasitic_value_list[attacker.prefab] and self.inst.components.health and not IsEntityDeadOrGhost(self.inst) then
				self.inst.components.health:DoDeltaMedalParasitic(parasitic_value_list[attacker.prefab])
			end
	...
	self.DoAreaAttack = function(self, target, range, weapon, ...)
		...
		if (self.inst.medal_aoecombat or self.inst:GetMedalDebuffLayer("buff_medal_aoecombat") ~= nil) and weapon ~= nil and weapon:HasTag("keep_equip_toss") and not weapon:HasTag("INLIMBO") then
			return 0
		end
...
AddComponentPostInit("harvestable", function(self)
	self.HarvestMedalBeeBox=function(self, picker)
		if self:CanBeHarvested() and self.inst.prefab=="medal_beebox" and self.inst:HasTag("medal_beebox_full") then
			...
			if loot ~= nil then
				...
				if picker ~= nil and picker.components.inventory ~= nil then
					--本源蜂王勋章双倍采集
					if HasOriginMedal(picker,"isbeeking") and loot.components.stackable then
						loot.components.stackable:SetStackSize(2)
					end
	...
	self.Harvest = function(self, picker, ...)
		--本源蜂王勋章双倍采集育王蜂箱蜂王浆
		if self.inst.prefab == "medal_beebox" and HasOriginMedal(picker,"isbeeking") and self:CanBeHarvested() then
			if self.can_harvest_fn ~= nil then
				local can_harvest, fail_reason = self.can_harvest_fn(self.inst, picker)
				if not can_harvest then
					return false, fail_reason
...
AddComponentPostInit("growable", function(self)
	...
	self.SetStage=function(self, stage, ...)
		--杂草种子只能种出杂草
		if self.inst and self.inst.prefab=="farm_plant_randomseed" and self.is_weed_seed and self.stages[stage] ~= nil and self.stages[stage].name=="sprout" then
		...
		--月树最大状态时可采摘
		if self.inst and self.inst.prefab=="moon_tree" and self.stages[stage] ~= nil then
			if self.stages[stage].name=="tall" then
				self.inst:AddTag("medal_harvestable")--最大形态可采摘
			else
				self.inst:RemoveTag("medal_harvestable")--非最大形态不可采摘
			end
		end
	end
	--本源之树不能强制催长
...
AddComponentPostInit("drownable", function(self)
	local oldShouldX_InternalCheck = self.ShouldX_InternalCheck
	self.ShouldX_InternalCheck=function(self, ...)
		if self.inst.medal_canTread then
			return false
		end
		return oldShouldX_InternalCheck and oldShouldX_InternalCheck(self, ...)
	end
end)

--栅栏击剑旋转
AddComponentPostInit("fencerotator", function(self)
	local oldRotate = self.Rotate
	self.Rotate=function(self, target, delta, ...)
		--自定义旋转角度，转出风采，转出水平！
		if target ~= nil and target.fencerotator_delta then
			delta = target.fencerotator_delta
		end
		if oldRotate then
			oldRotate(self, target, delta, ...)
		end
	end
end)

--支持本源主厨叠时长的buff料理
local special_spices = {
	SPICE_JELLY = {--果冻粉
		buff = "buff_moistureimmunity",
	...
	SPICE_GARLIC = {--蒜粉
		buff = "buff_playerabsorption",
		duration = TUNING.BUFF_PLAYERABSORPTION_DURATION,
	},
	SPICE_SUGAR = {--蜂蜜水晶
		buff = "buff_workeffectiveness",
...
--方便兼容其他mod
TUNING.MEDAL_SPECIAL_SPICES = TUNING.MEDAL_SPECIAL_SPICES or {}
for k, v in pairs(special_spices) do
	TUNING.MEDAL_SPECIAL_SPICES[k] = v
end
...
AddComponentPostInit("edible", function(self)
	...
	self.OnEaten=function(self, eater, ...)
		...
		local bufftime = 0
		local duration = 0
		local hasorigin = HasOriginMedal(eater,"seasoningchef")

		if self.spice and self.inst:HasTag("spicedfood") then
			--如果是加原版Buff的调料,则获取对应的Buff数据并记录
			local spice_data = TUNING.MEDAL_SPECIAL_SPICES[self.spice]
			if spice_data then
				buff = eater:GetDebuff(spice_data.buff)
				if buff and buff.components.timer then
					bufftime = buff.components.timer:GetTimeLeft("buffover")
					duration = spice_data.duration
				end
			elseif hasorigin then
				eater.medal_eat_spicefood_flag = true--勋章buff的话加个标记统一处理即可
			end
		end
		if oldOnEaten then
		...
		--如果原先有Buff数据
		if buff ~= nil and bufftime > 0 and duration > 0 then
			--有本源主厨则叠时间
			if hasorigin then
				bufftime = math.min(duration + math.ceil(bufftime * 5 / 6), duration * 6)
			else--没有则确保叠的时间不会被覆盖掉
				bufftime = math.max(bufftime, duration)
			end
			buff.components.timer:StopTimer("buffover")
			buff.components.timer:StartTimer("buffover", bufftime)
		end
		eater.medal_eat_spicefood_flag = nil--移除临时标记
	end
end)

--矿工勋章提升清理月雹效率
AddComponentPostInit("lunarhailbuildup", function(self)
	local oldDoWorkToRemoveBuildup = self.DoWorkToRemoveBuildup
	self.DoWorkToRemoveBuildup=function(self, workcount, doer, ...)
		if doer and doer.large_mine or doer.medium_mine then
			local mult = TUNING_MEDAL.MINER_MEDAL[HasOriginMedal(doer,"large_mine") and "ORIGIN_MULT" or doer.large_mine and "LARGE_MULT" or "MEDIUM_MULT"]
			workcount = workcount * mult
		end
		if oldDoWorkToRemoveBuildup then
			oldDoWorkToRemoveBuildup(self, workcount, doer, ...)
		end
	end
end)
...
AddComponentPostInit("planarentity", function(self)
	...
	self.OnSave = function(self)
		local data = oldOnSave and oldOnSave(self) or {}
		data.chaos_times = self.chaos_times
		return data
	end
	--加载
	local oldOnLoad = self.OnLoad
	self.OnLoad = function(self, data)
		if data ~= nil and data.chaos_times then
			self.chaos_times = data.chaos_times
		end
		if oldOnLoad then
			oldOnLoad(self, data)
		end
...
AddClassPostConstruct("screens/playerhud", function(self)
    ...
    self.OpenContainer = function(self,container, side, ...)
		...
		if container:HasTag("multivariate_certificate") then
			OpenMedalWidget(self,container,false)
			return
		elseif special_medal_box[container.prefab] then
			OpenMedalWidget(self,container,true)
			return
		end
