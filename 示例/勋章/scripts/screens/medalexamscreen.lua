...
local MedalExamScreen =
    ...
    function(self, owner, attach, hasdictionary)
        ...
        local button_h = 50
        local buttons = {}
        table.insert(buttons,{text="A.!@$^@#@#@%", cb = function() self:MakeChoice(1) end })
        table.insert(buttons,{text="B.@!@#@&@%", cb = function() self:MakeChoice(2) end })
        table.insert(buttons,{text="C.*%%)$##(-*%!!@#$!%", cb = function() self:MakeChoice(3) end })
        table.insert(buttons,{text="D.*-*-!%*&#", cb = function() self:MakeChoice(4) end })

        self.menu = self.destspanel:AddChild(Menu(buttons, -button_h, false, "carny_xlong", nil, 30))
        ...
        if TheInput:ControllerAttached() and self.destspanel then
            self.destspanel:SetFocus()
        end
...
function MedalExamScreen:GetHelpText()
