...
function MedalTownportalScreen:RefreshDests()
    ...
    local function ScrollWidgetsCtor(context, index)
        ...
        local _OnControl = widget.OnControl
		widget.OnControl = function(_, control, down)
            --X键移除坐标
            if control == CONTROL_CONTROLLER_ATTACK and down then
                if widget.destitem and widget.destitem.deletebutton and widget.destitem.deletebutton:IsVisible() and widget.destitem.deletebutton.onclick then
                    widget.destitem.deletebutton.onclick()
                    return true
                end
            end
        ...
        --手柄控制提示
		function widget:GetHelpText()
			--移除坐标点按钮按键提示
            if widget.destitem and widget.destitem.deletebutton and widget.destitem.deletebutton:IsVisible() then
                return TheInput:GetLocalizedControl(TheInput:GetControllerID(), CONTROL_CONTROLLER_ATTACK, false, false) .. " " .. STRINGS.MEDAL_UI.DELETEPOS
            end
		end
...
function MedalTownportalScreen:GetHelpText()
    return TheInput:GetLocalizedControl(TheInput:GetControllerID(), CONTROL_CANCEL, false, false) .. " " .. STRINGS.UI.HELP.CLOSE
end
