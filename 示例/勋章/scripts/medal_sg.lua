...
AddStategraphState('wilson',
	State{
		...
		onenter = function(inst, posinfo)
			...
			inst.components.locomotor:Stop()
			inst.components.locomotor:Clear()
			if posinfo and posinfo.savehealth and inst.components.playercontroller ~= nil then
                inst.components.playercontroller:Enable(false)
            end
...
AddStategraphPostInit("bigshadowtentacle", function(sg)
	if sg.states then
		...
		if sg.states.idle then
			local oldonupdate = sg.states.idle.onupdate
			sg.states.idle.onupdate = function(inst, ...)
				if inst.from_tentaclemedal and (inst.components.combat.target == nil or HasOriginMedal(inst.components.combat.target,"tentaclemedal")) then
					inst.sg:GoToState("leave")
