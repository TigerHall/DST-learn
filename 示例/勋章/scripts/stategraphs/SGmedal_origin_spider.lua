...
local events =
{
    ...
    EventHandler("attacked", function(inst, data)
        ...
        --         inst.sg:GoToState("hit_stunlock")--挨打发呆
        --     end
        -- end
        if not inst.components.health:IsDead() and not inst.sg:HasAnyStateTag("attack", "evade") then
            if data and data.attacker and not inst.is_evaded then
                inst.sg:GoToState("evade",data.attacker)
            else
                inst.sg:GoToState("hit")
            end
        end
    end),
    EventHandler("onmissother", function(inst, data)--扑空了
        inst.miss_count = (inst.miss_count or 0) + 1--扑空计数
        --达到3次就不连扑了
        if inst.miss_count >= 3 then
            inst.miss_count = nil
        end
    end),
    EventHandler("onattackother", function(inst, data)--打中了，清空扑空计数
        inst.miss_count = nil
    end),
    EventHandler("doattack", function(inst, data)
        if not (inst.sg:HasStateTag("busy") or inst.components.health:IsDead()) then
            inst.is_evaded = nil--解除刚后撤完的状态
            --target CAN go invalid because SG events are buffered
            if inst:HasTag("spider_warrior") then
                inst.sg:GoToState(
                    data.target:IsValid()
                    and not inst:IsNear(data.target, TUNING_MEDAL.MEDAL_ORIGIN_SPIDER_MELEE_RANGE)
                    and "warrior_attack"--跳击
                    or "attack",
                    data.target
                )
...
local states =
{
    ...
    State{--近身攻击
        ...
        events=
        {
            EventHandler("animover", function(inst)
                inst.sg:GoToState("idle")
                inst.miss_count = nil--清空扑空计数(普通攻击不需要触发连击)
            end),
        },
    },
    ...
    State{--跳击
        ...
        {
            EventHandler("animover", function(inst)
                if inst.miss_count and inst.miss_count < 3 then
                    inst.sg:GoToState("idle")
                    inst.components.combat:ResetCooldown()--扑击完直接让攻击冷却
                    inst.components.combat:TryAttack()--尝试马上再来一次攻击
                else
                    inst.sg:GoToState("taunt")
                end
            end),
    ...
    State{--后撤
        name = "evade",
        tags = {"evade", "canrotate", "busy", "jumping"},

        onenter = function(inst, target)
            inst.AnimState:PlayAnimation("evade")
            inst.AnimState:SetDeltaTimeMultiplier(16/10)
            ...
            inst.Physics:SetMotorVelOverride(-30,2,0)
        end,
        ...
        timeline =
        {
            TimeEvent(2*FRAMES, function(inst) inst.Physics:SetMotorVel(-15, 1, 0) end),
            TimeEvent(5*FRAMES, function(inst) inst.Physics:SetMotorVel(-5, 0, 0) end),
        },
        ontimeout = function(inst)
            -- inst.sg:GoToState("taunt")
            inst.is_evaded = true--刚后撤完标记一下，防止连续后撤
            inst.components.combat:ResetCooldown()--后撤完直接让攻击冷却
            local chance = inst.components.health and inst.components.health:GetPercent() or .5
            --血量越低直接发起下一次攻击的概率越高
...
return StateGraph("SGmedal_origin_spider", states, events, "idle", actionhandlers)
