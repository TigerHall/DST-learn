...
local events =
{
    ...
    EventHandler("doattack", function(inst, data) 
        if not (inst.sg:HasStateTag("busy") or inst.components.health:IsDead()) then
            inst.sg:GoToState(
                data.target:IsValid()
                and not inst:IsNear(data.target, TUNING_MEDAL.MEDAL_ORIGIN_BEETLE_MELEE_RANGE)
...
local states =
{
    ...
    State{
        ...
        timeline=
        {
            TimeEvent(0*FRAMES, function(inst) inst.SoundEmitter:PlaySound(inst.sounds.walk) end),         
            TimeEvent(3*FRAMES, function(inst) inst.SoundEmitter:PlaySound(inst.sounds.walk) end),
            TimeEvent(6*FRAMES, function(inst) inst.SoundEmitter:PlaySound(inst.sounds.walk) end),
         },
    ...
    State{
        ...
        timeline =
        {
            TimeEvent(5 * FRAMES, function(inst) 
                inst.SoundEmitter:PlaySound(inst.sounds.move, "move")
            end),
...
CommonStates.AddHopStates(states, true, { pre = "fly", loop = "fly", pst = "land"})

return StateGraph("SGmedal_origin_beetle", states, events, "emerge", actionhandlers)
