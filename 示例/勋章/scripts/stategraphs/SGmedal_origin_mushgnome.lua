...
--生成孢子
local function spawn_spore(inst,position, angle)
    local radius = math.random() + 1.0
    local offset = Vector3(math.cos(angle), 0, -math.sin(angle)) * radius

    local spore = SpawnPrefab("medal_origin_mushgnome_spore")
    spore.Transform:SetPosition((position + offset):Get())
    if inst.components.combat then
        spore.followtarget = inst.components.combat.target--绑定追踪目标
    end
end
--叶片攻击
local AURA_EXCLUDE_TAGS = { "INLIMBO", "notarget", "noattack", "flight", "invisible", "playerghost", "chaos_creature"}
local function do_leaf_attack(inst)
    inst.components.combat:DoAreaAttack(inst, TUNING_MEDAL.MEDAL_ORIGIN_MUSHGNOME_AOE_RANGE, nil, nil, nil, AURA_EXCLUDE_TAGS)
end
    ...
    State{
        ...
        events =
        {
            EventHandler("animover", function(inst)
                ...
                --与目标足够近就叶片攻击,太远就释放孢子
                if target and inst:IsNear(target,TUNING_MEDAL.MEDAL_ORIGIN_MUSHGNOME_LEAF_ATTACK_RANGE) then
                    inst.sg:GoToState("attack")
                else
                    inst.sg:GoToState("spore_attack")
                end
            end),
        },
    },
    ...
    State{
        ...
        timeline =
        {
            TimeEvent(10*FRAMES, function(inst)
                if inst.DoLeafAttack then
                    inst:DoLeafAttack(true)
                end
                do_leaf_attack(inst)
            end),
            TimeEvent(25*FRAMES, do_leaf_attack),
            TimeEvent(40*FRAMES, do_leaf_attack),
            TimeEvent(55*FRAMES, do_leaf_attack),
        },
    ...
    State{
        ...
        timeline =
        {
            ...
            TimeEvent(25*FRAMES, function(inst)
                spawn_spore(inst,inst.sg.statemem.position, inst.sg.statemem.angles[1])
            end),
            TimeEvent(26*FRAMES, function(inst)
                spawn_spore(inst,inst.sg.statemem.position, inst.sg.statemem.angles[2])
            end),
            TimeEvent(27*FRAMES, function(inst)
                spawn_spore(inst,inst.sg.statemem.position, inst.sg.statemem.angles[3])
            end),
            TimeEvent(28*FRAMES, function(inst)
                spawn_spore(inst,inst.sg.statemem.position, inst.sg.statemem.angles[4])
            end),
            TimeEvent(29*FRAMES, function(inst)
                spawn_spore(inst,inst.sg.statemem.position, inst.sg.statemem.angles[5])
            end),
            TimeEvent(30*FRAMES, function(inst)
                spawn_spore(inst,inst.sg.statemem.position, inst.sg.statemem.angles[6])
            end),
            TimeEvent(31*FRAMES, function(inst)
                spawn_spore(inst,inst.sg.statemem.position, inst.sg.statemem.angles[7])
            end),
            TimeEvent(32*FRAMES, function(inst)
                spawn_spore(inst,inst.sg.statemem.position, inst.sg.statemem.angles[8])
            end),
            TimeEvent(33*FRAMES, function(inst)
                spawn_spore(inst,inst.sg.statemem.position, inst.sg.statemem.angles[9])
            end),
            TimeEvent(34*FRAMES, function(inst)
                spawn_spore(inst,inst.sg.statemem.position, inst.sg.statemem.angles[10])
            end),
            TimeEvent(35*FRAMES, function(inst)
                spawn_spore(inst,inst.sg.statemem.position, inst.sg.statemem.angles[11])
            end),
            TimeEvent(36*FRAMES, function(inst)
                spawn_spore(inst,inst.sg.statemem.position, inst.sg.statemem.angles[12])
            end),
        },
...
return StateGraph("SGmedal_origin_mushgnome", states, events, "idle", actionhandlers)
