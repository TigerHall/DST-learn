...
function medal_showbufftime:getBuffTime(inst)
	...
	return inst.components.timer:GetTimeLeft("buffover") 
		or inst.components.timer:GetTimeLeft("regenover") 
		or inst.components.timer:GetTimeLeft("decay") 
		or -1
end
...
function medal_showbufftime:SetBuffInfo()
	...
	if self.inst and self.inst.components.debuffable and self.inst.components.debuffable.debuffs then
		for _, v in pairs(self.inst.components.debuffable.debuffs) do
			if v.inst and v.inst.components.timer then
				...
				--buff层数读取
				if v.inst.buff_layer then
					buffdata.bufflayer = v.inst.buff_layer
				end
	...
	if self.getbuffinfofn then
		self.getbuffinfofn(self.inst,buff_info)
	end
	--多一个方法选择,方便别的Mod作者兼容
	if self.inst.GetMedalBuffInfo then
		self.inst:GetMedalBuffInfo(buff_info)
	end
	--排序、转码
	if #buff_info>0 then
