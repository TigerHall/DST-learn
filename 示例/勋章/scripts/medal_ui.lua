...
local default_pos={
	...
	medal_krampus_chest_item = Vector3(-405, -90, 0),--坎普斯之匣
	...
	medal_box = Vector3(-137, -132, 0),--勋章盒2*7
	-- medal_box = Vector3(237, 336, 0),--Vector3(-140, -120, 0),--勋章盒2*7
	spices_box = Vector3(-270, -132, 0),--调料盒2*7
	-- spices_box = Vector3(237, 336, 0),--Vector3(0, 220, 0),--调料盒2*7
	medal_ammo_box = Vector3(-270, -132, 0),--弹药盒2*7
	-- medal_ammo_box = Vector3(237, 336, 0),--Vector3(-275, -120, 0),--弹药盒2*7
...
function params.medal_pay_tribute_box.widget.buttoninfo.fn(inst, doer)
	if inst.components.container then
		if inst.components.container:IsFull() and inst.PayTribute then
			inst:PayTribute(doer)--奉纳
		end
	elseif inst.replica.container ~= nil and not inst.replica.container:IsBusy() then
		SendRPCToServer(RPC.DoWidgetButtonAction, nil, inst, nil)
	end
...
function params.medal_krampus_chest.itemtestfn(container, item, slot)
    return krampus_chest_white_list[item.prefab] 
		...
		or not (item:HasTag("irreplaceable") or item:HasTag("_container") or item.prefab=="wortox_soul")
end
...
params.medal_krampus_chest_item = {
	widget =
	{
		...
		side_align_tip = -420,--手柄操作键位提示偏移值,正值靠右负值靠左
		dragtype="medal_krampus_chest_item",--拖拽标签，有则可拖拽
	},
	-- issidewidget = true,--是否为侧边容器,为true会导致手柄无法打开它
	type = "medal_krampus_chest",
}
--2*6
for y = 0, 5 do
...
params.medal_krampus_chest_item.itemtestfn = params.medal_krampus_chest.itemtestfn
...
params.spices_box = {
	widget =
	{
		...
		dragtype="medal_extra_box",--拖拽标签，有则可拖拽
	},
	-- issidewidget = true,
	type = "medal_extra_box",
}
...
local function autoDistributionFn(inst, doer)
	if inst.components.container ~= nil then
		if inst.autoDistribution then
			inst:autoDistribution()
		end
	elseif inst.replica.container ~= nil and not inst.replica.container:IsBusy() then
		SendRPCToServer(RPC.DoWidgetButtonAction, nil, inst, nil)
	end
...
params.armor_medal_space_time =
{
    widget =
    {
        ...
        pos = Vector3(-50, -80, 0),
		dragtype="armor_medal_space_time",--拖拽标签，有则可拖拽
    },
    issidewidget = true,
    type = "pack",
    openlimit = 1,
...
params.armor_medal_space_time_lv1 =
{
    widget =
    {
        slotpos = {},
        ...
        pos = Vector3(-5, -70, 0),
		dragtype="armor_medal_space_time_lv1",--拖拽标签，有则可拖拽
    },
    issidewidget = true,
    type = "pack",
    openlimit = 1,
...
for y = 0, 2 do
    table.insert(params.armor_medal_space_time_lv1.widget.slotpos, Vector3(-162, -75 * y + 75, 0))
    table.insert(params.armor_medal_space_time_lv1.widget.slotpos, Vector3(-162 + 75, -75 * y + 75, 0))
end

params.armor_medal_space_time_lv2 =
{
    widget =
    {
        slotpos = {},
        ...
        pos = Vector3(-5, -80, 0),
		dragtype="armor_medal_space_time_lv2",--拖拽标签，有则可拖拽
    },
    issidewidget = true,
    type = "pack",
    openlimit = 1,
...
for y = 0, 3 do
    table.insert(params.armor_medal_space_time_lv2.widget.slotpos, Vector3(-162, -75 * y + 114, 0))
    table.insert(params.armor_medal_space_time_lv2.widget.slotpos, Vector3(-162 + 75, -75 * y + 114, 0))
end

params.armor_medal_space_time_lv3 =
{
    widget =
    {
        slotpos = {},
        ...
        animbuild = "ui_piggyback_2x6",
        pos = Vector3(-5, -90, 0),
		dragtype="armor_medal_space_time_lv3",--拖拽标签，有则可拖拽
    },
    issidewidget = true,
    type = "pack",
    openlimit = 1,
...
for y = 0, 4 do
    table.insert(params.armor_medal_space_time_lv3.widget.slotpos, Vector3(-165, -81 * y + 145, 0))
    table.insert(params.armor_medal_space_time_lv3.widget.slotpos, Vector3(-165 + 81, -81 * y + 145, 0))
end

params.armor_medal_space_time_lv4 =
{
    widget =
    {
        slotpos = {},
        ...
        animbuild = "ui_piggyback_2x6",
        pos = Vector3(-5, -90, 0),
		dragtype="armor_medal_space_time_lv4",--拖拽标签，有则可拖拽
    },
    issidewidget = true,
    type = "pack",
    openlimit = 1,
...
for y = 0, 5 do
    table.insert(params.armor_medal_space_time_lv4.widget.slotpos, Vector3(-162, -75 * y + 170, 0))
    table.insert(params.armor_medal_space_time_lv4.widget.slotpos, Vector3(-162 + 75, -75 * y + 170, 0))
end

params.armor_medal_space_time_lv5 =
{
    widget =
    {
        slotpos = {},
        ...
        animbuild = "ui_krampusbag_2x8",
        pos = Vector3(-5, -130, 0),
		dragtype="armor_medal_space_time_lv5",--拖拽标签，有则可拖拽
    },
    issidewidget = true,
    type = "pack",
    openlimit = 1,
...
for y = 0, 6 do
    table.insert(params.armor_medal_space_time_lv5.widget.slotpos, Vector3(-162, -75 * y + 240, 0))
    table.insert(params.armor_medal_space_time_lv5.widget.slotpos, Vector3(-162 + 75, -75 * y + 240, 0))
end
...
local hideleaves = nil
local old_build = nil
local function ChangeBuild(widget)
...
local function new_hideleaves(widget)
	ChangeBuild(widget)
	if hideleaves ~= nil then
		hideleaves(widget)
	end
end
...
AddClassPostConstruct("widgets/leafcanopy", function(self)
	if self.OnUpdate ~= nil then
		...
		if hideleaves == nil then
			hideleaves = upvaluehelper.Get(self.OnUpdate, "hideleaves")
			if hideleaves ~= nil then
				upvaluehelper.Set(self.OnUpdate,"hideleaves",new_hideleaves)
			end
		end
	end
end)
...
AddClassPostConstruct("widgets/itemtile", function(self)
	--同步特效
	self.HandleMedalLoseHealthFX = function(self,islosehealth)
		if islosehealth == nil then
			islosehealth = self.item.ismedallosehealth and self.item.ismedallosehealth:value()
		end
		if islosehealth then
			if self.medal_lose_health == nil then
				self.medal_lose_health = self.image:AddChild(UIAnim())
				self.medal_lose_health:GetAnimState():SetBank("inventory_fx_acidsizzle")
				self.medal_lose_health:GetAnimState():SetBuild("inventory_fx_acidsizzle")
				self.medal_lose_health:GetAnimState():PlayAnimation("idle", true)
				self.medal_lose_health:GetAnimState():SetMultColour(.65, 0, 0, 1)
				-- self.medal_lose_health:GetAnimState():SetTime(math.random())
				self.medal_lose_health:GetAnimState():SetDeltaTimeMultiplier(38/30)--加快播放速度，和扣血的周期匹配
				self.medal_lose_health:SetScale(.33)
				self.medal_lose_health:SetPosition(0,20)--适当向上偏移
				self.medal_lose_health:GetAnimState():AnimateWhilePaused(false)
				self.medal_lose_health:SetClickable(false)
			end
		else
			if self.medal_lose_health ~= nil then
				self.medal_lose_health:Kill()
				self.medal_lose_health = nil
			end
		end
	end

	self:HandleMedalLoseHealthFX()
	self.inst:ListenForEvent("medallosehealthchange",
        function(invitem)
	...
	self.StartDrag = function(self,...)
		if self.item.replica.inventoryitem ~= nil then 
			self:HandleMedalLoseHealthFX(false)
		end
		oldStartDrag(self,...)
	end
...
-------------------------------------------勋章gm加入提示栏----------------------------------------------
---------------------------------------------------------------------------------------------------------
local prediction_command_dm
AddClassPostConstruct("screens/consolescreen",function(self)
	local should_dm = not prediction_command_dm
    if should_dm then
        if should_dm then
            prediction_command_dm = {}
        end
        for k, v in pairs(_G) do
            if type(v) == "function" then
                if should_dm and k:find("dm_") == 1 then
					table.insert(prediction_command_dm, k:sub(4, -1))
                end
            end
        end
    end
	if prediction_command_dm then
		self.console_edit:AddWordPredictionDictionary({words = prediction_command_dm, delim = "dm_", num_chars = 0})
	end
end)
...
AddClassPostConstruct("screens/redux/pausescreen", function(self)
	if self.menu then
		self.menu.medalpage = self.menu:AddItem(STRINGS.MEDAL_SETTING_UI.TITLE,function()
			self:unpause()
			if ThePlayer and ThePlayer.HUD then
				ThePlayer.HUD:ShowMedalSettingsScreen()
			end
		end)
		self.menu.medalpage:SetScale(.7)
		--调整下按钮之间的间距
		self.menu.offset = self.menu.offset + 5
