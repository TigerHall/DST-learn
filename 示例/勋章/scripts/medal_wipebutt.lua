...
AddPrefabPostInit("wolfgang",function(inst)
	...
	inst.SetGymStopState = function(inst, ...)
		...
		--做个延迟,重选人物可能会导致网络变量的执行顺序发生改变，从而导致这段代码生效后又被覆盖成nil
		inst:DoTaskInTime(0,function(inst)
...
GLOBAL.GetActionFailString = function(inst,action,reason,...)
	--如果是勋章特有的动作,直接劫持掉,别走原本那套傻不拉几的匹配方式了
	if STRINGS.MEDAL_ACTIONFAIL and STRINGS.MEDAL_ACTIONFAIL[action] then
		local str = nil
		if reason ~= nil and type(reason)== "string" then
			str = STRINGS.MEDAL_ACTIONFAIL[action][string.upper(reason)]
		end
		if str ~= nil then
			return str
		end
	end
	return oldGetActionFailString and oldGetActionFailString(inst,action,reason,...)
end
