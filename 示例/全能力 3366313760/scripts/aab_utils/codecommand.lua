...
------------------------------------------------------------------------------------------------------------------------
--13. AnimState
weapon.AnimState:OverrideSymbol("swap_hat", "myflowerhat", "swap_hat")
-- 表示用myflowerhat.scml动画中的swap_hat(后)文件夹中的图，替换owner中swap_hat(前)中的图。

weapon.AnimState:ClearOverrideSymbol("swap_hat")
-- 取消替换某个文件夹的图，即替换回来。
...
-- 可以让物品一闪一闪的
weapon.AnimState:SetHaunted(true)
--发光
weapon.AnimState:SetSymbolBloom("bolt_f")
weapon.AnimState:SetSymbolLightOverride("bolt_b", .5)

-- 动画时长
print(weapon.AnimState:GetCurrentAnimationLength())
