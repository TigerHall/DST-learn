...
TUNING.RIFT_BIRD_HEALTH = 100  -- 亮喙鸟
...
local errortext = TUNING.isCh2hm and "它感觉到了这种食物的危险" or "The bird can't eat it now"
...
-- 乌鸦黑血
local function birdcd(inst) 
    inst.birdattackedtask2hm = nil 
end
...
-- 啄木鸟流血效果
local function woodpecker_bleeding_task(inst, attacker)
    if attacker and attacker:IsValid() and attacker.components.health and not attacker.components.health:IsDead() then
        ...
        attacker.woodpecker_bleed_count = (attacker.woodpecker_bleed_count or 0) + 1
        
        if attacker.woodpecker_bleed_count >= 15 and attacker.woodpecker_bleed_task then
            attacker.woodpecker_bleed_task:Cancel()
            attacker.woodpecker_bleed_task = nil
            attacker.woodpecker_bleed_count = nil
            
            if attacker.components.talker then
                attacker.components.talker:Say(TUNING.isCh2hm and "流血终于停止了!" or "The bleeding finally stopped!")
            end
        end
    elseif attacker then 
        if attacker.woodpecker_bleed_task then
            attacker.woodpecker_bleed_task:Cancel()  
            attacker.woodpecker_bleed_task = nil    
        end
        attacker.woodpecker_bleed_count = nil       
...
local function woodpecker_onblockedorattacked(inst, data)  
    if not inst:HasTag("swc2hm") and data and data.attacker and data.attacker.components.health and not data.attacker.components.health:IsDead() and
        not data.attacker:HasTag("catcoon") and not data.attacker.birdattackedtask2hm then
        
        data.attacker.birdattackedtask2hm = data.attacker:DoTaskInTime(0.25, birdcd)

        if data.attacker.components.combat then data.attacker.components.combat:GetAttacked(inst, 2) end
        
        if data.attacker.woodpecker_bleed_task then
            data.attacker.woodpecker_bleed_task:Cancel()
        end
        if data.attacker.components.talker then
            data.attacker.components.talker:Say(TUNING.isCh2hm and "它的嘴很尖!" or "The woodpecker's beak is sharp!")
        end
        
        data.attacker.woodpecker_bleed_count = 0
        
        data.attacker.woodpecker_bleed_task = data.attacker:DoPeriodicTask(1, function() 
            woodpecker_bleeding_task(inst, data.attacker) 
        end)
...
if TUNING.DSTU then
    AddPrefabPostInit("woodpecker", function(inst)
        if not TheWorld.ismastersim then return end
        inst:ListenForEvent("attacked", woodpecker_onblockedorattacked)
    end)
end