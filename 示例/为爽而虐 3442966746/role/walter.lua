if TUNING.DSTU and TUNING.WIXIE_HEALTH then -- 仅不妥协和维希改动开启时生效
    -- 烟花弹跳
    local radius = 0.5
...
-- 基础射程缩短
TUNING.SLINGSHOT_DISTANCE = 9
TUNING.SLINGSHOT_DISTANCE_MAX = 12


local slingshotammo_rock_projs = {
...
for i, v in pairs(slingshotammo_rock_projs) do  
    AddPrefabPostInit(v, function(inst)
        if not TheWorld.ismastersim then return end
        if inst.components.projectile then
            inst.components.projectile.range = 20   -- 蓄力射程缩短
        end
    end)
end

-- 减速弹药效果削弱
TUNING.SLINGSHOT_AMMO_MOVESPEED_MULT = 4/5

-- 弹药产量削弱
local slingshot_ammos = {
    "slingshotammo_rock",
    "slingshotammo_gold",
...
for _, recipe_name in ipairs(slingshot_ammos) do
    AddRecipePostInit(recipe_name, function(inst)
        if inst.numtogive then
            inst.numtogive = math.floor(inst.numtogive * 0.5)
        end
    end)
    AddPrefabPostInit(recipe_name, function(inst)
        if not TheWorld.ismastersim then return end
        if inst.components.edible then
            inst.components.edible.hungervalue = 0.1 -- 原食用价值为1
        end
    end)
end

-- 冲刺与被动的暗影冲刺wobydodge2hm共享cd
    
-- AddPrefabPostInit("wobybig", function(inst)
--     if not TheWorld.ismastersim then return end
--     inst:RemoveAllEventCallbacks("ondash_woby")
--     inst:ListenForEvent("ondash_woby", OnDash2hm)
-- end) 