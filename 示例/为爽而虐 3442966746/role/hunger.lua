...
AddPrefabPostInit("lifeinjector", function(inst)
    ...
    -- 处理DSTU重做后的强心针机制
    if inst.components.maxhealer then
        local originalHeal = inst.components.maxhealer.Heal
        inst.components.maxhealer.Heal = function(self, target, ...)
            local result = originalHeal(self, target, ...)
            if result and target and target:IsValid() and target:HasTag("player") and target.components.hunger and 
                target.components.hunger.penalty2hm and target.components.hunger.penalty2hm > 0 then
                local healthp = target and target.components.health and target.components.health.penalty or 0
                target.components.hunger:DeltaPenalty2hm(TUNING.MAX_HEALING_NORMAL - (healthp < 0.25 and (0.25 - healthp) or 0))
            end
            return result
        end
    elseif inst.components.healer then
        -- 兼容原版机制
        local OnUse = inst.components.healer.onhealfn
        inst.components.healer.onhealfn = function(inst, target, ...)
            OnUse(inst, target, ...)
            if target and target:IsValid() and target:HasTag("player") and target.components.hunger and target.components.hunger.penalty2hm and
                target.components.hunger.penalty2hm > 0 then
                local healthp = target and target.components.health and target.components.health.penalty or 0
                target.components.hunger:DeltaPenalty2hm(TUNING.MAX_HEALING_NORMAL - (healthp < 0.25 and (0.25 - healthp) or 0))
            end
        end
...
local function OnEat(inst, data)
    if inst.components.hunger and inst.components.hunger.penalty2hm and inst.components.hunger.penalty2hm > 0 and inst.components.eater and 
       data and data.food and data.food.components.edible then
        -- 处理齿轮（WX78专属食物）
        if data.food.components.edible.foodtype == FOODTYPE.GEARS and data.food.components.edible.healthvalue and data.food.components.edible.healthvalue > 0 then
            ...
            local value = -health_delta / 1500
            inst.components.hunger:DeltaPenalty2hm(value)
        -- 处理其他食物
        elseif (data.food:HasTag("fresh") or data.food.dryseeds2hm or data.food.dryveggies2hm) and 
            ...
            elseif table.contains(dried_food, data.food.prefab) then
