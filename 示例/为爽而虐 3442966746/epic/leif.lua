...
local function onleifdeath(inst) 
    --统一阵亡
    TheWorld:PushEvent("leifdeath2hm", {leif = inst}) 
    --添加掉落
...
AddComponentPostInit("deciduoustreeupdater", function(self)
    if self.inst.components.workable then
        self.inst.components.workable:SetShouldRecoilFn(function(inst, worker, tool, ...)
            if worker ~= nil and worker:HasTag("epic") then 
                return false, 1 
            elseif worker ~= nil and worker.prefab == "woodie" and worker.components.skilltreeupdater 
                and worker.components.skilltreeupdater:IsActivated("woodie_human_treeguard_2") then   
                if not GetModConfigData("Woodie SkillTree") then return end 
                if tool and tool.prefab == "lucy" then
                    return false, 1
                end
            end
