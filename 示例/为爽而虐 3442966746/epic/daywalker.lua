...
AddComponentPostInit("forestdaywalkerspawner",function(self)
	...
	self.OnDayChange = function(self, ...)
		...
		-- 绕过shard_daywalkerspawner的位置检查，让计时器正常工作，否则计时器位于地下而不是地上
		...
		-- 天数到达0时，尝试设置垃圾堆并埋入拾荒疯猪
		if not self:TryToSetDayWalkerJunkPile() then
			-- 如果找不到垃圾堆，重置计时器为1天后再次尝试
...
AddComponentPostInit("daywalkerspawner",function(self)
	self.OnDayChange = function(self, ...)
		...
		-- 完全绕过shard_daywalkerspawner的位置检查，让梦魇疯猪独立刷新
		local days_to_spawn = self.days_to_spawn
		if days_to_spawn > 0 then
			self.days_to_spawn = days_to_spawn - 1
			return
...
-- 激光炮伤害削弱
TUNING.ALTERGUARDIAN_PHASE3_LASERDAMAGE = TUNING.ALTERGUARDIAN_PHASE3_LASERDAMAGE * 5/6
-- 血量解锁能力增强
local PHASES =
{
	[0] = {
...
AddBrainPostInit("daywalker2brain", function(self)
	if self.bt and self.bt.root and self.bt.root.children and self.bt.root.children[1] then
...
AddStategraphPostInit("daywalker2", function(sg)
	-- 缩短本体冲刺时间
	if sg.states.tackle_loop then
		local old_onenter = sg.states.tackle_loop.onenter
		sg.states.tackle_loop.onenter = function(inst, target)
...
AddStategraphState("daywalker2", State{
	...
	onenter = function(inst, target)
		...
		-- 吼叫前摇
		if inst.tacklenumberPG == 0 then
			inst.components.locomotor:Stop()
			inst.AnimState:PlayAnimation("taunt")
			if target and target:IsValid() then
				inst:ForceFacePoint(target.Transform:GetWorldPosition())
			end
			inst.sg.statemem.roaring = true
		...
		inst.sg:SetTimeout(inst.AnimState:GetCurrentAnimationLength() * 2) -- 16 * 2 
	...
	onupdate = function(inst)
		if inst.sg.statemem.roaring then
			return  
		end
	...
	events =
	{
		EventHandler("animover", function(inst)
			if inst.AnimState:AnimDone() and inst.sg.statemem.roaring then
				inst.tacklenumberPG = 1 
				inst.sg.statemem.tackling = true
				inst.sg:GoToState("tackle_pre", inst.sg.statemem.target)
			end
		end),
	},

	ontimeout = function(inst)
		if inst.tacklenumberPG >= 4 then
...
-- 扔垃圾球动画加快为2倍
local function updateattackspeed2hm(sg)
    for _, state in pairs(sg.states) do
...
AddRecipe2("wagpunk_bits", {Ingredient("gears", 1), Ingredient("trinket_6", 1)}, TECH.LOST,
			...
			{"REFINE"})
-- 视野拓展套件
AddRecipe2("scrap_monoclehat", {Ingredient("wagpunk_bits", 2), Ingredient("transistor", 1), Ingredient("trinket_6", 1)}, 
			TECH.LOST,
...
local newloots ={
	"scraphat",							-- 拾荒尖帽
	"wagpunk_bits_blueprint",			-- 废铁蓝图
	"scrap_monoclehat_blueprint",		-- 视野拓展套件蓝图
	"wagpunkbits_kit_blueprint",		-- 修理机蓝图
}

-- 月后额外掉落废铁套蓝图，弹性空间容器蓝图
local afterlunarloot = {				
	"wagpunkhat_blueprint",				-- 废铁头蓝图
	"armorwagpunk_blueprint",			-- 废铁甲蓝图
	"chestupgrade_stacksize_blueprint",	-- 弹性空间容器蓝图
}
