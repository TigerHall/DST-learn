...
local function GetPlayerFromClientTable(c) for _, v in ipairs(AllPlayers) do if v.userid == c.userid then return v end end end
...
function SinkholeSpawner:StartSinkholes()
    ...
    if num_players > 0 then
        ...
        for i, v in pairs(target_players) do
            local num_attacks = math.min(base_num_attacks + math.floor((((v.playerage or 1) - 1) / year_length)) * 2 + TUNING.ANTLION_SINKHOLE.WAVE_MIN_ATTACKS,
                                         TUNING.ANTLION_SINKHOLE.WAVE_MAX_ATTACKS)
            if num_attacks > 0 then
                local targetinfo = {client = v, userhash = smallhash(v.userid), attacks = num_attacks}
                self:UpdateTarget(targetinfo)
                if targetinfo.client ~= nil then
                    if #self.targets >= MAX_TARGETS then table.remove(self.targets, 1) end
...
function SinkholeSpawner:UpdateTarget(targetinfo)
    ...
    if targetinfo.pos ~= nil and not TheShard:IsMigrating(targetinfo.client.userid) then targetinfo.pos = nil end
end
    ...
    elseif targetinfo.warnings >= 0 then
        if targetinfo.pos ~= nil then
            ...
            for i = 1, math.ceil((TUNING.ANTLION_SINKHOLE.NUM_WARNINGS - (targetinfo.warnings or 0) + 1) / 3.2) do
                local fxpt = targetinfo.pos +
                                 Vector3((math.random() * (WARNING_RADIUS * 2)) - WARNING_RADIUS, 0, (math.random() * (WARNING_RADIUS * 2)) - WARNING_RADIUS)
...
function SinkholeSpawner:DoTargetAttack(targetinfo)
    if targetinfo.pos ~= nil then
        ...
        targetinfo.next_attack = targetinfo.attacks > 0 and TUNING.ANTLION_SINKHOLE.WAVE_ATTACK_DELAY + math.random() *
                                     TUNING.ANTLION_SINKHOLE.WAVE_ATTACK_DELAY_VARIANCE or nil
...
local function eyeofterror_sinkholefx(inst, x, y, z)
    -- local fx = SpawnPrefab("alterguardian_spintrail_fx")
    -- fx.Transform:SetPosition(x, y, z)
    -- SpawnPrefab("mining_moonglass_fx").Transform:SetPosition(x, y, z)
    -- inst:DoTaskInTime(70 * FRAMES, function()
    local eyeofterror_sinkhole = SpawnPrefab("eyeofterror_sinkhole")
    eyeofterror_sinkhole.Transform:SetPosition(x, y, z)
...
function SinkholeSpawner:SpawnSinkhole(spawnpt)
    ...
    offset = IsValidSinkholePosition(offset) and offset or
                 FindValidPositionByFan(math.random() * 2 * PI, TUNING.ANTLION_SINKHOLE.RADIUS / 2 * 1.8 + math.random(), 9, IsValidSinkholePosition) or
                 FindValidPositionByFan(math.random() * 2 * PI, TUNING.ANTLION_SINKHOLE.RADIUS / 2 * 2.9 + math.random(), 17, IsValidSinkholePosition) or
                 FindValidPositionByFan(math.random() * 2 * PI, TUNING.ANTLION_SINKHOLE.RADIUS / 2 * 3.9 + math.random(), 17, IsValidSinkholePosition) or nil
    ...
    if offset ~= nil then
        ...
        antlion_sinkhole:DoTaskInTime(3, function()
            antlion_sinkhole.remainingrepairs = antlion_sinkhole.remainingrepairs or 1
            for i = 1, antlion_sinkhole.remainingrepairs do
                if antlion_sinkhole and antlion_sinkhole:IsValid() then
                    antlion_sinkhole.components.timer:StopTimer("nextrepair")
                    antlion_sinkhole:PushEvent("timerdone", {name = "nextrepair"})
                end
            end
            eyeofterror_sinkholefx(self.inst, x + offset.x, 0, z + offset.z)
        end)
        -- sinkhole:PushEvent("startcollapse")
        -- sinkhole:DoTaskInTime(7, function() SpawnPrefab("mod_hardmode_lavae").Transform:SetPosition(x + offset.x, 0, z + offset.z) end)
        -- sinkhole:DoTaskInTime(120, sinkhole.Remove)
    end
end
...
function SinkholeSpawner:PushRemoteTargets()
    ...
    for i, v in ipairs(self.targets) do
        if v.pos == nil then
            ...
            table.insert(data, {userhash = v.userhash, warn = warn or nil, attack = attack or nil})
        end
    end
...
function SinkholeSpawner:OnUpdate(dt)
    ...
    for i, v in ipairs(self.targets) do
        ...
        if v.next_warning ~= nil then
            ...
            if v.next_warning <= 0 then table.insert(towarn, v) end
        elseif v.next_attack ~= nil then
            v.next_attack = v.next_attack - dt
            if v.next_attack <= 0 then table.insert(toattack, 1, {i, v}) end
        end
    end

    self:PushRemoteTargets()

    for i, v in ipairs(towarn) do self:DoTargetWarning(v) end

    for i, v in ipairs(toattack) do
        self:DoTargetAttack(v[2])
        if v[2].attacks <= 0 then table.remove(self.targets, v[1]) end
    end
    ...
    if data.targets ~= nil then
        for i, v in ipairs(data.targets) do
            if (v.attacks or 0) > 0 and v.x ~= nil and v.z ~= nil then
                local targetinfo = {pos = Vector3(v.x, 0, v.z), attacks = v.attacks, next_attack = v.next_attack, warnings = 0, next_warning = v.next_warning}
                if #self.targets >= MAX_TARGETS then table.remove(self.targets, 1) end
                table.insert(self.targets, targetinfo)
            end
        end
...
function SinkholeSpawner:GetDebugString()
    ...
    for i, v in ipairs(self.targets) do
        if v.next_warning ~= nil then
            s = (s ~= nil and (s .. "\n") or "") ..
                    string.format("  Warning %s (%d/%d) at (%.1f, %.1f) in %.2fs", tostring(v.player), TUNING.ANTLION_SINKHOLE.NUM_WARNINGS - v.warnings,
                                  TUNING.ANTLION_SINKHOLE.NUM_WARNINGS, v.pos ~= nil and v.pos.x or 0, v.pos ~= nil and v.pos.z or 0, v.next_warning)
        elseif v.next_attack ~= nil then
            s = (s ~= nil and (s .. "\n") or "") ..
                    string.format("  Attacking %s (%d/%d) at (%.1f, %.1f) in %.2fs", tostring(v.player), TUNING.ANTLION_SINKHOLE.WAVE_MAX_ATTACKS - v.attacks,
                                  TUNING.ANTLION_SINKHOLE.WAVE_MAX_ATTACKS, v.pos ~= nil and v.pos.x or 0, v.pos ~= nil and v.pos.z or 0, v.next_attack)
        end
    end
    return s
end
