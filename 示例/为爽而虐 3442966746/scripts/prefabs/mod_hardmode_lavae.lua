...
local assets = {Asset("ANIM", "anim/lavae.zip"), Asset("SOUND", "sound/together.fsb")}

local prefabs = {"lavae_move_fx"}
...
--]]
local function RetargetFn(inst)
    return FindEntity(inst, TUNING.HOUND_TARGET_DIST, function(guy) return inst.components.combat:CanTarget(guy) end, nil,
                      {"lavae", "dragonfly", "mock_dragonfly"}) or nil
end

local function KeepTargetFn(inst, target)
    return target ~= nil and target.components.combat ~= nil and target.components.health ~= nil and inst.components.combat:CanTarget(target) and
               not target.components.health:IsDead()
end

local function LockTarget(inst, target) inst.components.combat:SetTarget(target) end

local function OnNewTarget(inst, data)
    if data.oldtarget ~= nil then inst:RemoveEventCallback("death", inst._ontargetdeath, data.oldtarget) end
    if data.target ~= nil and data.target:HasTag("player") then inst:ListenForEvent("death", inst._ontargetdeath, data.target) end
end

local function OnEntitySleep(inst) end

local function OnTimerDone(inst, data) if data.name == "selfdestruct" then inst.components.health:Kill() end end
...
STRINGS.NAMES.MOD_HARDMODE_LAVAE = STRINGS.NAMES.LAVAE

return Prefab("mod_hardmode_lavae", fn, assets, prefabs)
