...
local function CalcNewSize() return math.random() end
...
local function OnInventoryLanded(inst)
    ...
    if TheWorld.Map:IsPassableAtPoint(x, y, z) then
        if inst.flop_task ~= nil then inst.flop_task:Cancel() end
...
local function OnProjectileLand(inst)
    ...
    if land_in_water then
        ...
        if inst.components.weighable ~= nil then inst.components.weighable:SetPlayerAsOwner(nil) end
        inst.leaving = true
        inst.persists = false
        ...
        if fish.flop_task then fish.flop_task:Cancel() end
        Flop(fish)
...
local function OnReelingIn(inst, doer)
    if inst:HasTag("partiallyhooked") then
        ...
        if inst.components.homeseeker ~= nil and inst.components.homeseeker.home ~= nil and inst.components.homeseeker.home:IsValid() and
            inst.components.homeseeker.home.prefab == "oceanfish_shoalspawner" then
...
local function ondroppedasloot(inst, data) if data ~= nil and data.dropper ~= nil then inst.components.weighable.prefab_override_owner = data.dropper.prefab end end
...
local function topocket(inst) if inst.components.propagator ~= nil then inst.components.propagator:StopSpreading() end end

local function toground(inst) if inst.components.propagator ~= nil then inst.components.propagator:StartSpreading() end end
...
local function OnEntitySleep(inst) if not POPULATING then inst.remove_task = inst:DoTaskInTime(.1, HandleEntitySleep) end end

local function OnSave(inst, data)
    if inst.components.herdmember.herdprefab then data.herdprefab = inst.components.herdmember.herdprefab end
    if inst.heavy then data.heavy = true end
end

local function OnLoad(inst, data)
    if data ~= nil and data.herdprefab ~= nil then inst.components.herdmember.herdprefab = data.herdprefab end
    if data ~= nil and data.heavy then inst.heavy = data.heavy end
end
...
local function water_common(data)
    ...
    if data.fishtype ~= nil then inst:AddTag("ediblefish_" .. data.fishtype) end
    if data.tags ~= nil then for _, tag in ipairs(data.tags) do inst:AddTag(tag) end end
    ...
    inst.components.oceanfishable:StrugglingSetup(inst.components.locomotor.walkspeed, inst.components.locomotor.runspeed,
                                                  data.stamina or TUNING.OCEANFISH.FISHABLE_STAMINA)
...
local function onequip(inst, owner)
    local fx = SpawnPrefab(inst.fish_def.heavyfx2hm)
    if fx then
        fx.entity:AddTag("INLIMBO")
        fx.persists = false
        fx.entity:SetParent(owner.entity)
        fx.entity:AddFollower()
        fx.Follower:FollowSymbol(owner.GUID, "swap_body", 0, 1, 0)
        inst.fx = fx
    end
end
...
local function inv_common(fish_def)
    ...
    if fish_def.dynamic_shadow then inst.entity:AddDynamicShadow() end
    ...
    if fish_def.dynamic_shadow then inst.DynamicShadow:SetSize(fish_def.dynamic_shadow[1], fish_def.dynamic_shadow[2]) end
    ...
    if fish_def.tags ~= nil then for _, tag in ipairs(fish_def.tags) do inst:AddTag(tag) end end
    ...
    inst.components.inventoryitem:ChangeImageName(fish_def.prefab .. "_inv")
    ...
    inst.components.symbolswapdata:SetData(fish_def.build, "swap_body")
    ...
    if fish_def.heater ~= nil then
        ...
        inst.components.heater.carriedheatmultiplier = 4
        if fish_def.heater.endothermic then inst.components.heater:SetThermics(false, true) end
    end
...
local function fx_common(fish_def)
    local inst = CreateEntity()
...
local function MakeFish(olddata)
    local data = deepcopy(olddata)
    ...
    if data.bank ~= data.build then
        ...
        table.insert(assets, Asset("INV_IMAGE", data.prefab .. "_inv"))
    end

    if data.extra_anim_assets ~= nil then for _, v in ipairs(data.extra_anim_assets) do table.insert(assets, Asset("ANIM", "anim/" .. v .. ".zip")) end end

    local prefabs = {data.prefab .. "_inv", "schoolherd_" .. data.prefab, "spoiled_fish", data.cooking_product}
    ConcatArrays(prefabs, data.loot, data.extra_prefabs)
    data.heavyfx2hm = data.prefab .. "_heavyfx2hm"
    table.insert(fish_prefabs, Prefab(data.prefab .. "_heavy2hm", function() return water_common(data) end, assets, prefabs))
    table.insert(fish_prefabs, Prefab(data.prefab .. "_heavyinv2hm", function() return inv_common(data) end, assets, prefabs))
    -- if CopyPrefabNameAndDesc then CopyPrefabNameAndDesc(data.prefab .. "_heavyinv2hm", data.prefab .. "_inv", TUNING.isCh2hm and "巨大的" or "heavy ") end
    table.insert(fish_prefabs, Prefab(data.prefab .. "_heavyfx2hm", function() return fx_common(data) end, assets))
end

for _, fish_def in pairs(FISH_DATA.fish) do MakeFish(fish_def) end
