...
local prefabs = {"walrus", "little_walrus", "icehound"}
...
local function GetStatus(inst) if not inst.data.occupied then return "EMPTY" end end
...
local function UpdateCampOccupied(inst)
    if inst.data.occupied then
        if not TheWorld.state.iswinter then
            ...
            for k in pairs(inst.data.children) do k:Remove() end
            inst.data.children = {}
...
local function RemoveMember(inst, member)
    ...
    if inst:IsAsleep() then UpdateCampOccupied(inst) end
end

local function OnMemberKilled(inst, member)
    if inst.components.worldsettingstimer:ActiveTimerExists(member.prefab) then inst.components.worldsettingstimer:StopTimer(member.prefab) end
...
local function TrackMember(inst, member)
    ...
    if member.components.homeseeker == nil then member:AddComponent("homeseeker") end
...
local function GetMember(inst, prefab) for k in pairs(inst.data.children) do if k.prefab == prefab then return k end end end

local function GetMembers(inst, prefab)
    local members = {}
    for k in pairs(inst.data.children) do if k.prefab == prefab then table.insert(members, k) end end
    return members
end

local function CanSpawn(inst, prefab) return TUNING.WALRUS_REGEN_ENABLED and not inst.components.worldsettingstimer:ActiveTimerExists(prefab) end
...
local function SpawnHuntingParty(inst, target, houndsonly)
    ...
    if companion and leader then companion.components.follower:SetLeader(leader) end
    ...
    if target then
        if companion then companion.components.combat:SuggestTarget(target) end
        if leader then leader.components.combat:SuggestTarget(target) end
    end

    for i, v in ipairs(transformsToSet) do v.inst.Transform:SetPosition(v.x, v.y, v.z) end
end
...
OnMemberNewTarget = function(inst, member, data) if member:IsNear(inst, AGGRO_SPAWN_PARTY_RADIUS) then CheckSpawnHuntingParty(inst, data.target, false) end end
...
local function OnEntityWake(inst) end

local function OnStartDay(inst) CheckSpawnHuntingParty(inst, nil, false) end
...
local function OnSave(inst, data)
    ...
    for k in pairs(inst.data.children) do table.insert(data.children, k.GUID) end

    if #data.children < 1 then data.children = nil end
...
local function OnLoad(inst, data)
    if data then
        ...
        if data.occupied ~= nil then SetOccupied(inst, data.occupied) end

        if data.regentimeremaining then
            for k, v in pairs(data.regentimeremaining) do if v > 0 then inst.components.worldsettingstimer:StartTimer(k, v) end end
        end
    end
end
    ...
    inst:DoTaskInTime(5 + (math.random() * 20), function()
        if data.sourcept and TheWorld.Map:IsVisualGroundAtPoint(data.sourcept.x, data.sourcept.y, data.sourcept.z) then
            ...
            if inst.data.children then
                for k in pairs(inst.data.children) do
                    if k:IsValid() then
                        ...
                        if #players > 0 then engaged = true end

                        if k.components.combat.target then engaged = true end
                    end
                end
            end
                ...
                if #players > 0 then
                    local offset = FindValidPositionByFan(math.random() * 2 * PI, 40, 32, function(testoffset)
                        ...
                        if TheWorld.Map:IsAboveGroundAtPoint(newpt.x, newpt.y, newpt.z) then
                            ...
                            if #testplayers == 0 then return true end
                        end
                    end)

                    if offset then spawnpoint = data.sourcept + offset end
                end
            end
            ...
            if spawnpoint then
                ...
                if #ents > 0 then spawnpoint = nil end
            end
...
local function create()
    ...
    if not TUNING.walrus_camp2hm then inst:DoTaskInTime(0, inst.Remove) end
    inst:DoTaskInTime(0, inst.Hide)

    return inst
end

return Prefab("walrus_camp2hm", create, assets, prefabs)
