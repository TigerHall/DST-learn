local assets = {Asset("ANIM", "anim/x_marks_spot.zip"), Asset("MINIMAP_IMAGE", "pirate_stash")}
...
local function fling_loot_in_slot(inst, slot)
    ...
    if loot ~= nil then
        ...
        if loot ~= nil and loot:IsValid() then Launch(loot, loot, 2) end
    end
...
local function stash_dug(inst)
    if not inst.flinging then
        ...
        for k in pairs(inv.itemslots) do queue_fling_in_slot(inst, k) end
    end
end

local function hascopyof(inst, item)
    for k, v in pairs(inst.components.inventory.itemslots) do
        if v ~= item and v.prefab == "blueprint" and v.recipetouse == item.recipetouse then return true end
    end
    return false
end

local function checkistreasure(inst, item)
    return item.prefab == "blueprint" and (item.recipetouse == "pirate_flag_pole" or item.recipetouse == "polly_rogershat") and not hascopyof(inst, item)
end
    ...
    if item ~= nil and item:IsValid() then
        ...
        repeat
            ...
            if olditem == nil then
                ...
                if inst.flinging then queue_fling_in_slot(inst, inst.nextslot) end
...
local function OnSave(inst, data) data.nextslot = inst.nextslot > 1 and inst.nextslot or nil end

local function OnLoad(inst, data)
    if data ~= nil and data.nextslot ~= nil and data.nextslot <= TUNING.PIRATE_STASH_INV_SIZE then inst.nextslot = data.nextslot end
end
...
local function fn()
    ...
    inst:ListenForEvent("onremove", function() if TheWorld.components.piratespawner then TheWorld.components.piratespawner:ClearCurrentStash() end end)
...
return Prefab("pirate_stash2hm", fn, assets)
