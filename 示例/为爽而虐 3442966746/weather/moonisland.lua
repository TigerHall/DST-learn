...
-- 修复天体贡品因召唤天体英雄消失，探测仪会错误地指向帝王蟹位置的问题
...
local function OnUpdateLight(inst, dt)
    ...
    local lerpk = inst._endlight.time > 0 and inst._currentlight.time / inst._endlight.time or 1
    for k, v in pairs(inst._endlight) do
        if type(v) == "table" then
            for k2, v2 in pairs(v) do
                if type(v2) == "number" then
                    inst._currentlight[k][k2] = inst._startlight[k][k2] * (1 - lerpk) + v2 * lerpk
                end
            end
        elseif type(v) == "number" and k ~= "time" then
            inst._currentlight[k] = inst._startlight[k] * (1 - lerpk) + v * lerpk
        end
    end
    
    if inst.Light then
        inst.Light:SetRadius(inst._currentlight.radius)
        inst.Light:SetIntensity(inst._currentlight.intensity)
        inst.Light:SetFalloff(inst._currentlight.falloff)
        inst.Light:SetColour(unpack(inst._currentlight.colour))
    ...
    if inst.AnimState then
        local light_override = math.max(0, math.min(1, inst._currentlight.intensity / 0.8))
...
AddPrefabPostInit("archive_resonator_item", function(inst)
    if not TheWorld.ismastersim then return end
    ...
    if inst.components.deployable then
        ...
        inst.components.deployable.ondeploy = function(inst, pt, deployer)
            -- 检查是否存在天体英雄
            local alterguardian = FindEntity(inst, 9999, nil, {"brightmareboss"})
            
            if alterguardian then
                ...
                if at ~= nil then
                    ...
                    local light_off = {radius = 0, intensity = 0, falloff = 0.6, colour = {0, 0, 0}, time = 1}
                    copyparams(at._currentlight, light_off)
                    copyparams(at._startlight, light_off)
                    copyparams(at._endlight, light_off)
                    
                    -- 设置灯光渐变
                    at.task1 = at:DoTaskInTime(83/30, function()
                        copyparams(at._endlight, light_params_idle)
                        beginfade(at)
                    end)
                    
                    -- 直接执行持续发光逻辑，跳过扫描
                    at.task2 = at:DoTaskInTime(5, function()
                        at.task3 = at:DoTaskInTime(4, function()
                            at.SoundEmitter:KillSound("locating")
                            at.AnimState:PlayAnimation("idle_pre")
                            copyparams(at._endlight, light_params_idle)
                            beginfade(at)
                            at.AnimState:PushAnimation("idle_loop", true)
                            at.components.finiteuses:Use(1)
                        end)
                    end)
...
for index, marker in ipairs(markers) do 
    AddPrefabPostInit(marker, initmarker) 
end
    ...
    local function ongogglevision(inst, data)
        ...
        if not inst:HasTag("playerghost") and data and data.enabled and inst.components.stormwatcher and inst.components.inventory then
            ...
            elseif inst.components.stormwatcher:GetCurrentStorm() == STORM_TYPES.SANDSTORM then
                local v = inst.components.inventory:GetEquippedItem(EQUIPSLOTS.HEAD)
                if v and v:HasTag("goggles") and v.prefab ~= "alterguardianhat" and
