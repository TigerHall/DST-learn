...
local function exchangedestroydata(inst)
    if inst.components.container then
        ...
        for i = 1, container.numslots do
            ...
            if item ~= nil and item:IsValid() then 
                -- 为牛铃添加穿越保护，防止牛鞍刷物资
                if item.prefab == "beef_bell" or item.prefab == "shadow_beef_bell" then
                    local beefalo = item:GetBeefalo()
...
-- 自动暂停
local function exchangeannounce(countdown)
    local message = (TUNING.isCh2hm and ("世界容器穿越失败，"..countdown.."秒后服务器暂停") 
                    or ("Container exchange failed, server will pause in "..countdown.." seconds"))
    TheNet:Announce(message)
end

local function pausetask(inst)
    exchangeannounce(3)
    inst:DoTaskInTime(1, function() 
        ...
        inst:DoTaskInTime(1, function()
            ...
            inst:DoTaskInTime(1, function()
                ...
                TheNet:Announce((TUNING.isCh2hm and "服务器已暂停，15秒后恢复" 
                               or "Server paused, will resume in 15 seconds"))
                inst:DoStaticTaskInTime(15, function() 
                    if TheNet:IsServerPaused() then
                        TheNet:SetServerPaused(false)
                        TheNet:Announce((TUNING.isCh2hm and "服务器已恢复运行" or "Server has resumed"))
                    end
                end)
            end)
        end)
...
local function dopickup(inst, player)
    ...
    for i = #nearents, 1, -1 do
        ...
        if v:IsValid() and table.contains(data.prefabs, v.prefab) and v ~= inst and not v:IsInLimbo() and not iscontainerchild(inst, v) and
            ...
            if data.infiniteprefabs and data.infiniteprefabs[v.prefab] then
                -- 无限需求堆叠道具,所以可以无限给予
                showmovefx(v)
            ...
            elseif data.leftnumslots > 0 then
                -- 没有重叠格子但有空白格子（包括非堆叠物品）
                showmovefx(v)
                if v.components.stackable then
                    local slotsize = v.components.stackable.originalmaxsize or v.components.stackable.maxsize
                    local stacksize = v.components.stackable.stacksize
                    -- 先给予物品，让游戏自动处理堆叠
                    data.finiteuses = data.finiteuses - math.max(stacksize / 4, 1)
                    container:GiveItem(v, nil, data.entpos)
                    -- 重新计算剩余空间
                    local emptyslots = 0
                    local newlacksize = {}
                    ...
                    for i = 1, numslots do
                        ...
                        elseif slot.components.stackable and slot.components.stackable.stacksize < slot.components.stackable.maxsize then
                            local prefab = slot.prefab
                            if table.contains(data.prefabs, prefab) then
                                newlacksize[prefab] = (newlacksize[prefab] or 0) + 
                                    (slot.components.stackable.maxsize - slot.components.stackable.stacksize)
                            end
                        end
                    end
                    data.leftnumslots = emptyslots
                    data.lacksize = newlacksize
...
local lockbtninfo = {
    ...
    helptext1 = TUNING.isCh2hm and [[限制该容器仅可放置此时已存道具且记忆]] or
...
if containercfg and isclient then
    local btnlist = {
        sortbtn = {
            ...
            helptext = TUNING.isCh2hm and [[排序库存]] or [[Sort Your Items]],
            fn = realsortbtnfn,
            validfn = defaultbtnvalidfn
        },
        lockbtn = lockbtninfo,
        multisortbtn = {
            ...
            helptext = TUNING.isCh2hm and [[跨容器整理]] or [[Sort Your Items Pass Through Containers]],
            fn = multisortbtnfn,
            validfn = defaultbtnvalidfn
        },
        collectbtn = {
            text = TUNING.isCh2hm and "收集" or "Collect",
            helptext = TUNING.isCh2hm and [[收集附近的同名物品]] or [[Collect Same Items]],
            fn = collectbtnfn,
            validfn = defaultbtnvalidfn
        },
        exchangebtn = {
            text = TUNING.isCh2hm and "穿越" or "PassWorld",
            helptext = TUNING.isCh2hm and [[跨世界容器交换道具]] or
                [[exchange container data with another world]],
            fn = exchangebtnfn,
    ...
    local _itemtestfn = containers.params.shadow_container.itemtestfn
    function containers.params.shadow_container.itemtestfn(container, item, slot)
        if item:HasTag("nopassworld2hm") then return end
        return _itemtestfn(container, item, slot)
    end
...
-- ===================================================================================
-- 参考workshop-3424065137模组，支持显示超过999的真实堆叠数量
AddClassPostConstruct("components/stackable_replica", function(self, inst)
    -- 使用两个网络变量突破堆叠数量限制（最大支持65536*65536）
    self._newstacksizeupper = net_ushortint(inst.GUID, "stackable._newstacksizeupper", "stacksizedirty")
    self._newstacksize = net_ushortint(inst.GUID, "stackable._newstacksize", "stacksizedirty")
end)

local stackable_replica = require("components/stackable_replica")
stackable_replica.StackSize = function(self)
    return self:GetPreviewStackSize() or (self._newstacksizeupper:value() * 65536 + self._newstacksize:value() + 1)
end

stackable_replica.SetStackSize = function(self, stacksize)
    stacksize = stacksize - 1
    if stacksize <= 65535 then
        self._newstacksizeupper:set(0)
        self._newstacksize:set_local(stacksize)
        self._newstacksize:set(stacksize)
    else
        local upper = math.floor(stacksize / 65536)
        self._newstacksizeupper:set(upper)
        self._newstacksize:set_local(stacksize - upper * 65536)
        self._newstacksize:set(stacksize - upper * 65536)
    end
end

-- 取消999+限制，根据数量动态调整字体大小
AddClassPostConstruct("widgets/controls", function()
    ...
    function ItemTile:SetQuantity(quantity)
        ...
        local size = quantity > 1000000000 and 24 or 
                     quantity > 100000000 and 26 or 
                     quantity > 10000000 and 28 or 
                     quantity > 1000000 and 30 or 
                     quantity > 100000 and 32 or 
                     quantity > 10000 and 34 or 
                     quantity > 1000 and 36 or 42
