...
local function spawnreplacedself(inst, target)
    if TheWorld.components.lunarthrall_plantspawner then
        -- 检查转移次数限制
        inst.lunarthrall2hmtransfercount = inst.lunarthrall2hmtransfercount or 0
        if inst.lunarthrall2hmtransfercount >= 2 then
            -- 已达到转移次数上限，不再转移
            return
        end
        ...
        -- 传递转移次数
        target.lunarthrall2hmtransfercount = inst.lunarthrall2hmtransfercount + 1
        ...
        target.lunarthrall2hmhealthtask = target:DoTaskInTime(45, function()
            ...
            if target.lunarthrall2hmtransfercount then target.lunarthrall2hmtransfercount = nil end
        end)
        
        -- 原位置生成2个茄壳
        local x, y, z = inst.Transform:GetWorldPosition()
        for i = 1, 2 do
            local brightshade = SpawnPrefab("lunarplant_husk")
            if brightshade then
                local angle = math.random() * 2 * PI
                local radius = 0.5 + math.random() * 0.5
                local offset_x = x + radius * math.cos(angle)
                local offset_z = z + radius * math.sin(angle)
                brightshade.Transform:SetPosition(offset_x, y, offset_z)
            end
        end

        local thrall = SpawnPrefab("lunarthrall_plant_gestalt")
        thrall.plant_target = target
        thrall.Transform:SetPosition(x, y, z)
...
local function sethealthtrigger(inst)
    ...
    if not inst.target2hm then
        inst.maxvines = 3  -- 默认最大数量为3
    elseif inst.components.health then
        local percent = inst.components.health:GetPercent()
        
        -- 根据血量阶段确定最大藤蔓数量：血量越低，藤蔓越少
    ...
    -- vinelimit表示剩余可生成的藤蔓配额，需要减去已有的藤蔓数量
    inst.vinelimit = math.max(0, (inst.maxvines or 3) - #inst.vines)
...
local function OnWakeTask(inst)
    ...
    if not inst.components.health:IsDead() and inst.sg.laststate and inst.sg.laststate.name == "idle" and inst.showtired2hm then
        -- 检查是否已经处于真实的虚弱状态（没有藤蔓时）
        if #inst.vines == 0 then
            -- 如果已经真实虚弱，取消伪装恢复
            inst.showtired2hm = nil
            inst.showtiredpst2hm = nil
        else
            -- 重新计算最大藤蔓数量和剩余配额
            sethealthtrigger(inst)
...
AddStategraphPostInit("lunarthrall_plant", function(sg)
    ...
    sg.states.idle.onenter = function(inst, ...)
        ...
        elseif (#inst.vines + inst.vinelimit) < TUNING.LUNARTHRALL_PLANT_VINE_LIMIT then
            -- 丢失藤曼后,检查是否需要伪装
            -- 当前藤蔓数量小于最大允许数量时，可能需要伪装恢复
            local maxallowed = inst.maxvines or TUNING.LUNARTHRALL_PLANT_VINE_LIMIT
            if #inst.vines < maxallowed then
    ...
    -- 拦截原版的tired_pst状态退出，防止vinelimit被重置
    local tired_pst_onexit = sg.states.tired_pst.onexit
    sg.states.tired_pst.onexit = function(inst, ...)
        if tired_pst_onexit then tired_pst_onexit(inst, ...) end
        -- 原版会设置 inst.vinelimit = TUNING.LUNARTHRALL_PLANT_VINE_LIMIT
        -- 我们需要根据当前血量和已有藤蔓重新计算
        if inst.maxvines then
            inst.vinelimit = math.max(0, inst.maxvines - #inst.vines)
        end
    end
end)

-- 亮茄血量越低,藤蔓越少,且寄生食人花会改变其再生速率,令其防火
AddPrefabPostInit("lunarthrall_plant", function(inst)
    if not TheWorld.ismastersim then return end
    
    -- 初始化最大藤蔓数量
    inst.maxvines = TUNING.LUNARTHRALL_PLANT_VINE_LIMIT
    ...
    -- 拦截原版的 OnWakeTask 和 OnFreeze，防止 vinelimit 被错误重置
    local old_OnFreeze = inst.OnFreeze
    inst.OnFreeze = function(inst, ...)
        if old_OnFreeze then old_OnFreeze(inst, ...) end
        -- 原版会重置 vinelimit，我们需要重新计算
        if inst.maxvines then
            inst.vinelimit = math.max(0, inst.maxvines - #inst.vines)
        end
    end
    ...
    inst.infest = function(inst, target, ...)
        ...
        -- 继承转移次数
        if target.lunarthrall2hmtransfercount then
            inst.lunarthrall2hmtransfercount = target.lunarthrall2hmtransfercount
            target.lunarthrall2hmtransfercount = nil
        end
        
        -- 根据血量阶段确定最大藤蔓数量
        local percent = inst.components.health:GetPercent()
        if percent > 0.66 then
            inst.maxvines = 3  -- 血量>66%时：最多3个藤蔓
        elseif percent > 0.33 then
            inst.maxvines = 2  -- 血量33%-66%时：最多2个藤蔓
        else
            inst.maxvines = 1  -- 血量<33%时：最多1个藤蔓
        end
...
local lunarplant_targetplants = {"lureplant", "rock_avocado_bush", "bananabush", "monkeytail"}
...
local function trygrowlureplant(inst)
    if inst:HasTag("planted") then
        ...
        -- 立即进入交易冷却
        inst.lureplant_trade_day2hm = TheWorld.state.cycles
...
-- 检测物品是否是肉类
local function IsMeatItem(item)
...
-- 检测物品是否有怪物度
local function HasMonsterValue(item)
    if not item or not item.components.edible then 
        return false 
    end
    
    local health_value = item.components.edible.healthvalue or 0
    if health_value < 0 then
        return true
    end
    
    if item.prefab then
        local ingredient_data = cooking.ingredients[item.prefab]
        if ingredient_data and ingredient_data.tags and ingredient_data.tags.monster and ingredient_data.tags.monster > 0 then
            return true
        end
    end
...
local function LureplantShouldAcceptItem(inst, item)
    if not inst:HasTag("planted") then
        return false
    end
    -- 有叶肉了
    if inst.components.shelf and inst.components.shelf.itemonshelf and inst.components.shelf.itemonshelf:IsValid() then
        return false
    end
    -- 已经交易过了
    if inst.lureplant_trade_day2hm and inst.lureplant_trade_day2hm == TheWorld.state.cycles then
        return false
    end
    -- 检查是否是肉类
    return IsMeatItem(item)
end

local function LureplantOnGetItemFromPlayer(inst, giver, item)
    if item then
        local has_monster = HasMonsterValue(item)
        
        -- 生成叶肉
        local plantmeat = SpawnPrefab("plantmeat")
        if plantmeat then
            -- 如果有怪物度属性，叶肉新鲜度为50%，否则为满新鲜度
            if has_monster and plantmeat.components.perishable then
                plantmeat.components.perishable:SetPercent(0.5)
            end
            
            if inst.components.inventory then
                inst.components.inventory:GiveItem(plantmeat)
            end
        end
...
local function LureplantOnRefuseItem(inst, giver, item)
    if giver and giver.components.talker then
        giver.components.talker:Say(TUNING.isCh2hm and "我得再等等它" or "I should wait longer")
    end
end
...
AddPrefabPostInit("lureplant", function(inst)
    ...
    inst.components.trader:SetAcceptTest(LureplantShouldAcceptItem)
    inst.components.trader.onaccept = LureplantOnGetItemFromPlayer
    inst.components.trader.onrefuse = LureplantOnRefuseItem
...
-- 亮茄寄生动画保护，不可选中和攻击
AddStategraphPostInit("lunarthrall_plant", function(sg)
    if sg.states.spawn then
        local spawn_onenter = sg.states.spawn.onenter
        sg.states.spawn.onenter = function(inst, ...)
            if spawn_onenter then spawn_onenter(inst, ...) end
            inst:AddTag("NOCLICK")
            inst:AddTag("notarget")
        end
        
        local spawn_onexit = sg.states.spawn.onexit
        sg.states.spawn.onexit = function(inst, ...)
            inst:RemoveTag("NOCLICK")
            inst:RemoveTag("notarget")
            if spawn_onexit then spawn_onexit(inst, ...) end
        end
    end
end)
