...
local function newEvaluateTechTrees(self)
    ...
    if not prototyper_active then
        -- 如果没有激活的原型机，只使用角色自身加成
        for i, v in ipairs(TechTree.AVAILABLE_TECH) do
            self.accessible_tech_trees_no_temp[v] = (self[TechTree.AVAILABLE_TECH_BONUS[v] or string.lower(v).."_bonus"] or 0)
            self.accessible_tech_trees[v] = (self[TechTree.AVAILABLE_TECH_TEMPBONUS[v] or string.lower(v).."_tempbonus"] or 0) + 
                                           (self[TechTree.AVAILABLE_TECH_BONUS[v] or string.lower(v).."_bonus"] or 0)
        end
    else
        -- 如果有激活的原型机，将角色加成加到原型机科技上
        for i, v in ipairs(TechTree.BONUS_TECH) do
            self.accessible_tech_trees_no_temp[v] = self.accessible_tech_trees_no_temp[v] + 
                                                   (self[TechTree.AVAILABLE_TECH_BONUS[v] or string.lower(v).."_bonus"] or 0)
    ...
    if old_prototyper ~= nil and old_prototyper ~= self.current_prototyper and 
        old_prototyper.components.prototyper ~= nil and old_prototyper.entity:IsValid() then
...
AddComponentPostInit("book", function(self)
    local old_Interact = self.Interact
    self.Interact = function(self, fn, reader, ...)
        local success, reason = old_Interact(self, fn, reader, ...)
        
        -- 检查是否是万物百科（book_research_station）
        if success and self.inst.prefab == "book_research_station" and reader.components.builder then
            -- 获取当前的临时科技加成
            local temp_bonuses = reader.components.builder:GetTempTechBonuses()
            
            -- 将 science、magic、seafaring 的临时加成限制为 1
            if temp_bonuses.SCIENCE and temp_bonuses.SCIENCE > 1 then
                temp_bonuses.SCIENCE = 1
            end
            if temp_bonuses.MAGIC and temp_bonuses.MAGIC > 1 then
                temp_bonuses.MAGIC = 1
            end
            if temp_bonuses.SEAFARING and temp_bonuses.SEAFARING > 1 then
                temp_bonuses.SEAFARING = 1
            end
            
            -- 重新应用修改后的临时加成
            reader.components.builder:GiveTempTechBonus(temp_bonuses)--这里获得额外的一次加成，共两次
        end
...
AddComponentPostInit("builder", function(self)
    ...
    self.GiveTempTechBonus = function(self, tech, ...)
        if tech then 
            -- 获得制图桌科技
            tech.CARTOGRAPHY = TUNING.FUNCTIONAL_MEDAL_IS_OPEN and 2 or nil 
        end
