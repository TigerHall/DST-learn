...
local function changeWorldShardPortals(inst, change)
    if ShardPortals == nil or #ShardPortals <= 0 then return end
    -- 排除海上巨型漩涡/档案馆楼梯
    local ShardPortals_temp = {}
    for i,v in ipairs(ShardPortals) do
        if v.prefab ~= "oceanwhirlbigportal" and v.prefab ~= "oceanwhirlbigportalexit" then
            table.insert(ShardPortals_temp, v)
        end
    end
    if #ShardPortals_temp <= 0 then return end
    if not inst.components.persistent2hm.data.unlockShardPortals or change then
        ...
        local i = math.random(#ShardPortals_temp)
        table.insert(inst.components.persistent2hm.data.unlockShardPortals, ShardPortals_temp[i].components.worldmigrator.id)
        if #ShardPortals_temp > 1 then
            local i2 = math.random(#ShardPortals_temp)
            while i2 == i do i2 = math.random(#ShardPortals_temp) end
            table.insert(inst.components.persistent2hm.data.unlockShardPortals, ShardPortals_temp[i2].components.worldmigrator.id)
        end
    end
    for i, exit in ipairs(ShardPortals_temp) do
        ...
        if (exit.prefab == "cave_exit" or exit.prefab == "cave_entrance_open" or exit.enablelockworldmigrator2hm) and
            table.contains(inst.components.persistent2hm.data.unlockShardPortals, exit.components.worldmigrator.id) then
