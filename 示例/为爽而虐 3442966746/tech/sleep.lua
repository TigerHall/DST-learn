        ...
        if self.inst.components.sanity and self.inst.components.health then
            ...
            if self.bed and self.bed:IsValid() then
                if self.bed.components.persistent2hm and self.bed.components.persistent2hm.data.upgrade then
                    ...
                    health = 1
                    sanity = 1
                elseif (self.bed.components.inventoryitem and self.bed.components.inventoryitem.owner) or 
                        (GetModConfigData("walter") and self.bed.components.portablestructure) then
                    sleepout = true  
                end
            end
            if upgrade then
