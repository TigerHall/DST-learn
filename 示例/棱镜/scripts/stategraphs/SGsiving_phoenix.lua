...
local function GetTarget(inst) --优先获取自己的敌人，其次才是伴侣的敌人
    ...
    if target and IsValid(target) and not target:HasTag("playerghost") then
        return target
    end
    ...
    State{ --死亡
        ...
        onenter = function(inst)
            ...
            if inst:IsInLimbo() and inst.tree and inst.tree:IsValid() then --在隐藏状态死亡的话，就让掉落物产生在神木岩附近
                ...
                pos.y = 0 --玄鸟飞行时也可能死亡，所以战利品掉落高度必须为0
                inst.components.lootdropper:DropLoot(pos)
            end
