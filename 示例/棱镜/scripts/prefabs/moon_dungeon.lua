...
local blueprints = {
    "hiddenmoonlight_item_blueprint", "revolvedmoonlight_item_blueprint", "simmeredmoonlight_item_blueprint"
}
...
local function GetDungeonProtector(inst, protector, protdata)
    ...
    if monster.components.lootdropper ~= nil then
        if protector == "stalker_forest" then --森林影织者还是掉落一些关键物品吧，不然好没用
            ...
            monster.fn_l_onDead = function(monster) --官方检查了 persists 变量，所以这里换个写法
                ...
                if math.random() < 0.045 then
                    lootdropper:SpawnLootPrefab(GetRandomItem(blueprints), mypos)
                end
            end
        ...
        if math.random() < 0.045 then
            monster.components.lootdropper:AddChanceLoot(GetRandomItem(blueprints), 1.0)
        end
    end
