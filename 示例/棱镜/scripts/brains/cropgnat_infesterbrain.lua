...
local tags_cant_infest = TOOLS_L.TagsCombat2({ "nognatinfest", "insect", "glommer", "friendlyfruitfly", "shadowminion", "ghost" })
...
local function GetLightTarget(inst)
    if TheWorld.state.isday or TheWorld.state.isfullmoon then --白天或满月夜，肯定不需要寻求光源
...
local function GetInfestTarget(inst)
    ...
    then
        ...
        end, { "_combat", "_health" }, tags_cant_infest, nil)
