...
local SkinTravelScreen = Class(Screen, function(self, owner, attach)
...
function SkinTravelScreen:LoadDests()
    local info_pack = self.attach.replica.skintravelable and
        self.attach.replica.skintravelable:GetDestInfos()
    self.dest_infos = {}
...
function SkinTravelScreen:SkinTravel(index)
    if not self.isopen then return end

    local skintravelable = self.attach.replica.skintravelable
    if skintravelable then skintravelable:SkinTravel(self.owner, index) end

    self.owner.HUD:CloseSkinTravelScreen()
end
...
function SkinTravelScreen:OnCancel()
    ...
    local skintravelable = self.attach.replica.skintravelable
    if skintravelable then skintravelable:SkinTravel(self.owner, nil) end
